...
class RectanglesInColumnTestCase(
    ...
    lateinit var rootView: AndroidCraneView

    fun runSetup() {
        rootView = AndroidCraneView(activity)
        activity.setContentView(rootView)
        compositionContext = ComposeMaterialIntoWrapper(rootView) {
            Column {
                for (i in 0 until amountOfRectangles) {
                    if (i == 0) {
