...
fun SingleNodeQuery.doClick(): SingleNodeQuery {
    baseNodeQuery.doClickInternal(findExactlyOne())

    return this
}

fun MultipleNodesQuery.doClickAll(): MultipleNodesQuery {
    baseNodeQuery.doClickInternal(findAtLeastOne())

    return this
}

internal fun BaseNodeQuery.doClickInternal(nodes: List<SemanticsTreeNode>) {
    nodes.forEach {
