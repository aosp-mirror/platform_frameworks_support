...
internal class AndroidSemanticsTreeInteraction internal constructor(
    ...
    override fun findAllMatching(): List<SemanticsTreeNode> {
        ...
        waitForIdleComposeScreen(collectedInfo.context)
    ...
    private fun performAction(action: (SemanticsTreeProvider) -> Unit) {
        ...
        // potential flakes.
        hadPendingChangesAfterLastAction = waitForIdleComposeScreen(collectedInfo.context)
    }

    /**
     * Waits for Compose to be idle on screen. By screen we mean all the Compose hierarchies that
     * belong to the current visible activity.
     *
     * @return Whether the method had to wait for pending changes or not.
     */
    private fun waitForIdleComposeScreen(context: Context): Boolean {
    ...
    private fun scheduleIdleCheck(context: Context, latch: CountDownLatch) {
        Choreographer.getInstance().postFrameCallback(object : Choreographer.FrameCallback {
            @SuppressLint("SyntheticAccessor")
            override fun doFrame(frameTimeNanos: Long) {
                if (Compose.hasPendingChanges(context)) {
                    scheduleIdleCheck(context, latch)
                } else {
                    latch.countDown()
                }
    ...
    private class AggregatedSemanticTreeProvider(
        ...
        override fun getAllSemanticNodes(): List<SemanticsTreeNode> {
            ...
            return collectedRoots.map { it.getAllSemanticNodes() }.flatten()
        }
