...
class AppBarTest {
    ...
    fun topAppBar_defaultPositioning() {
        ...
        withDensity(composeTestRule.density) {
            ...
            val navigationIconPositionX = navigationIconCoords!!
                .localToGlobal(PxPosition.Origin)
                .x
                .round()
            val navigationIconExpectedPositionX = 16.dp.toIntPx()
            Truth.assertThat(navigationIconPositionX).isEqualTo(navigationIconExpectedPositionX)

            // Title should be next
            val titlePositionX = titleCoords!!.localToGlobal(PxPosition.Origin).x.round()
            val titleExpectedPositionX = navigationIconPositionX +
                    navigationIconCoords!!.size.width.round() + 32.dp.toIntPx()
            Truth.assertThat(titlePositionX).isEqualTo(titleExpectedPositionX)

            // Action should be placed at the end
            val actionPositionX = actionCoords!!.localToGlobal(PxPosition.Origin).x.round()
            val actionExpectedPositionX =
                appBarCoords!!.size.width.round() - 16.dp.toIntPx() - 24.dp.toIntPx()
    ...
    fun bottomAppBar_centerFab_positioning() {
        ...
        withDensity(composeTestRule.density) {
            ...
            val fabPositionX = fabCoords!!.localToGlobal(PxPosition.Origin).x.round()
            val fabExpectedPositionX = (appBarCoords!!.size.width.round() / 2) - 12.dp.toIntPx()
    ...
    fun bottomAppBar_endFab_positioning() {
        ...
        withDensity(composeTestRule.density) {
            ...
            val actionPositionX = actionCoords!!
                .localToGlobal(PxPosition.Origin)
                .x
                .round()
            val actionExpectedPositionX = 16.dp.toIntPx()
            Truth.assertThat(actionPositionX).isEqualTo(actionExpectedPositionX)

            // FAB should be placed at the end
            val fabPositionX = fabCoords!!.localToGlobal(PxPosition.Origin).x.round()
            val fabExpectedPositionX = appBarCoords!!.size.width.round() - 16.dp.toIntPx() -
                    24.dp.toIntPx()
            Truth.assertThat(fabPositionX).isEqualTo(fabExpectedPositionX)
        }
    }
