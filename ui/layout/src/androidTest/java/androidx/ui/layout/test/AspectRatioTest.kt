...
class AspectRatioTest : LayoutTest() {
    ...
    fun testAspectRatio_withNoChild() = withDensity(density) {
        ...
        assertEquals(sizeDp.toPx(), size[0].value!!.width)
        assertEquals(sizeDp.toPx(), size[0].value!!.height)
        assertNotNull(size[1].value)
        assertEquals(sizeDp.toPx(), size[1].value!!.width)
        assertEquals(sizeDp.toPx(), size[1].value!!.height)
    }
