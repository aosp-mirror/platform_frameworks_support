...
class TextPainter(
        ...
        get() {
            if (layoutTemplate == null) {
                ...
                layoutTemplate = Paragraph(
                    ...
                    textStyleRecords = textStyle?.let {
                        listOf(AnnotatedString.TextStyleRecord(it, 0, 1))
                    } ?: listOf()
                )
    ...
    private fun layoutText(minWidth: Float = 0.0f, maxWidth: Float = Float.POSITIVE_INFINITY) {
        ...
        needsLayout = false
        if (paragraph == null) {
            paragraph = Paragraph(text!!.text, createParagraphStyle(), text!!.textStyleRecords)
        }
    ...
    fun layout(constraints: Constraints) {
        ...
        overflowShader = if (hasVisualOverflow && overflow == TextOverflow.Fade) {
            val fadeSizePainter = TextPainter(
                text = AnnotatedString(text = "\u2026", textStyleRecords = listOf()),
                style = textStyle,
