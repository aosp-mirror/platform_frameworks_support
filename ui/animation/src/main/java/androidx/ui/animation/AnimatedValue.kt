...
@Model
class AnimatedFloat private constructor(private val animValue: AnimatedValueImpl<Float>)
    : DynamicTargetAnimation<Float> by animValue {
    init {
        animValue.onValueChanged = { value = it }
    }

    constructor(initValue: Float, valueInterpolator: ((Float, Float, Float) -> Float) = ::lerp)
            : this(AnimatedValueImpl(initValue, valueInterpolator)) {}

    override var value: Float = animValue.value
        private set
}
