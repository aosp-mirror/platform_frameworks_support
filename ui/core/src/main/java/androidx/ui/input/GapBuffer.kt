...
private class GapBuffer(initBuffer: CharArray, initGapStart: Int, initGapEnd: Int) {
    ...
    private fun makeSureAvailableSpace(requestSize: Int) {
        ...
        val shortfall = requestSize - gapLength()

        var newCapacity = capacity * 2
        while ((newCapacity - capacity) < shortfall) {
            newCapacity *= 2
        }

        if (newCapacity >= 512 * 1024) {
            Log.w("GapBuffer", "Allocating 1MB buffer. Consider alternative data structure.")
        }
    ...
    private fun delete(start: Int, end: Int) {
        if (start < gapStart && end <= gapStart) {
            ...
            val startInBuffer = start + if (start >= gapStart) gapLength() else 0
            val endInBuffer = end + if (end >= gapStart) gapLength() else 0
...
/**
 * A modified Gap Buffer implementation
 *
...
internal class PartialGapBuffer(var text: String) {
    private val BUF_SIZE = 255
    private val SURROUNDING_SIZE = 64
    private val NOWHERE = -1
