...
/**
 * Create a new Canvas instance that targets it's drawing commands
...
/**
 * Create a new Canvas instance that targets it's drawing commands to the provided
 ...
 *    commands
 */
/* expect */ inline fun Canvas.save(block: () -> Unit) {
    try {
        nativeCanvas.save()
        block()
    } finally {
...
inline fun Canvas.saveLayer(bounds: Rect?, paint: Paint, block: () -> Unit) {
    try {
        nativeCanvas.saveLayer(bounds?.toFrameworkRectF(), paint.toFrameworkPaint())
        block()
    } finally {
        nativeCanvas.restore()
    }
