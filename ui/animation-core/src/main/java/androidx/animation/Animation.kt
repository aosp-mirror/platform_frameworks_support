 ...
 * Animation<T>, DecayAnimation<T> does not have an end value defined. The end value is a
 * result of the animation rather than an input.
 */
interface DecayAnimation<T> {
    fun isFinished(
        playTime: Long,
        start: T,
        startVelocity: Float
...
class ExponentialDecay (frictionMultiplier : Float = 1f) : DecayAnimation<Float> {
...
internal fun DecayAnimation<Float>.createWrapper(startValue: Float,
...
internal class DecayAnimationWrapper(
    ...
    private val startVelocity: Float = 0f,
    private val anim: DecayAnimation<Float>
) : AnimationWrapper<Float> {
