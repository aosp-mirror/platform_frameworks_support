...
interface DynamicTargetAnimation<T> {
    ...
    val target: T
    // TODO: use lambda with default values to combine the following two methods when b/134103877
    //  is fixed
...
interface FloatDynamicTargetAnimation : DynamicTargetAnimation<Float> {
    ...
    fun fling(startVelocity: Float, adjustTarget: ((Float) -> Float) = {it},
...
//    operator fun timesassign(newval: float) = tovalue(target * newval)
    ...
    override fun fling(
        ...
        if (newTarget == target) {
            ...
            target = newTarget
            val animWrapper = physics<Float> {
                dampingRatio = 2.0f
                stiffness = 100f
            }.createWrapper(value, startVelocity, newTarget, ::lerp)
            startAnimation(animWrapper)
        }
    }
