...
sealed class AnimatedValueImpl<T>(initVal : T,
    ...
    fun toValue(targetValue : T,
                onFinished: ((Boolean) -> Unit)? = null,
                         anim: () -> AnimationBuilder<T> = defaultAnimation) {
 ...
 */
@RestrictTo(RestrictTo.Scope.LIBRARY)
class AnimatedFloatImpl(initVal: Float) : AnimatedValueImpl<Float>(initVal, ::lerp) {
...
//    operator fun timesassign(newval: float) = tovalue(target * newval)
    ...
    fun decay(startVelocity: Float, onFinished: ((Boolean) -> Unit)? = null) {
        if (isRunning) {
