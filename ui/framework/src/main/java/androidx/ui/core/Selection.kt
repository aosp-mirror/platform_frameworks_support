        ...
        Layout(
            ...
            layoutBlock = { measurables, constraints ->
                ...
                val start =
                    measurables[startHandle as () -> Unit].first().measure(
                        Constraints.tightConstraints(
                            HANDLE_WIDTH.value.toInt().ipx,
                            HANDLE_HEIGHT.value.toInt().ipx
                        )
                    )
                val end =
                ...
                layout(width, height) {
                    ...
                    selection?.let {
                        val startLayoutCoordinates = it.startLayoutCoordinates
                        val endLayoutCoordinates = it.endLayoutCoordinates

                        startLayoutCoordinates?.let {
                            endLayoutCoordinates?.let {
                                ...
                                val endOffset = manager.containerLayoutCoordinates!!.childToLocal(
                                    ...
                                    PxPosition(
                                        selection.endOffset.dx.px,
                                        selection.endOffset.dy.px
                                    )
                                )
