...
internal class TextSelectionProcessor(
    ...
    private val length = textPainter.text?.let { it.toPlainText().length } ?: 0
    ...
    private fun processTextSelection() {
        ...
        if (textSelectionStart.offset == textSelectionEnd.offset) {
            ...
            // Currently selection is inclusive-inclusive in Crane, but inclusive-exclusive in
            // Android. Thus before calling drawing selection background, make the selection matches
            // Android behaviour. This should be a temporary workaround.
            textSelectionEnd =
