        ...
        +onCommit(renderParagraph) {
            val id = registrar.subscribe(object : TextSelectionHandler {
                override fun getSelection(coordinates: Pair<PxPosition, PxPosition>):
                    ...
                    Log.e("SelectionDrag","(s,e) = (" + start.dx.toString()+", "+start.dy.toString()+"), ("+end.dx.toString()+", "+ end.dy.toString()+")")
                    Log.e("SelectionDrag","TextSelection = (" + selectionStart.offset.toString()+", " + selectionEnd.offset.toString()+") Before WordBoundary" )
                    ...
                    Log.e("SelectionDrag","TextSelection = (" + selectionStart.offset.toString()+", " + selectionEnd.offset.toString()+") Final Result" )

                    selectionEnd = TextPosition(selectionEnd.offset - 1, TextAffinity.upstream)

                    Log.e("SelectionDrag","TextSelection = (" + selectionStart.offset.toString()+", " + selectionEnd.offset.toString()+") Final Result + 1" )
                    ...
                    return Selection(
                        startOffset = renderParagraph.getBoundingBoxForTextPosition(selectionStart),
                        endOffset = renderParagraph.getBoundingBoxForTextPosition(selectionEnd)
                    )
                }
            })
            onDispose {
