    ...
    val pointerInputHandler =
        { changes: List<PointerInputChange>, pass: PointerEventPass ->
            ...
            if (pass == PointerEventPass.PostDown &&
                started &&
                internalChanges
                    .any { it.anyPositionChangeConsumed() || it.changedToUpIgnoreConsumed() }
            ) {
                started = false
                onCancel?.invoke()
            }
