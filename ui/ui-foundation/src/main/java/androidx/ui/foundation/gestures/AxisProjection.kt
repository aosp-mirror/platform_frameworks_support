...
sealed class AxisProjection {
    ...
    abstract val xProjection: (Px) -> Float
    abstract val yProjection: (Px) -> Float
    ...
    /**
     * Projection that projects coordinates with certain angle,
     * starting from horizontal line (positive X) clockwise.
     *
     * @param angle angle is radians clockwise from positive X axis
     */
    data class AngleProjection(val angle: Float) : AxisProjection() {
        val x1 = 1
        val y1 = x1 * Math.tan(angle.toDouble()).toFloat()
        override fun project(pos: PxPosition): Float {
            val x2 = pos.x.value
            val y2 = pos.y.value
            val modulo1 = Math.sqrt(x1 * x1 + y1 * y1.toDouble())
            val modulo2 = Math.sqrt(x2 * x2 + y2 * y2.toDouble())
            val cosBetween = (x1 * x2 + y1 * y2) / (modulo1 * modulo2)
            val res = (modulo2 * cosBetween).toFloat()
            return if (res.isNaN()) 0f else res
        }

        // TODO: implement all of the below later
        override val xProjection: (Px) -> Float = { it.value }
