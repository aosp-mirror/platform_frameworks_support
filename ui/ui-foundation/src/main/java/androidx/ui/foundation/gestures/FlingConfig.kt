...
import androidx.animation.SnapBuilder
 ...
 * [Decay] to control how much friction is applied to the fling
 *
 * If you don't need fling, consider using [NoFlingConfig].
 *
...
/**
 * Fling config that prevents all flings no matter the velocity.
 *
 * @param onDragEnded callback to be invoked when drag has ended,
 * hence there's no fling animation performed
 */
data class NoFlingConfig(
    val onDragEnded: (finishValue: Float, cancelled: Boolean) -> Unit = { _, _ -> }
) : FlingConfig {
    override fun fling(value: AnimatedFloat, startVelocity: Float) {
        value.fling(0f,
            adjustTarget = { TargetAnimation(value.value, SnapBuilder()) },
            onFinished = { onDragEnded(value.value, it) })
    }
}
...
data class AnchorsFlingConfig(
    ...
    val onAnimationFinished: (finishValue: Float, cancelled: Boolean) -> Unit,
    ...
    override fun fling(
        ...
        value.fling(startVelocity, decayAnimation, adjust, { onAnimationFinished(value.value, it) })
    }
}
...
data class DecayFlingConfig(
    ...
    override fun fling(value: AnimatedFloat, startVelocity: Float) {
        value.fling(
            startVelocity,
            decayAnimation,
            onFinished = { onAnimationFinished(value.value, it) })
    }
}
