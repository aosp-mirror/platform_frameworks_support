...
class ShapeModelTest {
    ...
    @Test
    fun sharpCornerWithRoundedFamily() {
        val sharp = RoundedShapeModel(Corner(0.dp))

        val outline = sharp.toOutline() as Outline.Sharp
        assertThat(outline.rect).isEqualTo(size.toRect())
    }

    @Test
    fun sharpCornerWithCutFamily() {
        val sharp = CutShapeModel(Corner(0.dp))
