...
import androidx.ui.baseui.shape.corner.ZeroCornerSize

/**
 * Components can apply one of this styles to the shapes of their cornerSizes.
 *
 * The rectangular shape can be defined as a shape with [ZeroCornerSize] corners of any family.
 */
enum class ShapeFamily {
    /**
     * Shape with the rounded cornerSizes. [CornerSizes] size means a corner radius.
     */
    Rounded,
    /**
     * Shape with the cut cornerSizes. [CornerSizes] size means a cut length.
     */
    Cut
}
...
) = MaterialShape(family, CornerSizes(cornerSize))
...
fun MaterialShape.toShape(): Shape {
    return when (family) {
        ShapeFamily.Rounded -> RoundedCornerShape(cornerSizes)
        ShapeFamily.Cut -> CutCornerShape(cornerSizes)
    }
}
