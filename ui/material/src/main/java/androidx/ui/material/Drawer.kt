    ...
    Container(expanded = true) {
        WithConstraints { constraints ->
            ...
            val dragSpec = +memo(constraints.maxHeight) {
                ...
                BottomDragInfo(constraints.maxHeight.value.toFloat(), isLandscape)
            }
            DrawerImpl(
                drawerState, onStateChange, dragSpec,
                drawerContent = { dpOffset ->
...
private data class DragInfo(
    ...
    val isDraggableInDirection: (direction: Direction, fraction: Float) -> Boolean
)
...
private fun ModalDragInfo(width: Float) = DragInfo(
    ...
    isDraggableInDirection = { direction, fraction ->
        when (direction) {
            Direction.RIGHT -> fraction <= 1f
            Direction.LEFT -> fraction >= 0
            else -> false
        }
    }
)

private fun BottomDragInfo(height: Float, isLandscape: Boolean) = DragInfo(
    minValue = height,
    maxValue = 0f,
    openedValue = if (isLandscape) 0f else height / 2,
    axisChoice = { it.y },
    axisToPosition = { PxPosition(0.px, it.px) },
    isDraggableInDirection = { direction, fraction ->
        when (direction) {
            Direction.UP -> fraction <= 1f
            Direction.DOWN -> fraction >= 0f
            else -> false
        }
    }
...
private fun DrawerDraggable(
    ...
    DragGestureDetector(
        ...
        dragObserver = object : DragObserver {
            ...
            fun scaleToAbsolute(fraction: Float) =
                fraction * (dragInfo.maxValue - dragInfo.minValue)

            override fun onDrag(dragDistance: PxPosition): PxPosition {
                val draggedFraction = scaleToFraction(dragInfo.axisChoice(dragDistance).value)
                val newValue = offset.value + draggedFraction
                val overflowed = when {
                    newValue > maxFraction -> newValue - maxFraction
                    newValue < minFraction -> minFraction - newValue
                    else -> 0f
                }
                val consumed = draggedFraction - overflowed
                val actualValue = offset.value + consumed
                offset.snapTo(actualValue)
