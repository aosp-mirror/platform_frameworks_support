 ...
 *
 * @param drawerContent composable that represents content inside drawer
 */
@Composable
fun StaticDrawer(
 ...
 * @param onStateChange lambda to be invoked when drawer requests to change its state,
 * e.g. when drawer is being swiped to the new state or when scrim is clicked
 * @param drawerContent composable that represents content inside drawer
 * @param children the rest of UI underneath the drawer
 */
@Composable
fun ModalDrawer(
...
/**
 * Header of the drawer content to put inside of the drawer
 *
 * @param title title text composable of the drawer content
 * @param subtitle subtitle text composable of the drawer content
 */
@Composable
fun DrawerHeader(
...
/**
 * Composable that represents space between different sections in drawer content
 */
@Composable
...
/**
 * Composable that represents section label for section in drawer content
 *
 * @param text text composable to put as a label
 */
@Composable
...
/**
 * Composable that represents destination in drawer content.
 *
 * Destinations in a drawer take the form of actionable list items.
 * Each item describes its destination using a text label and optional iconography.
 *
 * @param selected whether or not this item is selected as an active destination
 * @param onSelect lambda to be invoked when item is clicked, therefore the selection is requested
 * @param icon composable to put as an icon before label
 * @param text text composable to put as a destination label
 */
...
fun NavigationDrawerItem(
    selected: Boolean,
    onSelect: () -> Unit,
    icon: @Composable() () -> Unit = EmptySlot,
    ...
    val textStyle = +themeTextStyle {
        if (selected) body1.copy(color = +themeColor { primary }) else body1
    }
    Container(
...
// TODO: is this is a temporary solution? I need to understand when there's nothing in the slot
val EmptySlot: @Composable() () -> Unit = @Composable {}
