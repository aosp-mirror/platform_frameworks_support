...
class TabRowScope internal constructor() {
    ...
    fun TabRow(
        ...
        Layout(layoutBlock = { measurables, constraints ->
            ...
            // the tab
            val totalHeight = row.height + 1.dp.toIntPx()
            layout(width = constraints.maxWidth, height = totalHeight) {
                row.place(IntPx(0), IntPx(0))
                tabIndicator.place(
            ...
            Surface(shape = RoundedRectangleBorder(), color = Color.Transparent) {
                FlexRow {
    ...
    class TabScope internal constructor() {
        ...
        fun Tab(text: String, selected: Boolean, onSelected: () -> Unit) {
            ...
            val textStyle =
                buttonTextStyle.copy(color = if (selected) colors.primary else Color.Gray)
        ...
        fun Tab(image: Image, selected: Boolean, onSelected: () -> Unit) {
            val colors = +ambient(Colors)
            val tint = if (selected) colors.primary else Color.Gray
            Tab(onSelected = { onSelected() }) {
                Container(height = SmallTabHeight) {
                    TabHorizontalPadding {
                        TabImage(image, tint)
                ...
                Container(height = LargeTabHeight) {
                    TabHorizontalPadding {
                        FlexColumn {
                            ...
                            expanded(1f) {
                                Padding(EdgeInsets(bottom = 16.dp)) {
        ...
        @Composable
        private fun TabHorizontalPadding(children: @Composable() () -> Unit) {
