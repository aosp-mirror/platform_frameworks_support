    ...
    CraneWrapper {
        MaterialTheme {
            Wrap(Alignment.Center) {
                Button(shape = CustomTriangleShape, color = Color.Aqua, onClick = {}) {
...
private object CustomTriangleShape : Shape {
    ...
    override fun DensityReceiver.createOutline(parentSize: PxSize): Outline =
        Outline.Convex(Path().apply {
            moveTo(parentSize.width.value / 2f, 0f)
            lineTo(parentSize.width.value, parentSize.height.value)
            lineTo(0f, parentSize.height.value)
            close()
        })
}
