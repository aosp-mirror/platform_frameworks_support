...
private fun TabDemo(image: Image) {
    FlexColumn {
        expanded(flex = 1f) {
            ...
            TabLayout(items = titles, selectedIndex = state.value, tab = { index, text ->
                Tab(text = text, selected = state.value == index) { state.value = index }
            }) { selected ->
                TabContent("Text tab", selected)
            }
        }
        expanded(flex = 1f) {
            val state = +state { 0 }
            val icons = listOf(image, image, image)
            TabLayout(items = icons, selectedIndex = state.value, tab = { index, icon ->
                Tab(icon = icon, selected = state.value == index) { state.value = index }
            }) { selected ->
                TabContent("Icon tab", selected)
            }
        }
        expanded(flex = 1f) {
            val state = +state { 0 }
            val titlesAndIcons = listOf("TAB 1" to image, "TAB 2" to image, "TAB 3" to image)
            TabLayout(items = titlesAndIcons, selectedIndex = state.value, tab = { index, data ->
                val (title, icon) = data
                Tab(text = title, icon = icon, selected = state.value == index) {
                    state.value = index
                }
            }) { selected ->
                TabContent("Text and icon tab", selected)
            }
        }
        expanded(flex = 1f) {
            val state = +state { 0 }
            val titles = listOf("TAB 1", "TAB 2", "TAB 3")
            TabLayout(items = titles, selectedIndex = state.value, tab = { index, title ->
                FancyTab(
                    title = title,
                ...
                TabContent("Custom tab", selected)
            }
        }
    }
}
