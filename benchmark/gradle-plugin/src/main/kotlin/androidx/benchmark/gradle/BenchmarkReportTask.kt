...
open class BenchmarkReportTask @Inject constructor(private val adbPath: String) :
    ...
    private fun getReportsForDevices() {
        ...
        val deviceIds = AdbUtil.execSync(adbPath, "devices -l", logger).stdout
            .split("\n")
            .drop(1)
    ...
    private fun getReportsForDevice(benchmarkReportDir: File, dataDir: String, deviceId: String) {
        AdbUtil.execSync(adbPath, "shell ls $dataDir", logger, deviceId)
            .stdout
            .split("\n")
            .map { it.trim() }
            ...
            .forEach {
                ...
                if (AdbUtil.execSync(
                    ...
                    AdbUtil.execSync(adbPath, "pull $src $benchmarkReportDir/$it", logger, deviceId)
                    AdbUtil.execSync(adbPath, "shell rm $src", logger, deviceId)
                }
            }
    }
    ...
    private fun getReportDirForDevice(deviceId: String): String {
        ...
        return AdbUtil.execSync(adbPath, cmd, logger, deviceId).stdout
            .split("\n")
            .first()
            .trim()
            .split(Regex("\\s+"))
