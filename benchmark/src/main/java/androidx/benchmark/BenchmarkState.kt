...
class BenchmarkState internal constructor() {
    @JvmField @PublishedApi /* used by keepRunningInline() */
    ...
    private val mResults = ArrayList<Long>()
    ...
    private fun startNextTestRun(): Boolean {
        ...
        mResults.add((currentTime - startTimeNs - pausedDurationNs) / maxIterations)
        mRepeatCount++
        if (mRepeatCount >= REPEAT_COUNT) {
            if (ENABLE_PROFILING) {
                Debug.stopMethodTracing()
            }
            internalStats = Stats(mResults)
            state = FINISHED
            return false
        }
        pausedDurationNs = 0
    ...
    private fun throwIfPaused() {
        if (paused) {
            throw IllegalStateException("Benchmark step finished with paused state." +
    ...
    private fun summaryLine(): String {
        ...
        val sampleNumber = Math.min(mResults.size, 16)
        for (i in 0 until sampleNumber) {
            sb.append("No ").append(i).append(" result is ").append(mResults[i]).append(", ")
        }
        return sb.toString()
    }
    ...
    private fun ideSummaryLineWrapped(key: String): String {
        ...
        val warningString = WarningState.acquireWarningStringForLogging()
        if (warningString != null) {
            for (s in warningString.split("\n")) {
                if (result == null) {
                    if (s.isEmpty()) {
                        continue
                    }
                    result = StringBuilder(s).append("\n")
                } else {
    ...
    internal fun ideSummaryLine(key: String): String {
    ...
    private fun csvLine(): String {
        val sb = StringBuilder()
        for (i in mResults.indices) {
            sb.append(", ").append(mResults[i])
        }
        return sb.toString()
    }
