...
internal object WarningState {
    ...
    private var sWarningString: String? = null
    ...
    private fun isCpuLocked(): Boolean {
        ...
        for (coreDir in coreDirs) {
            try {
                // skip disabled cores
                if (readFirstLineFromFile("/sys/devices/system/cpu/$coreDir/online") == "0") {
                    break
                }

                val cpuMinFreqStr = readFirstLineFromFile(
                    "/sys/devices/system/cpu/$coreDir/cpufreq/scaling_min_freq"
                ) ?: break
                val cpuMinFreq = Integer.parseInt(cpuMinFreqStr)

                val cpuAllAvailFreqStr = readFirstLineFromFile(
                    "/sys/devices/system/cpu/$coreDir/cpufreq/scaling_available_frequencies"
                ) ?: break
                val cpuAvailMinFreq =
    ...
    fun acquireWarningStringForLogging(): String? {
        val ret = sWarningString
        sWarningString = null
        return ret
    }

    /**
     * Read the first line of a file as a String, null if file doesn't exist.
     *
     * @return null if the file at given path does not exist. String of first line otherwise.
     */
    @Throws(IOException::class)
    private fun readFirstLineFromFile(path: String): String? {
        if (!File(path).exists()) {
            return null
        }
