...
class NestedScrollViewNestedScrollingFlingTest(
    ...
    fun uiFingerFling_flingVelocityIsCorrect() {
        ...
        if (!fingerDirectionUp) {
            mNestedScrollingParent.scrollTo(0, 100)
        }
    ...
    companion object {
        ...
        @Parameterized.Parameters(
            name = "fingerDirectionUp:{0},preScrollConsumption:{1},postScrollConsumption:{2}"
        )
        fun data(): Collection<Array<Any>> {
            val configurations = ArrayList<Array<Any>>()

            for (fingerDirectionUp in booleanArrayOf(true, false)) {
                configurations.addAll(
                    Arrays.asList(
                        *arrayOf(
                            arrayOf(fingerDirectionUp, 0, 0),
                            arrayOf(fingerDirectionUp, 25, 0),
                            arrayOf(fingerDirectionUp, 50, 0),
                            arrayOf(fingerDirectionUp, 100, 0),
                            arrayOf(fingerDirectionUp, 0, 25),
                            arrayOf(fingerDirectionUp, 0, 50),
                            arrayOf(fingerDirectionUp, 0, 100),
                            arrayOf(fingerDirectionUp, 12, 13),
                            arrayOf(fingerDirectionUp, 25, 25),
                            arrayOf(fingerDirectionUp, 50, 50)
                        )
                    )
                )
            }
