        ...
        with(classBuilder) {
            ...
            +innerClass(recordClassDescriptor) { it ->
                ...
                +method(assignMethod) { irFunction ->
                    val thisParameterReceiver = irFunction.dispatchReceiverParameter ?: error("No receiver found for assign method")
                    val valueParameter = irFunction.valueParameters[0]
                    for (i in 0 until attributes.count()) {
                        val field = fields[i]
                        val thisParameter = syntheticGetValue(thisParameterReceiver.symbol, thisParameterReceiver.type)
                        val other = toRecord(syntheticGetValue(valueParameter.symbol, valueParameter.type))
            ...
            val thisReceiver = declaration.thisReceiver ?: error("No this receiver found for class ${declaration.name}")
            ...
            +method(metadata.firstFrameDescriptor(recordTypeDescriptor)) {
                val receiverParameter = it.dispatchReceiverParameter ?: error("Expected receiver for $metadata")
                val dispatchReceiver = syntheticGetValue(receiverParameter.symbol, receiverParameter.type)
            ...
            +method(metadata.prependFrameRecordDescriptor(recordTypeDescriptor)) {
                val receiverParameter = it.dispatchReceiverParameter ?: error("Expected reciver for $metadata")
                val dispatchReceiver = syntheticGetValue(receiverParameter.symbol, receiverParameter.type)
            ...
            fun recordGetter(thisSymbol: IrValueSymbol, thisType: IrType) = syntheticGetField(fieldReference, syntheticGetValue(thisSymbol, thisType))

            fun getRecord(thisSymbol: IrValueSymbol, thisType: IrType) = toRecord(recordGetter(thisSymbol, thisType))
            ...
            +initializer {
                ...
                metadata.getFramedProperties(bindingContext).forEach { propertyDescriptor ->
                    ...
                    backingField.initializer?.let { initializer ->
                        ...
                        +syntheticSetField(irRecordField, getRecord(thisSymbol, thisType), initializer.expression)
                    }
                }
            ...
            metadata.getFramedProperties(
                ...
                irFramedProperty.getter?.let { getter ->
                    ...
                    val getterThisReceiver = getter.dispatchReceiverParameter ?: error("No dispatch receiver found for getter")
                            ...
                            val newExpression = if (expression.descriptor ==
                                ...
                                syntheticGetField(irRecordField,
                                                  toRecord(syntheticCall(
                                                      ...
                                                      it.putValueArgument(0, recordGetter(getterThisSymbol, getterThisType))
                                                      it.putValueArgument(1, syntheticGetValue(getterThisSymbol, getterThisType))
                                                  }
                                                  )
                                )
                            } else expression
                ...
                irFramedProperty.setter?.let { setter ->
                    val setterThisReceiver = setter.dispatchReceiverParameter ?: error("No dispatch receiver found for setter")
                            ...
                            val newExpression = if (expression.descriptor ==
                                ...
                                syntheticSetField(
                                    ...
                                    toRecord(syntheticCall(
                                        ...
                                        it.putValueArgument(1, syntheticGetValue(setterThisSymbol, setterThisType))
                                    }),
                                    expression.value
                                )
                            } else expression
...
class IrClassBuilder(
    ...
    fun syntheticImplicitCast(
        ...
        IrTypeOperatorCallImpl(
            ...
            fromType.toIrType(), classifier, argument
        )
}
