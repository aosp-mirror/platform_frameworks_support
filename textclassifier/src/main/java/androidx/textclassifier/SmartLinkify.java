...
public final class SmartLinkify {
    ...
    private static final class LinkifyTask extends AsyncTask<Void, Void, TextLinks>
        ...
        private Collection<TextLinks.TextLink> generatePatternLinks() {
            if (mParams.getPatterns().isEmpty()) {
                return Collections.emptyList();
            }
            final Collection<TextLinks.TextLink> links = new ArrayList<>();
            for (Pattern pattern : mParams.getPatterns().keySet()) {
                final Matcher matcher = pattern.matcher(mTruncatedText);
                while (matcher.find()) {
                    final String entityType = mParams.getPatterns().get(pattern);
                    final Map<String, Float> score = new ArrayMap<>();
                    score.put(entityType, 1f);
                    links.add(new TextLinks.TextLink(matcher.start(), matcher.end(), score, null));
                }
            }
            return links;
        }
