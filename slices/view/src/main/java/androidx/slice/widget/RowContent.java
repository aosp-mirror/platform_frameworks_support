...
public class RowContent extends SliceContent {
    ...
    @Override
    public int getHeight(SliceStyle style,
            @SliceView.SliceMode SliceViewPolicy policy) {
        int maxSmallHeight = policy.getMaxSmallHeight();
        return policy.getMode() == MODE_SMALL
                ? getSmallHeight(maxSmallHeight)
                : getActualHeight(maxSmallHeight);
    }

    /**
     * @return the height to display a row at when it is used as a small template.
     */
    private int getSmallHeight(int maxSmallHeight) {
     ...
     */
    private int getActualHeight(int maxSmallHeight) {
        if (!isValid()) {
            return 0;
        }
        int maxHeight = maxSmallHeight > 0 ? maxSmallHeight : mMaxHeight;
