...
public class SliceQuery {
    ...
    private static Queue<SliceItem> toQueue(Slice item) {
        Queue<SliceItem> q = new LinkedList<>();
        Collections.addAll(q, item.getItemArray());
        return q;
    }

    private static Queue<SliceItem> toQueue(SliceItem item) {
        Queue<SliceItem> q = new LinkedList<>();
        q.add(item);
        return q;
    }

    private static SliceItem findSliceItem(final Queue<SliceItem> items, Filter<SliceItem> f) {
        while (!items.isEmpty()) {
    ...
    private static void findAll(final Queue<SliceItem> items, Filter<SliceItem> f,
            List<SliceItem> out) {
        while (!items.isEmpty()) {
            SliceItem item = items.poll();
            if (f.filter(item)) {
