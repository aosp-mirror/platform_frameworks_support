...
class ExperimentalDetector : Detector(), SourceCodeScanner {
    ...
    private fun checkExperimentalUsage(
        ...
        if (!hasOrUsesAnnotation(context, usage, useAnnotation)) {
            val level = getSimpleNameFromExpression(annotation.attributeValues[0])!!
            ...
            if (element is UAnnotated) {
                ...
                for (annotation in annotations) {
                    if (annotation.qualifiedName == annotationName) {
                        ...
                        if (getQualifiedNameFromExpression(context, markerClass) == annotationName) {
                            return true
                        }
                    }
                }
    ...
    private fun getSimpleNameFromExpression(
        expression: UNamedExpression?
    ): String? {
        if (expression != null) {
            val simpleName = expression.expression
            if (simpleName is USimpleNameReferenceExpression) {
    ...
    private fun getQualifiedNameFromExpression(
        context: JavaContext,
        expression: UNamedExpression?
    ): String? {
        if (expression != null) {
    ...
    companion object {
        ...
        private const val BASE_ISSUE_ID = "UnsafeExperimentalUsage"

        val ISSUE_ERROR = Issue.create(
            id = "${BASE_ISSUE_ID}Error",
            briefDescription = "Unsafe experimental usage intended to be error-level severity",
            explanation = """
                TODO
            """,
        ...
        val ISSUE_WARNING = Issue.create(
            id = "${BASE_ISSUE_ID}Warning",
            briefDescription = "Unsafe experimental usage intended to be warning-level severity",
            explanation = """
                TODO
            """,
            category = Category.CORRECTNESS,
