...
fun MotionEventData.toMotionEvent(startTime: Long): MotionEvent = MotionEvent.obtain(
        startTime,
        this.eventTimeDelta + startTime,
        this.action,
        this.x,
        this.y,
        this.metaState)
...
fun FlingData.generateFlingMotionEvents(originX: Int, originY: Int, fingerDirection: Direction):
    ...
    motionEventData.add(MotionEventData(0, 0, MotionEvent.ACTION_DOWN, fromX, fromY, 0))
    for (i in 0..(numberOfInnerEvents - 1)) {
        val timeDelta = (i + 1) * MOTION_EVENT_INTERVAL_MILLIS
        motionEventData.add(MotionEventData(0, timeDelta, MotionEvent.ACTION_MOVE, toX, toY, 0))
    }
    motionEventData.add(MotionEventData(0, time, MotionEvent.ACTION_MOVE, toX, toY, 0))
    motionEventData.add(MotionEventData(0, time, MotionEvent.ACTION_UP, toX, toY, 0))
 ...
 */
fun View.dispatchMotionEventsToView(startTime: Long, motionEventData: List<MotionEventData>) {
    for (motionEventDataItem in motionEventData) {
        this.dispatchTouchEvent(motionEventDataItem.toMotionEvent(startTime))
    }
}
...
fun View.simulateFling(
    ...
    this.dispatchMotionEventsToView(startTime,
