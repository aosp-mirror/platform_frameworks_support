...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent3,
    ...
    public void onNestedScroll(@NonNull View target, int dxConsumed, int dyConsumed,
        ...
        int consumedByParents = consumedBeforeParents - consumed[1];

        // This is a bit of a hack. Nested scrolling works from the bottom up, and as we are
        // sometimes between two nested scrolling views, we need a way to be able to know when all
        // nested scrolling parents have stopped consuming scroll distances.
        //
        // We first try to do that by directly checking if any parents have done so via tracking
        // changes in the 'consumed' parameter.
        //
        // When it appears that the parents have consumed all of the remaining scroll distance, it
        // may actually be the case that a parent simply doesn't implement nested scrolling version
        // 3 (that is the fallback behavior). If that is the case we use the 'offset in window'
        // functionality to see if we have been moved from the event.
        int dy = dyUnconsumed + consumedByParents;
        if (dy == 0) {
            dy = dyUnconsumed + mParentOffsetInWindow[1];
        }
