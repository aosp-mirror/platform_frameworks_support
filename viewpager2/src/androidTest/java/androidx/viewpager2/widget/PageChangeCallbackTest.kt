...
class PageChangeCallbackTest(private val config: TestConfig) : BaseTest() {
    ...
    private val adapterProvider: AdapterProviderForItems get() {
        ...
        val marginFloat = config.pageMargin * resources.displayMetrics.density
        val marginPx = if (config.pageMargin > 0) max(1, marginFloat.roundToInt()) else 0
        return if (marginPx > 0) {
            { items -> { MarginViewAdapter(marginPx, items) } }
        } else {
    ...
    class MarginViewAdapter(private val margin: Int, items: List<String>) : ViewAdapter(items) {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            ...
            // Similarly, make sure the margins on opposite sides do not differ by a factor of 2
            lp.setMargins(margin, margin*2, margin*4, margin*3)
