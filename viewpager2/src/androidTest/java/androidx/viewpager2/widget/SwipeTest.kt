...
private fun createTestSet(): List<TestConfig> {
    // cartesian product
    infix fun <A, B> List<A>.`×`(other: List<B>): List<Pair<A, B>> {
        return flatMap { a -> other.map { b -> a to b } }
    }

    val activities = listOf(ViewAdapterActivity::class.java,
            FragmentAdapterActivity::class.java)
    val orientations = listOf(VERTICAL, HORIZONTAL)

    return (activities `×` orientations)
...
private fun createTestSet(activity: Class<out BaseActivity>, orientation: Int): List<TestConfig> {
    return listOf(
            ...
            .filterNot { it.stepToNewValue.isNotEmpty() && !activity.supportsMutations }
}

val Class<out BaseActivity>.supportsMutations: Boolean
    get() {
        // Mutation testing only applies to Fragment-based adapters
        return this::class.java == FragmentStatePagerAdapter::class.java
    }
