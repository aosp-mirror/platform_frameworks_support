 ...
 * onPageScrollStateChanged(0)
 *
 * 0 -> 1 (smooth) -> 2 (immediate)
 * >> setCurrentItem(1, true);
 * onPageScrollStateChanged(2)
 * onPageSelected(1)
 * onPageScrolled(0, 0.141667, 153)
 * onPageScrolled(0, 0.457407, 494)
 * onPageScrolled(0, 0.672222, 726)
 ...
 *
 * 0 -> 1 (smooth) -> 1 (immediate)
 * Trace left out because it is incorrect due to a bug in ViewPager
 */
...
private fun pickIndices(r: Random, probability: Float, numScrolls: Int): Set<Int> {
    return (0 until numScrolls).filter { r.nextFloat() < probability }.toSet()
}
