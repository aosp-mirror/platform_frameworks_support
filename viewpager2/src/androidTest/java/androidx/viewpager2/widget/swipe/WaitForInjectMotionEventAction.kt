...
class WaitForInjectMotionEventAction : ViewAction {
    companion object {
        fun waitForInjectMotionEvents(): ViewAction {
            return WaitForInjectMotionEventAction()
        }
    }
    ...
    override fun perform(uiController: UiController?, view: View?) {
        if (uiController == null || view == null) {
            throw PerformException.Builder()
                .withActionDescription("waiting for inject capabilities")
        ...
        var injectionPossible = false
        var attempt = 0

        while (!injectionPossible) {
