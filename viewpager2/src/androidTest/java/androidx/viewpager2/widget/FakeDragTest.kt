...
import androidx.viewpager2.widget.ViewPager2.SCROLL_STATE_FAKE_DRAGGING as FAKE_DRAG
...
class FakeDragTest(private val config: TestConfig) : BaseTest() {
    ...
    fun test_startFakeDragDuringManualDrag() {
        ...
        // start fake drag
        doIllegalAction("Cannot start a fake drag when user is dragging") {
            test.viewPager.beginFakeDrag()
        }
    }
    ...
    fun test_startManualDragDuringFakeDrag() {
        ...
        repeat(2) {
            ...
            recorder.apply {
                ...
                assertFirstEvents(FAKE_DRAG)
                assertLastEvents(expectedFinalPage)
                assertPageSelectedEvents(initialPage, expectedFinalPage)
                assertStateChanges(
                    DRAGGING,
                    listOf(FAKE_DRAG, DRAGGING, SETTLING, IDLE),
                    listOf(FAKE_DRAG, DRAGGING, IDLE)
                )
            }
    ...
    private fun basicFakeDragTest(
        ...
        repeat(2) {
            ...
            fakeDrag(relativeDragDistance, duration, frameLengthMs, interpolator)
            ...
            recorder.apply {
                ...
                assertStateChanges(
                    FAKE_DRAG,
                    listOf(FAKE_DRAG, SETTLING, IDLE),
                    listOf(FAKE_DRAG, IDLE)
                )
            }
    ...
    private fun startFakeDragWhileSettling(
        ...
        // start fake drag
        val idleLatch = test.viewPager.addWaitForIdleLatch()
        fakeDrag(dragDistance(), 200, frameLengthMs)
        assertThat(idleLatch.await(2, SECONDS), equalTo(true))
        ...
        recorder.apply {
            ...
            if (fakeDragMustEndSnapped) {
                assertThat("When a fake drag should end in a snapped position, we expect the last" +
                    ...
                    expectSettlingAfterState(FAKE_DRAG), equalTo(false))
            }
            assertStateChanges(
                FAKE_DRAG,
                listOf(SETTLING, FAKE_DRAG, SETTLING, IDLE),
                listOf(SETTLING, FAKE_DRAG, IDLE)
            )
        }
    ...
    private fun setCurrentItemDuringFakeDrag(smoothScroll: Boolean) {
        val initialPage = test.viewPager.currentItem
        // start fake drag
        val latch = test.viewPager.addWaitForStateLatch(FAKE_DRAG)
        fakeDrag(.5f, 500, frameLengthMs)
        assertThat(latch.await(1, SECONDS), equalTo(true))
    ...
    private fun fakeDrag(
        ...
        val rtlModifier = if (config.orientation == ORIENTATION_HORIZONTAL && config.rtl) -1 else 1
        val steps = max(1, (duration / periodMs.toFloat()).roundToInt())
        val distancePx = test.viewPager.pageSize * -relativeDragDistance * rtlModifier
        ...
        val delayMs = { eventTime - uptimeMillis() }
        test.viewPager.post { test.viewPager.beginFakeDrag() }
        for (delta in deltas) {
            eventTime += periodMs
            test.viewPager.postDelayed({ test.viewPager.fakeDragBy(delta) }, delayMs())
        }
        eventTime++
        test.viewPager.postDelayed({ test.viewPager.endFakeDrag() }, delayMs())
    }
    ...
    private fun RecordingCallback.assertStateChanges(
        ...
        assertThat(
            "Unexpected sequence of state changes (0=IDLE, 1=DRAGGING, 2=SETTLING, " +
                    "3=FAKE_DRAGGING)" + dumpEvents(),
            stateEvents.map { it.state },
            equalTo(
                if (expectSettlingAfterState(lastStateBeforeSettling)) {
                    statesWithSettling
                } else {
