...
open class BaseTest {
    ...
    fun ViewPager2.addWaitForScrolledLatch(): CountDownLatch {
        val latch = CountDownLatch(1)
        registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
...
fun tryNTimes(n: Int, reset: () -> Unit, block: () -> Unit) {
    var lastException: RetryException? = null
    repeat(n) {
        try {
            block()
            return
    ...
    throw AssertionError("Block hit bad state $n times", lastException!!)
}
