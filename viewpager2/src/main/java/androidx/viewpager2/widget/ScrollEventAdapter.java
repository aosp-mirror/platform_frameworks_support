    ...
    public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
        ...
        if (isAdapterInAnyDragginState() && newState == RecyclerView.SCROLL_STATE_IDLE) {
            ...
            if (!mScrollHappened) {
                ...
                // Note that if we went to idle in that last onScrolled event, resetState() has also
                // been called, and this if-statement will not be triggered because mAdapterState
                // is no longer in STATE_IN_PROGRESS_MANUAL_DRAG.
                dispatchIdle = true;
            ...
            if (dispatchIdle) {
                // Normally idle is fired in last onScrolled call, but scroll did not happen
