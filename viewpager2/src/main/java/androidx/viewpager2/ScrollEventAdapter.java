...
public class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    ...
    /**
     * Calculates the position of the currently visible item of the ViewPager in the continuous
     * domain (includes the fraction of the item that is off screen). Non-integral values mean that
     * the ViewPager is not in a snapped position. In that case, the user may be dragging the
     * ViewPager, or the ViewPager is settling towards its {@link ViewPager2#getCurrentItem()
     * current item}.
     *
     * @return The position of the currently visible item in the continuous domain
     */
