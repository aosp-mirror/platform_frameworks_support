...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.create_new_tc).setOnClickListener(view -> {
            mTextClassifier = createTextClassifier();
            updateStatusText();
        });
        findViewById(R.id.destroy).setOnClickListener(v -> destroyTextClassifier());
    ...
    private void setupSpinner() {
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int pos, long l) {
                if (pos == DEFAULT) {
                    mTextClassificationManager.setTextClassifierFactory(null);
                } else {
                    mTextClassificationManager.setTextClassifierFactory(
                            textClassificationContext ->
                                    new SimpleTextClassifier(
                                            MainActivity.this, textClassificationContext));
                }
            }
    ...
    private TextClassifier createTextClassifier() {
        TextClassifier textClassifier = mTextClassificationManager.createTextClassifier(
                new TextClassificationContext.Builder(
                        getPackageName(), TextClassifier.WIDGET_TYPE_UNKNOWN).build());
        return textClassifier;
    }

    private void updateStatusText() {
        mStatusTextView.setText(
                mTextClassifier.getClass().getName()
                        + " is in use. Destroyed: " + mTextClassifier.isDestroyed());
    }

    private void smartLinkify() {
        TextLinksParams textLinksParams = new TextLinksParams.Builder().build();
        SmartLinkify.addLinksAsync(mInput, textLinksParams);
    }

    private void destroyTextClassifier() {
        mTextClassifier.destroy();
        updateStatusText();
    }
}
