...
class MutableCollectionViewActivity : MutableCollectionBaseActivity() {
    override fun createViewPagerAdapter(): RecyclerView.Adapter<*> {
        ...
        return object : RecyclerView.Adapter<PageView>() {
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = PageView(parent)
            override fun onBindViewHolder(holder: PageView, position: Int) = holder.bind()
...
class PageView(parent: ViewGroup) :
    ...
    fun bind() {
        ...
        fun updateCountView() {
            itemView.textViewCount.text = items.getClickCount(itemId).toString()
        }
        updateCountView()

        itemView.buttonCountIncrease.setOnClickListener {
            items.registerClick(itemId); updateCountView()
        }
    }
}
