...
class Items : ViewModel() {
    ...
    private val items = (1..9).map { longToItem(nextValue.getAndIncrement()) }.toMutableList()
    private val clickCount = mutableMapOf<Long, Int>()

    operator fun get(position: Int): String = items[position]
    operator fun get(itemId: Long): String = items.first { itemToLong(it) == itemId }
    fun getItemId(position: Int): Long = itemToLong(items[position])
    fun contains(itemId: Long): Boolean = items.any { itemToLong(it) == itemId }
    fun addNewAt(position: Int) = items.add(position, longToItem(nextValue.getAndIncrement()))
