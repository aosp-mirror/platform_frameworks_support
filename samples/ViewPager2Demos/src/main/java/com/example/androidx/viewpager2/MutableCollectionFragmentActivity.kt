...
class MutableCollectionFragmentActivity : MutableCollectionBaseActivity() {
    override fun createViewPagerAdapter(): RecyclerView.Adapter<*> {
        ...
        return object : FragmentStateAdapter(supportFragmentManager) {
            override fun getItem(position: Int) = PageFragment.create(items.getItemId(position))
...
class PageFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fun updateCountView() {
            textViewCount.text = items.getClickCount(itemId).toString()
        }
        updateCountView()

        buttonCountIncrease.setOnClickListener { items.registerClick(itemId); updateCountView() }
    }
