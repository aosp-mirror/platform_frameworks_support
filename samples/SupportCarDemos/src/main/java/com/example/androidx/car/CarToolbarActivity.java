...
public class CarToolbarActivity extends AppCompatActivity {
    public static final String TAG = "CarToolbarActivity";
    private boolean mAddTitles = true;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CarMenuItem.OnClickListener onClickListener = (item -> {
            if (item.isCheckable()) {
                Log.d(TAG, "Switch checked? " + (item.isChecked() ? "Yes" : "No"));
            } else {
        ...
        findViewById(R.id.add_action).setOnClickListener(v -> {
            items.add(new CarMenuItem.Builder()
                        .setTitle(mAddTitles ? "Action item" : "")
        ...
        findViewById(R.id.add_switch).setOnClickListener(v -> {
            items.add(new CarMenuItem.Builder()
