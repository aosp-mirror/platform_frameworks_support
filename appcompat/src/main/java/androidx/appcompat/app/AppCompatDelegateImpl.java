...
class AppCompatDelegateImpl extends AppCompatDelegate
    ...
    private static final Map<Class<?>, Integer> sLastNightModes = new ArrayMap<>();
    ...
    public void attachBaseContext(Context context) {
        ...
        if (mLocalNightMode == MODE_NIGHT_UNSPECIFIED) {
            ...
            final Integer value = sLastNightModes.get(mContext.getClass());
            if (value != null) {
                mLocalNightMode = value;
                // Finally remove the value
                sLastNightModes.remove(mContext.getClass());
            }
        }
    ...
    public void onSaveInstanceState(Bundle outState) {
        if (mLocalNightMode != MODE_NIGHT_UNSPECIFIED) {
            ...
            sLastNightModes.put(mContext.getClass(), mLocalNightMode);
        }
    }
    ...
    private boolean updateForNightMode(@ApplyableNightMode final int mode) {
        ...
        if (!mAttached && Build.VERSION.SDK_INT >= 17
                && mContext instanceof android.view.ContextThemeWrapper) {
            final Configuration conf = new Configuration();
            conf.setToDefaults();

            final Configuration deltaConf = new Configuration();
            deltaConf.uiMode = newNightMode | (conf.uiMode & ~Configuration.UI_MODE_NIGHT_MASK);

            conf.updateFrom(deltaConf);
        ...
        if (!handled) {
            ...
            if (!isActivityManifestHandlingUiMode() && currentNightMode != newNightMode) {
                // The component isn't handling the uiMode configChange, and we have a new night
                // mode
                if (mCreated && mContext instanceof Activity) {
                    // If we're created and have an activity, we can just recreate() to apply
                    if (DEBUG) {
    ...
    private void updateResourcesConfigurationForNightMode(int newNightMode) {
