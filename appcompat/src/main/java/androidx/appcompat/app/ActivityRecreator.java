 ...
 * get the exact same behavior as < 26. However, that behavior has problems, namely b/30060825. When
 * an Activity in the background is recreated, it goes through: destroy -> create -> start ->
 * resume
 * -> pause and doesn't stop. This is a violation of the contract for onStart/onStop, but that might
 * be palatable if it didn't also have the effect of preventing new configurations from being
 * applied - since the Activity doesn't go through onStop, our tracking of whether our app is
 * visible thinks we're always visible, and thus can't do another recreation later.
 *
...
final class ActivityRecreator {
     ...
     */
    static boolean recreate(@NonNull final AppCompatActivity activity,
            @NonNull final Runnable platformRecreateCall) {
        // On Android O and later we can rely on the platform recreate()
        if (Build.VERSION.SDK_INT >= 28) {
            platformRecreateCall.run();
            return true;
        }
