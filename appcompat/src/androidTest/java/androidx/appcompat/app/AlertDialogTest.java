...
public class AlertDialogTest {
    ...
    @Test
    @SmallTest
    public void testTitleThemeFontIsAppliedToTitle() throws Exception {
        final Context context = mActivityTestRule.getActivity();
        ...
        final String title = context.getString(R.string.alert_dialog_title);
        onView(withId(R.id.test_button)).perform(click());
        onView(withText(title)).inRoot(isDialog()).check(matches(withFont(typeface)));
    }
    ...
    static class FontMatcher extends TypeSafeMatcher<View> {
        ...
        FontMatcher(final Typeface typeface) {
            super();
        ...
        protected boolean matchesSafely(View item) {
            ...
            assertEquals(measureText("a", mTypeface), measureText("a", typeface), 0f);
            assertEquals(measureText("b", mTypeface), measureText("b", typeface), 0f);
            assertEquals(measureText("c", mTypeface), measureText("c", typeface), 0f);
            assertEquals(measureText("a", mTypeface), measureText("bbb", typeface), 0f);
    ...
    static Matcher<View> withFont(final Typeface typeface) {
        return new FontMatcher(typeface);
    }
}
