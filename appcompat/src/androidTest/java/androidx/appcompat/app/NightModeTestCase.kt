...
class NightModeTestCase(private val setMode: NightSetMode) {
    ...
    fun testLocalDayNightModeRecreatesActivity() {
        ...
        setNightModeAndWaitForRecreate(rule, nightMode = MODE_NIGHT_YES, setMode = setMode)
    ...
    fun testSwitchingYesToFollowSystem() {
        ...
        if (!isSystemNightThemeEnabled(rule.activity)) {
            ...
            setNightModeAndWaitForRecreate(rule,
    ...
    fun testColorConvertedDrawableChangesWithNightMode() {
        ...
        for (i in 0 until 5) {
            ...
            setNightModeAndWaitForRecreate(rule, nightMode = MODE_NIGHT_NO, setMode = setMode)
    ...
    fun testNightModeAutoTimeRecreatesOnTimeChange() {
        ...
        val twilightManager = FakeTwilightManager()
    ...
    fun testOnNightModeChangedCalled() {
        ...
        setNightModeAndWait(rule, nightMode = MODE_NIGHT_YES, setMode = setMode)
    ...
    private inner class FakeTwilightManager : TwilightManager(
        rule.activity,
        rule.activity.getSystemService(Context.LOCATION_SERVICE) as LocationManager
    ) {
        var fakeNight = false

        internal override fun isNight() = fakeNight
    }
