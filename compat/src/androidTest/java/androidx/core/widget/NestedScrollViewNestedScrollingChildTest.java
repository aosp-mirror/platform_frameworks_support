...
public class NestedScrollViewNestedScrollingChildTest {
    ...
    @Test
    public void uiFling_parentOnStartNestedScrollCalledCorrectly() {
        int[] targetFlingTimeAndDistance = NestedScrollViewTestUtils
                .getTargetFlingVelocityTimeAndDistance(InstrumentationRegistry.getContext());
        MotionEvent[] motionEvents =
                NestedScrollViewTestUtils.generateMotionEvents(targetFlingTimeAndDistance);
        mNestedScrollView.dispatchTouchEvent(motionEvents[0]);
        mNestedScrollView.dispatchTouchEvent(motionEvents[1]);
        Mockito.clearInvocations(mParentSpy);

        mNestedScrollView.dispatchTouchEvent(motionEvents[2]);
    ...
    @Test
    public void uiFling_parentOnStopNestedScrollCalledCorrectly() {
        when(mParentSpy.onStartNestedScroll(any(View.class), any(View.class), anyInt(), anyInt()))
                .thenReturn(true);
        int[] targetFlingTimeAndDistance = NestedScrollViewTestUtils
                .getTargetFlingVelocityTimeAndDistance(InstrumentationRegistry.getContext());
        ...
        verify(mParentSpy).onStopNestedScroll(mNestedScrollView, ViewCompat.TYPE_TOUCH);
    }
