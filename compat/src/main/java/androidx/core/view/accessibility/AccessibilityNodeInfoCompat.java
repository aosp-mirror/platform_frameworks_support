...
public class AccessibilityNodeInfoCompat {
    ...
    private List<AccessibilityActionCompat> createExtrasActionList() {
        ...
        List<Integer> ids = extrasIntList(ACTION_IDS_KEY);
        List<CharSequence> labels = extrasCharSequenceList(ACTION_LABELS_KEY);
    ...
    private boolean removeExtrasAction(AccessibilityActionCompat action) {
        int index = extrasIntList(ACTION_IDS_KEY).indexOf(action.getId());
        if (index != -1) {
            extrasIntList(ACTION_IDS_KEY).remove(index);
            extrasCharSequenceList(ACTION_LABELS_KEY).remove(index);
            return true;
        } else {
            return false;
        }
    ...
    private void addExtrasAction(AccessibilityActionCompat action) {
        extrasIntList(ACTION_IDS_KEY).add(action.getId());
        extrasCharSequenceList(ACTION_LABELS_KEY).add(action.getLabel());
    }
