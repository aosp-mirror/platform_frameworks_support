...
class AsyncPagedListDiffer<T> {
    ...
    lateinit var updateCallback: ListUpdateCallback
    ...
    val itemCount: Int
        get() {
            ...
            return if (snapshot == null) 0 else snapshot!!.size
        }
     ...
     */
    fun submitList(pagedList: PagedList<T>?) = submitList(pagedList, null)
    ...
    fun submitList(pagedList: PagedList<T>?, commitCallback: Runnable?) {
        if (pagedList != null) {
            if (this.pagedList == null && snapshot == null) {
                isContiguous = pagedList.isContiguous()
            } else {
                if (pagedList.isContiguous() != isContiguous) {
                    throw IllegalArgumentException(
                        "AsyncPagedListDiffer cannot handle both" +
        ...
        val previous = snapshot ?: this.pagedList

        if (pagedList == null) {
            val removedCount = itemCount
            if (this.pagedList != null) {
                this.pagedList!!.removeWeakCallback(pagedListCallback)
                this.pagedList!!.removeWeakLoadStateListener(loadStateListener)
                this.pagedList = null
        ...
        if (this.pagedList == null && snapshot == null) {
            ...
            this.pagedList!!.addWeakLoadStateListener(loadStateListener)
        ...
        if (this.pagedList != null) {
            ...
            snapshot = this.pagedList!!.snapshot() as PagedList<T>
            this.pagedList = null
        }

        if (snapshot == null || this.pagedList != null) {
        ...
        config.backgroundThreadExecutor.execute {
            val result = PagedStorageDiffHelper.computeDiff(
    ...
    internal fun latchPagedList(
        ...
        if (snapshot == null || pagedList != null) {
        ...
        PagedStorageDiffHelper.dispatchDiff(
            updateCallback,
            previousSnapshot!!.getStorage(),
            newList.getStorage(),
            diffResult
        )
        ...
        if (!pagedList!!.isEmpty()) {
            ...
            val newPosition = PagedStorageDiffHelper.transformAnchorIndex(
                diffResult,
                previousSnapshot.getStorage(),
     ...
     */
    fun addPagedListListener(listener: PagedListListener<T>) = listeners.add(listener)
     ...
     */
    fun removePagedListListener(listener: PagedListListener<T>) = listeners.remove(listener)
     ...
     * @see addPagedListListener
     */
    fun removeLoadStateListListener(listener: LoadStateListener) {
