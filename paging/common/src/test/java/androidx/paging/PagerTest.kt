...
class PagerTest {
    ...
    inner class ImmediateListDataSource(private val data: List<String>) : ListenablePositionalDataSource<String>() {
        init {
        ...
        override fun loadRange(params: PositionalDataSource.LoadRangeParams): ListenableFuture<RangeResult<String>> {
            ...
            executor.execute {
                ...
                println("loadRange, $position, end $end")
    ...
    private val emptyRangeResult = RangeResult(0, 0)
    ...
    fun emptyAppend() {
        ...
        // Currently we call through to the PositionalDataSource to return an empty list here.
        // TODO: consider filtering this in ListenablePositionalDataSource with a size check
        testExecutor.executeAll()

        assertEquals(listOf(
            Result(END, emptyRangeResult)
        ), consumer.takeResults())
        assertEquals(listOf(
            StateChange(END, LOADING),
    ...
    fun emptyPrepend() {
        ...
        // ListenablePositionalDataSource triggers an immediate empty response here,
        // so we don't need to flush the testExecutor
        assertEquals(listOf(
            Result(START, emptyRangeResult)
        ), consumer.takeResults())
        assertEquals(listOf(
            StateChange(START, LOADING),
