...
public class Pager<K, V> {
    ...
    void onLoadSuccess(PagedList.LoadType type, DataSource.BaseResult<V> value) {
        System.out.println("onLoadSuccess, nr=" + value.data.size());
        if (isDetached()) {
            // abort!
            return;
        }
    ...
    static class SimpleAdjacentProvider<V> implements AdjacentProvider<V> {
        ...
        @Override
        public void onPageResultResolution(@NonNull PagedList.LoadType type, @NonNull DataSource.BaseResult<V> result) {
            ...
            System.out.println("NOW: first index " + mFirstIndex + ", last index " + mLastIndex);
        }
    }
}
