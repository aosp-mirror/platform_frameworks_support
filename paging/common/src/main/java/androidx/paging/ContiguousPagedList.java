...
class ContiguousPagedList<K, V> extends PagedList<V> implements PagedStorage.Callback {
    ...
    PageResult.Receiver<V> mReceiver = new PageResult.Receiver<V>() {
            ...
            if (resultType == PageResult.INIT) {
                ...
                if (resultType == PageResult.APPEND) {
                    if (skipNewPage && !trimFromFront) {
                        ...
                        mLoadStateManager.setState(LoadType.End, LoadState.IDLE);
                    } else {
                    ...
                    if (skipNewPage && trimFromFront) {
                        ...
                        mLoadStateManager.setState(LoadType.Start, LoadState.IDLE);
                    } else {
        ...
        public void onPageError(@PageResult.ResultType int resultType,
                @NonNull DataSource.Error error) {
            LoadState errorState = LoadState.fromError(error);
            if (resultType == PageResult.PREPEND) {
                mLoadStateManager.setState(LoadType.Start, errorState);
            } else if (resultType == PageResult.APPEND) {
                mLoadStateManager.setState(LoadType.End, errorState);
            } else {
    ...
    public void retry() {
        super.retry();
        DataSource.Error error = mLoadStateManager.getStart().getError();
        if (error != null && error.isRetryable) {
            schedulePrepend();
        }

        error = mLoadStateManager.getEnd().getError();
        if (error != null && error.isRetryable) {
            scheduleAppend();
        }
    }
    ...
    private void schedulePrepend() {
        mLoadStateManager.setState(LoadType.Start, LoadState.LOADING);
    ...
    private void scheduleAppend() {
        mLoadStateManager.setState(LoadType.End, LoadState.LOADING);
    ...
    public void onEmptyPrepend() {
        mLoadStateManager.setState(LoadType.Start, LoadState.DONE);
    }
    ...
    public void onEmptyAppend() {
        mLoadStateManager.setState(LoadType.End, LoadState.DONE);
    }
