...
internal class WrapperPositionalDataSource<A, B>(
    ...
    val listFunction: Function<List<A>, List<B>> /* synthetic access */
    ...
    override fun loadInitial(params: LoadInitialParams, callback: LoadInitialCallback<B>) {
        source.loadInitial(params, object : PositionalDataSource.LoadInitialCallback<A>() {
            override fun onResult(data: List<A>, position: Int, totalCount: Int) {
                callback.onResult(DataSource.convert(listFunction, data), position, totalCount)
            }

            override fun onResult(data: List<A>, position: Int) {
                callback.onResult(DataSource.convert(listFunction, data), position)
            }
    ...
    override fun loadRange(params: LoadRangeParams, callback: LoadRangeCallback<B>) {
        source.loadRange(params, object : PositionalDataSource.LoadRangeCallback<A>() {
            override fun onResult(data: List<A>) {
                callback.onResult(DataSource.convert(listFunction, data))
            }
