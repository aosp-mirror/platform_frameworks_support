...
public abstract class DataSource<Key, Value> {
    ...
    static class LoadCallbackHelper<T> {
        ...
        void dispatchResultToReceiver(@NonNull PageResult<T> result) {
            dispatchToReceiver(result, null);
        }

        void dispatchErrorToReceiver(@NonNull Error error) {
            dispatchToReceiver(null, error);
        }
        ...
        private void dispatchToReceiver(final @Nullable PageResult<T> result,
            ...
            if (executor != null) {
                executor.execute(new Runnable() {
                    @Override
                    public void run() {
                        dispatchOnCurrentThread(result, error);
                    }
                });
            } else {
                dispatchOnCurrentThread(result, error);
        ...
        void dispatchOnCurrentThread(@Nullable PageResult<T> result,
            ...
            if (result != null) {
                ...
                mReceiver.onPageError(mResultType, error);
            }
        }
    }
    ...
    /**
     * Error container for DataSource loading errors.
     */
    public static final class Error {
        ...
        @SuppressWarnings("ConstantConditions")
        public Error(@NonNull Throwable throwable, boolean isRetryable) {
            if (throwable == null) {
