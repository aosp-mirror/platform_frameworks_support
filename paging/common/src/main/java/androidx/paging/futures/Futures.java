...
public class Futures {
    ...
    // TODO: naming - doNotTransformAsync? :P
    public static <I, O> ListenableFuture<I> awaitAsync(
            @NonNull ListenableFuture<I> input,
            @NonNull final AsyncFunction<? super I, ? extends O> function,
            @NonNull final Executor executor) {
        // TODO: cancellation
        final ResolvableFuture<I> out = ResolvableFuture.create();
        addCallback(input, new FutureCallback<I>() {
            @Override
            public void onSuccess(final I origValue) {
                addCallback(function.apply(origValue), new FutureCallback<O>() {
                    @Override
                    public void onSuccess(O value) {
                        out.set(origValue);
                    }
