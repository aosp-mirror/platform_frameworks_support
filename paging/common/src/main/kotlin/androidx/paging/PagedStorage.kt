...
class PagedStorage<T> : AbstractList<T>, Pager.AdjacentProvider<T> {
    ...
    var positionOffset: Int = 0
        private set
    /**
     * Number of loaded items held by [.pages]. When tiling, doesn't count unloaded pages in
     * [.pages]. If tiling is disabled, same as [.mStorageCount].
     *
    ...
    /**
     * Number of items represented by [.pages]. If tiling is enabled, unloaded items in
     * [.pages] may be null, but this value still counts them.
     */
    ...
    fun needsTrimFromEnd(maxSize: Int, requiredRemaining: Int): Boolean {
        return needsTrim(maxSize, requiredRemaining, pages.size - 1)
    }
    ...
    fun allocatePlaceholders(index: Int, prefetchDistance: Int, pageSize: Int, callback: Callback) {
        ...
        val minimumPage = Math.max((index - prefetchDistance) / this.pageSize, 0)
        val maximumPage = Math.min((index + prefetchDistance) / this.pageSize, maxPageCount - 1)
    ...
    fun hasPage(pageSize: Int, index: Int): Boolean {
        // NOTE: we pass pageSize here to avoid in case pageSize
