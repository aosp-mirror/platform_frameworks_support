...
abstract class ItemKeyedDataSource<Key : Any, Value : Any> :
     ...
     * either if it's difficult to compute, or if [LoadInitialParams.placeholdersEnabled] is
    ...
    abstract class LoadInitialCallback<Value : Any> : LoadCallback<Value>() {
        /**
    ...
    abstract class LoadCallback<Value : Any> {
        /**
    ...
    internal fun getFutureAsCallback(future: ResolvableFuture<Result<Value>>): LoadCallback<Value> {
    ...
    ): ItemKeyedDataSource<Key, ToValue> = mapByPage(createListFunction(function))
}
