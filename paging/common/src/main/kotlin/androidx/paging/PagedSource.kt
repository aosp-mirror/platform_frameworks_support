...
abstract class PagedSource<Key : Any, Value : Any> {
    ...
    data class LoadParams<Key>(
        ...
        // TODO: Possibly delete if pageSize is unnecessary?
        val pageSize: Int
    )
    ...
    // TODO: Remove this after switching to coroutines and check if PagedSource can be an interface.
    private val _invalid = AtomicBoolean(false)
    val invalid: Boolean
        get() = _invalid.get()

    fun invalidate() {
        _invalid.compareAndSet(false, true)
    }
