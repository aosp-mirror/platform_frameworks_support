...
open class ContiguousPagedList<K : Any, V : Any>(
    private val _dataSource: DataSource<K, V>,
    ...
    override fun getLastKey() = when (_dataSource.type) {
        DataSource.KeyType.POSITIONAL -> lastLoad as K
        else -> lastItem?.let { _dataSource.getKeyInternal(it) }
    }
