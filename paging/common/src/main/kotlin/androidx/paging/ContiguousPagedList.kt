...
open class ContiguousPagedList<K, V>(
    private val _dataSource: DataSource<K, V> /* synthetic access */,
    ...
    override fun isDetached() = pager.isDetached

    override fun isContiguous() = true

    override val dataSource: DataSource<*, V>
        get() = _dataSource

    override val lastKey: Any?
        get() = _dataSource.getKey(lastLoad, lastItem)
    ...
    override fun onPageResult(type: LoadType, pageResult: DataSource.BaseResult<V>): Boolean {
        ...
        if (shouldTrim) {
            ...
            if (trimFromFront) {
                if (pager.mLoadStateManager.start != LoadState.LOADING) {
                    if (storage.trimFromFront(
                        ...
                        pager.mLoadStateManager.setState(
                            LoadType.START,
                            LoadState.IDLE,
                            null
                        )
                ...
                if (pager.mLoadStateManager.end != LoadState.LOADING) {
                    if (storage.trimFromEnd(
                        ...
                        pager.mLoadStateManager.setState(LoadType.END, LoadState.IDLE, null)
                    }
                }
            }
        }
    ...
    override fun retry() {
        ...
        if (pager.mLoadStateManager.refresh == LoadState.RETRYABLE_ERROR) {
    ...
    init {
        ...
        shouldTrim =
            _dataSource.supportsPageDropping() && config.maxSize != Config.MAX_SIZE_UNBOUNDED
    ...
    override fun dispatchCurrentLoadState(listener: LoadStateListener) =
        pager.mLoadStateManager.dispatchCurrentLoadState(listener)

    override fun setInitialLoadState(loadState: LoadState, error: Throwable?) =
        pager.mLoadStateManager.setState(LoadType.REFRESH, loadState, error)
    ...
    override fun loadAroundInternal(index: Int) {
        val prependItems =
            getPrependItemsRequested(getConfig().prefetchDistance, index, storage.leadingNullCount)
        val appendItems = getAppendItemsRequested(
            getConfig().prefetchDistance,
            index,
            storage.leadingNullCount + storage.storageCount
        )

        prependItemsRequested = Math.max(prependItems, prependItemsRequested)
        if (prependItemsRequested > 0) {
            pager.trySchedulePrepend()
        }

        appendItemsRequested = Math.max(appendItems, appendItemsRequested)
    ...
    companion object {
        ...
        fun getPrependItemsRequested(prefetchDistance: Int, index: Int, leadingNulls: Int) =
