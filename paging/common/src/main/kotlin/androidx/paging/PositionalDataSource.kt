 ...
 * the two parameter [LoadInitialCallback.onResult].
 *
 *
...
abstract class PositionalDataSource<T> : ListenablePositionalDataSource<T>() {
    ...
    abstract class LoadInitialCallback<T> {
         ...
         * requested (when [LoadInitialParams.placeholdersEnabled] is false), you can instead
         * call [.onResult].
         *
         ...
         * are disabled ([LoadInitialParams.placeholdersEnabled] is false).
         *
         *
         * Call this method from your DataSource's `loadInitial` function to return data,
    ...
    companion object {
        ...
        fun computeInitialLoadSize(
            ...
            return ListenablePositionalDataSource.computeInitialLoadSize(
                params, initialLoadPosition, totalCount
            )
        }
    }
        ...
        executor.execute {
            val callback = object : LoadRangeCallback<T>() {
                override fun onResult(data: List<T>) {
                    if (isInvalid) {
                        future.set(RangeResult(emptyList()))
                    } else {
                        future.set(RangeResult(data))
                    }
                }
    ...
    final override fun <V> mapByPage(
        function: Function<List<T>, List<V>>
    ): PositionalDataSource<V> = WrapperPositionalDataSource(this, function)

    final override fun <V> map(function: Function<T, V>): PositionalDataSource<V> =
        mapByPage(DataSource.createListFunction(function))
}
