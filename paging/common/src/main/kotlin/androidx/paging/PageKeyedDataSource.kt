...
abstract class PageKeyedDataSource<Key, Value> : ListenablePageKeyedDataSource<Key, Value>() {
    /**
     * Holder object for inputs to [.loadInitial].
     *
     * @param <Key> Type of data used to query pages.
    </Key> */
    open class LoadInitialParams<Key>(requestedLoadSize: Int, placeholdersEnabled: Boolean) :
    ...
    open class LoadParams<Key>(key: Key, requestedLoadSize: Int) :
    ...
    internal fun getFutureAsCallback(
        future: ResolvableFuture<Result<Key, Value>>
    ...
    final override fun <ToValue> mapByPage(
        function: Function<List<Value>, List<ToValue>>
    ): PageKeyedDataSource<Key, ToValue> = WrapperPageKeyedDataSource(this, function)

    final override fun <ToValue> map(
        function: Function<Value, ToValue>
    ): PageKeyedDataSource<Key, ToValue> = mapByPage(DataSource.createListFunction(function))
}
