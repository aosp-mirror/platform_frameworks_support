...
abstract class ListenablePageKeyedDataSource<Key : Any, Value> :
    DataSource<Key, Value>(KeyType.PAGE_KEYED) {

    internal override fun load(params: Params<Key>): ListenableFuture<out BaseResult<Value>> {
    ...
    open class LoadInitialParams<Key>(
        /**
     ...
     * count to the
     * [InitialResult constructor][InitialResult]. This
     * enables PagedLists presenting data from this source to display placeholders to represent
     * unloaded items.
     *
     *
     * [LoadInitialParams.requestedLoadSize] is a hint, not a requirement,
     ...
     * [LoadParams.key][PageKeyedDataSource.LoadParams.key].
     *
     *
    ...
    internal override fun getKey(item: Value): Key? = null

    internal override fun supportsPageDropping(): Boolean {
    ...
    open class InitialResult<Key, Value> : BaseResult<Value> {
        constructor(
            data: List<Value>,
            position: Int,
            totalCount: Int,
    ...
    /**
     * Type produced by [.loadBefore] and [.loadAfter] to
     * represent a page of loaded data.
     *
     * @param <Key> Type of key used to identify pages.
     * @param <Value> Type of items being loaded by the DataSource.
    </Value></Key> */
    open class Result<Key, Value>(data: List<Value>, adjacentPageKey: Key?) :
