...
internal class Pager<K, V>(
    val mConfig: PagedList.Config/* synthetic access */,
    val mSource: DataSource<K, V>/* synthetic access */,
    ...
    private var mNextKey: K? = null

    private val mDetached = AtomicBoolean(false)

    var mLoadStateManager: PagedList.LoadStateManager = object : PagedList.LoadStateManager() {
        override fun onStateChanged(
            type: PagedList.LoadType,
            state: PagedList.LoadState, error: Throwable?
        ) {
    ...
    init {
        ...
        if (mSource.type === DataSource.KeyType.POSITIONAL && mConfig.enablePlaceholders) {
    ...
    fun onLoadError(type: PagedList.LoadType, throwable: Throwable) {
        ...
        // TODO: handle nesting
        val state = if (mSource.isRetryableError(throwable))
    ...
    fun trySchedulePrepend() {
        if (mLoadStateManager.start == PagedList.LoadState.IDLE) {
            schedulePrepend()
        }
    }

    fun tryScheduleAppend() {
        if (mLoadStateManager.end == PagedList.LoadState.IDLE) {
            scheduleAppend()
        }
    }
    ...
    private fun scheduleAppend() {
        ...
        val key = when (mSource.type) {
            ...
            DataSource.KeyType.PAGE_KEYED -> mNextKey
            DataSource.KeyType.ITEM_KEYED -> mSource.getKey(adjacentProvider.lastLoadedItem!!)
        }

        mLoadStateManager.setState(PagedList.LoadType.END, PagedList.LoadState.LOADING, null)
        listenTo(
            PagedList.LoadType.END, mSource.load(
                DataSource.Params(
    ...
    fun retry() {
        if (mLoadStateManager.start == PagedList.LoadState.RETRYABLE_ERROR) {
            schedulePrepend()
        }
        if (mLoadStateManager.end == PagedList.LoadState.RETRYABLE_ERROR) {
            scheduleAppend()
        }
    }
