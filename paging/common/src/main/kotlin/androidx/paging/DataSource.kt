 ...
 * position in data set. Note - this is distinct from e.g. Room's `<Value> Value type loaded by the
 * DataSource.
 *
 */
...
internal constructor(internal val type: KeyType) {
    ...
    abstract class Factory<Key, Value> {
         ...
         */
        open fun <ToValue> map(function: Function<Value, ToValue>): Factory<Key, ToValue> =
     ...
     * @see DataSource.Factory.mapByPage
     */
    open fun <ToValue> map(function: Function<Value, ToValue>): DataSource<Key, ToValue> =
        mapByPage(Function { list -> list.map { function.apply(it) } })
    ...
    open class BaseResult<Value> protected constructor(
        ...
        internal companion object {
            ...
            internal fun <ToValue, Value> convert(
