...
internal constructor(internal val type: KeyType) {
    ...
    open fun invalidate() {
        if (invalid.compareAndSet(false, true)) {
            for (callback in onInvalidatedCallbacks) {
    ...
    internal fun getKey(lastLoad: Int, item: Value?) = when (type) {
        KeyType.POSITIONAL -> lastLoad as Key
        else -> item?.let { getKey(it) }
    }
