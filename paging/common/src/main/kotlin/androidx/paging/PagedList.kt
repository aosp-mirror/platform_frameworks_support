 ...
 * Items can be accessed with [.get], and further loading can be triggered with
 * [.loadAround]. To display a PagedList, see [androidx.paging.PagedListAdapter], which enables the
 * binding of a PagedList to a [androidx.recyclerview.widget.RecyclerView].
 * <h4>Loading Data</h4>
 *
 *
 ...
 *
 *
 * If you use [androidx.paging.LivePagedListBuilder] to get a
 * [androidx.lifecycle.LiveData], it will initialize PagedLists on a
 ...
 * listen to these updates with a [Callback]. (Note that [androidx.paging.PagedListAdapter] will listen
 ...
 * from [.isImmutable], and a new DataSource / PagedList pair must be created to load
 * further data. See [DataSource] and [androidx.paging.LivePagedListBuilder] for how new PagedLists are
 ...
 * a `List`. It will reference the same inner items, and contain the same `null`
...
abstract class PagedList<T> : AbstractList<T> {
    ...
    constructor(
        ...
        mRequiredRemainder = this.config.prefetchDistance * 2 + this.config.pageSize
    }

    internal var mRefreshRetryCallback: Runnable? = null
    ...
    internal val mRequiredRemainder: Int

    // if set to true, boundaryCallback is non-null, and should
    // be dispatched when nearby load has occurred
    internal /* synthetic access */ var mBoundaryCallbackBeginDeferred = false
    internal /* synthetic access */ var mBoundaryCallbackEndDeferred = false
    ...
    internal/* synthetic access */ val mListeners: ArrayList<WeakReference<LoadStateListener>>
     ...
     *
     *
     * If data is supplied by a [PositionalDataSource], the item returned from
     * `get(i)` has a position of `i + getPositionOffset()`.
     *
     *
     * If the DataSource is a [ItemKeyedDataSource] or [PageKeyedDataSource], it
     ...
     *
     *
     * You can use a [LoadStateListener] to observe [LoadState] of
     * any [LoadType]. For UI purposes (swipe refresh, loading spinner, retry button), this
     * is typically done by registering a Listener with the `PagedListAdapter` or
     ...
     *
     *
     * Can be used to observe the [LoadState] of any [LoadType] (REFRESH/START/END).
     * For UI purposes (swipe refresh, loading spinner, retry button), this is typically done by
     * registering a Listener with the `PagedListAdapter` or `AsyncPagedListDiffer`.
     *
     *
     * These calls will be dispatched on the executor defined by
     * [Builder.setNotifyExecutor], which is generally the main/UI thread.
     *
     * @see LoadType
     *
     * @see LoadState
    ...
    interface LoadStateListener {
         ...
         * can be used to drive loading spinner items in your `RecyclerView`.
         *
        ...
        fun onLoadStateChanged(
            type: LoadType,
            state: LoadState, error: Throwable?
        )
    }
    ...
    internal abstract class LoadStateManager {
        ...
        fun setState(type: LoadType, state: LoadState, error: Throwable?) {
            ...
            when (type) {
                LoadType.REFRESH -> {
                    if (refresh == state && equalsHelper(mRefreshError, error)) return
                    refresh = state
                    mRefreshError = error
                }
                LoadType.START -> {
                    if (start == state && equalsHelper(mStartError, error)) return
                    start = state
                    mStartError = error
                }
                LoadType.END -> {
                    if (end == state && equalsHelper(mEndError, error)) return
                    end = state
                    mEndError = error
                }
            }
     ...
     * failed append load. Note that your DataSource will need to pass `true` to
     * `onError()` to signify the error as retryable.
     *
     *
     * You can observe loading state via [.addWeakLoadStateListener],
     * though generally this is done through the [androidx.paging.PagedListAdapter] or
    ...
    internal fun dispatchStateChange(type: LoadType, state: LoadState, error: Throwable?) {
        for (i in mListeners.indices.reversed()) {
            val currentListener = mListeners[i].get()
            currentListener?.onLoadStateChanged(type, state, error) ?: mListeners.removeAt(i)
        }
    }
     ...
     *
     *
     * [androidx.paging.LivePagedListBuilder] does this creation on a background thread automatically, if you
     ...
     */
        (private val dataSource: DataSource<Key, Value>, private val config: Config) {
         ...
         *
         *
         * This call will dispatch the [androidx.paging.DataSource]'s loadInitial method immediately on the
         ...
         * `LiveData<PagedList>` can be safely constructed with [androidx.paging.LivePagedListBuilder]
        ...
        @WorkerThread
        @Deprecated(
            "This method has no means of handling errors encountered during initial load,\n" +
    ...
    internal fun deferBoundaryCallbacks(
        deferEmpty: Boolean,
        deferBegin: Boolean, deferEnd: Boolean
    ) {
        ...
        if (mLowestIndexAccessed == Integer.MAX_VALUE) {
            mLowestIndexAccessed = storage.size
        }
     ...
     */
    internal /* synthetic access */ fun tryDispatchBoundaryCallbacks(post: Boolean) {
        val dispatchBegin =
            mBoundaryCallbackBeginDeferred && mLowestIndexAccessed <= config.prefetchDistance
        val dispatchEnd =
            mBoundaryCallbackEndDeferred && mHighestIndexAccessed >= size - 1 - config.prefetchDistance
    ...
    internal /* synthetic access */ fun dispatchBoundaryCallbacks(begin: Boolean, end: Boolean) {
     ...
     *
     * To get the number of loaded items, not counting placeholders, use [.getLoadedCount].
     *
     ...
     *
     * If this PagedList [is immutable][.isImmutable] due to its DataSource being invalid, it
     * will be returned.
     *
     ...
     * reference to its observer, such as a [androidx.paging.PagedListAdapter]. If an adapter were held with a
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    abstract fun dispatchUpdatesSinceSnapshot(
        snapshot: PagedList<T>,
        callback: Callback
    )
     ...
     * the executor defined by [Builder.setNotifyExecutor], which is generally
     * the main/UI thread.
     */
     ...
     *
     *
     * Use a Config [Builder] to construct and define custom loading behavior, such as
    ...
    class Config internal constructor(
        ...
        class Builder {
            ...
            private var mEnablePlaceholders = true
            private var mMaxSize = MAX_SIZE_UNBOUNDED
             ...
             * @return this
             */
            fun setPageSize(@IntRange(from = 1) pageSize: Int): Builder {
                if (pageSize < 1) {
             ...
             * @return this
             */
            fun setPrefetchDistance(@IntRange(from = 0) prefetchDistance: Int): Builder {
             ...
             * [PagedList.Callback.onChanged] will not be issued as part of loading,
             * though a [androidx.paging.PagedListAdapter] may still receive change events as a result of
             * PagedList diffing.
             *
             * @param enablePlaceholders False if null placeholders should be disabled.
             * @return this
             */
            fun setEnablePlaceholders(enablePlaceholders: Boolean): Builder {
             ...
             * @return this
             */
            fun setInitialLoadSizeHint(@IntRange(from = 1) initialLoadSizeHint: Int): Builder {
             ...
             * [.setPrefetchDistance] prefetch distance} plus the
             * [page size][.setPageSize]). This constraint prevent loads from being
             ...
             * [requested pageSize][Config.pageSize], so two pages may be larger than
             * expected.
             ...
             * `MAX_SIZE_UNBOUNDED` to disable page dropping.
             * @return this
             *
             ...
             * @see Config.maxSize
             */
            fun setMaxSize(@IntRange(from = 2) maxSize: Int): Builder {
                mMaxSize = maxSize
                return this
            }
            ...
            fun build(): Config {
                ...
                return Config(
                    mPageSize, mPrefetchDistance,
                    mEnablePlaceholders, mInitialLoadSizeHint, mMaxSize
                )
            }

            companion object {
                internal val DEFAULT_INITIAL_PAGE_MULTIPLIER = 3
            }
        }

        companion object {
            /**
             * When [.maxSize] is set to `MAX_SIZE_UNBOUNDED`, the maximum number of items
             * loaded is unbounded, and pages will never be dropped.
             */
            val MAX_SIZE_UNBOUNDED = Integer.MAX_VALUE
        }
    }
     ...
     * [androidx.paging.LivePagedListBuilder.setBoundaryCallback], the callbacks may be issued multiple
     ...
     * [in the
 * Paging Codelab](https://codelabs.developers.google.com/codelabs/android-paging/index.html#8), the GitHub network page index is stored in memory.
     *
    ...
    abstract class BoundaryCallback<T> {
         ...
         */
        abstract fun onZeroItemsLoaded()
         ...
         */
        abstract fun onItemAtFrontLoaded(itemAtFront: T)
         ...
         */
        abstract fun onItemAtEndLoaded(itemAtEnd: T)
    }

    companion object {

        /* synthetic access */
        internal fun equalsHelper(a: Any?, b: Any?): Boolean {
        ...
        internal fun <K, T> create(
            ...
            dataSource.executor = initialLoadExecutor
            ...
            return Futures.transform(
                ...
                Function { initialResult ->
                    dataSource.executor = fetchExecutor
                    ContiguousPagedList(
                        dataSource,
                        notifyExecutor,
                        fetchExecutor,
