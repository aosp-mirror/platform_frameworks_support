 ...
 * from [.isImmutable], and a new DataSource / PagedList pair must be created to load further data.
...
abstract class PagedList<T> : AbstractList<T> {
    ...
    class Builder<Key, Value> {
         ...
         * @return this
         */
        fun setBoundaryCallback(boundaryCallback: BoundaryCallback<Value>?): Builder<Key, Value> {
            this.boundaryCallback = boundaryCallback
            return this
        }
     ...
     * held with a strong reference, it would be necessary to clear its PagedList observer before it
     * could be GC'd.
     *
    ...
    class Config internal constructor(
        ...
        class Builder {
            ...
            fun build(): Config {
                ...
                if (maxSize != MAX_SIZE_UNBOUNDED) {
                    if (maxSize < pageSize + prefetchDistance * 2) {
                        throw IllegalArgumentException(
                            "Maximum size must be at least" +
                                    " pageSize + 2*prefetchDist, pageSize=" + pageSize +
                                    ", prefetchDist=" + prefetchDistance + ", maxSize=" + maxSize
                        )
                    }
                }

                return Config(
                    pageSize, prefetchDistance,
                    enablePlaceholders, initialLoadSizeHint, maxSize
                )
            }
        ...
        companion object {
             ...
             */
            const val MAX_SIZE_UNBOUNDED = Integer.MAX_VALUE
        }
    }
