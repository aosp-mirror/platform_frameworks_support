 ...
 * use [PageKeyedDataSource] or [ItemKeyedDataSource], or pass the initial result with
 * the two parameter [InitialResult constructor][InitialResult].
 *
 * @see PositionalDataSource
 *
 * @param <T> Type of items being loaded by the PositionalDataSource.
 */
abstract class ListenablePositionalDataSource<T> : DataSource<Int, T>(KeyType.POSITIONAL) {

    /**
     * @hide
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    final override fun load(params: Params<Int>): ListenableFuture<out BaseResult<T>> {
        if (params.type == LoadType.INITIAL) {
            ...
            if (params.key != null) {
                ...
                if (params.placeholdersEnabled) {
                    ...
                    initialLoadSize =
                        Math.max(initialLoadSize / params.pageSize, 2) * params.pageSize
    ...
    public final override fun getKey(item: T): Int? = null
    ...
    companion object {
        ...
        fun computeInitialLoadSize(
            ...
            totalCount: Int
        ) = Math.min(totalCount - initialLoadPosition, params.requestedLoadSize)
    }

    /**
     * Type produced by [.loadInitial] to represent
     * initially loaded data.
     *
     * @param <V> The type of the data loaded.
    </V> */
    open class InitialResult<V> : BaseResult<V> {
    ...
    /**
     * Type produced by [.loadRange] to represent a page
     * of loaded data.
     *
     * @param <V> The type of the data loaded.
    </V> */
    open class RangeResult<V>(data: List<V>) : BaseResult<V>(data, null, null, 0, 0, 0, false)
}
