 ...
 *
 * @constructor Create a DataSource from the customer table of the given database
 */
...
internal constructor(private val db: SampleDatabase) : ItemKeyedDataSource<String, Customer>() {
    companion object {
        internal fun factory(db: SampleDatabase) = object : DataSource.Factory<String, Customer>() {
            override fun create() = LastNameAscCustomerDataSource(db)
        }
    ...
    override fun loadInitial(
        ...
        if (params.placeholdersEnabled && list.isNotEmpty()) {
            val firstKey = getKey(list.first())
            val lastKey = getKey(list.last())
