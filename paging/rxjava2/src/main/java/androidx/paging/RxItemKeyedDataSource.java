...
abstract class RxItemKeyedDataSource<Key, Value> extends ListenableItemKeyedDataSource<Key, Value> {
    ...
    /**
     * An explicit getter is necessary to lazily init mScheduledExecutor as
     * {@link DataSource#getExecutor()} throws an error if it is accessed before this DataSource
     * is added to a {@link PagedList} and assigned an {@link Executor}.
     */
    private ScheduledExecutor getScheduledExecutor() {
        if (mScheduledExecutor == null) {
            mScheduledExecutor = new ScheduledExecutor(getExecutor());
        }
