...
internal abstract class RxItemKeyedDataSource<Key, Value> :
    ...
    override fun loadInitial(params: LoadInitialParams<Key>) =
        RxDataSourceUtil.singleToListenableFuture(onLoadInitial(params), executor, scheduler)

    override fun loadAfter(params: LoadParams<Key>) =
        RxDataSourceUtil.singleToListenableFuture(onLoadAfter(params), executor, scheduler)

    override fun loadBefore(params: LoadParams<Key>) =
        RxDataSourceUtil.singleToListenableFuture(onLoadBefore(params), executor, scheduler)
