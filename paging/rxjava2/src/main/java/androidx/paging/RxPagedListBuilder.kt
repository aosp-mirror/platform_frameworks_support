...
class RxPagedListBuilder<Key, Value>(
    ...
    fun setNotifyScheduler(scheduler: Scheduler) = apply {
        notifyExecutor = when (scheduler) {
            is Executor -> scheduler as ScheduledExecutor
    ...
    fun setFetchScheduler(scheduler: Scheduler) = apply {
        fetchExecutor = when (scheduler) {
            is ScheduledExecutor -> scheduler
            else -> ScheduledExecutor(scheduler)
        }
    ...
    internal class PagingObservableOnSubscribe<Key, Value>(
        ...
        private var emitter: ObservableEmitter<PagedList<Value>>? = null
        ...
        private fun createPagedList(): PagedList<Value> {
            ...
            val initializeKey = if (list != null) list!!.lastKey as Key? else initialLoadKey

            do {
                dataSource?.removeInvalidatedCallback(this)
                dataSource = dataSourceFactory.create()
                dataSource!!.addInvalidatedCallback(this)
