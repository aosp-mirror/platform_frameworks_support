                    ...
                    jintArray limits)
{
    ...
    if (limits != nullptr) {
        ...
        if (limit_len != 6) {
            LOG_API("LunchOptions cannot be recognized.");
            goto exit;
        }
...
nIncAllocationCreateTyped(JNIEnv *_env, jobject _this, jlong con, jlong incCon, jlong alloc, jlong type, jint xBytesSize)
{
    ...
    if (alloc != 0) {
        ...
        /*
         * By definition stride is a roundup of xBytesSize with byteAligned, so byteAligned must
         * be strictly larger than the difference of (stride - xBytesSize).
         *
         * We can prove that as long as byteAligned satisfies the following two conditions, the
         * memory layout will be identical :
         * 1. Smaller or equal than stride;
         * 2. Larger than minByteAligned.
         *
         * In this case we can simply choose the first power of 2 satisfies both conditions.
         */
        size_t byteAligned = 16;
        size_t minByteAligned = strideIn - xBytesSize;
        while (byteAligned <= minByteAligned) {
            byteAligned <<= 1;
        }
