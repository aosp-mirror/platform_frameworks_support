    ...
    private static RenderScript internalCreate(Context ctx, int sdkVersion, ContextType ct, int flags) {
        ...
        if (!rs.nLoadSO(useNative, dispatchAPI, rssupportPath)) {
            ...
            try {
                ...
                throw new RSRuntimeException("Error loading RS Compat library: " + e + " Support lib API: " + SUPPORT_LIB_VERSION);
            }
            if (!rs.nLoadSO(false, dispatchAPI, rssupportPath)) {
                throw new RSRuntimeException("Error loading libRSSupport library, Support lib JAVA API: " + SUPPORT_LIB_VERSION);
            }
        }
