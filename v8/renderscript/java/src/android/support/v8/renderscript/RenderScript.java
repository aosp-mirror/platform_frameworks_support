...
public class RenderScript {
    ...
    private static class LibraryLoader {
        ...
        private static String getFileNameInMap(String libName) {
            String cpu_abi = Build.CPU_ABI;
            if (cpu_abi.contains("arm64")) {
                // 64bit ARM case
                return "ARM64_" + libName;
            } else if (cpu_abi.contains("armeabi")) {
                return "ARM_" + libName;
            } else if (cpu_abi.contains("mips")) {
                return "MIPS_" + libName;
        ...
        private static String getRefChecksums(String libName) {
            String libNameinMap = getFileNameInMap(libName);
            return MD5CheckSums.getMD5CheckSums(libNameinMap);
        }
        ...
        // Extract shared library from the APK to the workaround dir.
        ...
        // Calculate MD5 Checksum of a shared lib.
        ...
        // Check the workaround marker to see if the workaround has been applied or not.
        ...
        // Method to load shared library and apply workaround if needed.
        private static String loadRSLibraries(Context context, String library) {
            String libChecksum = getRefChecksums(library);
            ...
            if (libChecksum.equals(getMD5Checksum(nativeLibName))) {
    ...
    */
    ...
    synchronized long nScriptIntrinsicCreate(int id, long eid, boolean mUseInc) {
        ...
        if (mUseInc) {
            ...
            if (!mIncLoaded) {
                String libRSSupportPath = LibraryLoader.loadRSLibraries(mApplicationContext, "RSSupport");
    ...
    private static RenderScript internalCreate(Context ctx, int sdkVersion, ContextType ct, int flags) {
        ...
        synchronized(lock) {
            if (sInitialized == false) {
                ...
                try {
                    if (LibraryLoader.loadRSLibraries(ctx, "rsjni") == null) {
        ...
        if (!useNative || !rs.nLoadSO(true, dispatchAPI, null)) {
            ...
            String libRSSupportPath = LibraryLoader.loadRSLibraries(ctx, "RSSupport");
            if (libRSSupportPath == null) {
        ...
        if (useIOlib) {
            String libRSSupportIOPath = LibraryLoader.loadRSLibraries(ctx, "RSSupportIO");
        ...
        if (dispatchAPI >= 23) {
            ...
            if (LibraryLoader.loadRSLibraries(ctx, "blasV8") == null) {
