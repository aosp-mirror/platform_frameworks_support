...
class PostponedTransitionTest {
    ...
    fun testTimedPostponeDuration() {
        val fm = activityRule.activity.supportFragmentManager
        val startBlue = activityRule.activity.findViewById<View>(R.id.blueSquare)

        val fragment = PostponedDurationFragment()
        fm.beginTransaction()
            .addSharedElement(startBlue, "blueSquare")
            .replace(R.id.fragmentContainer, fragment)
            .addToBackStack(null)
    ...
    class PostponedDurationFragment : TransitionFragment(R.layout.scene2) {
        val startPostponedCountDownLatch = CountDownLatch(1)
        override fun onCreateView(
            inflater: LayoutInflater,
            container: ViewGroup?,
            ...
            postponeEnterTransition(Duration.ofMillis(1000))
        }
