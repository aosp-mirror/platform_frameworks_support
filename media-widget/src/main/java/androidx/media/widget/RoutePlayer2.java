...
public class RoutePlayer2 extends RemoteSessionPlayer2 {
    ...
    private final List<SettableFuture<CommandResult2>> mPendingVolumeResult = new ArrayList<>();
        ...
        public void onRouteVolumeChanged(MediaRouter router, MediaRouter.RouteInfo route) {
            if (TextUtils.equals(route.getId(), mSelectedRoute.getId())) {
                ...
                for (Map.Entry<PlayerCallback, Executor> entry : callbacks.entrySet()) {
                    final RemoteSessionPlayer2.Callback callback = (Callback) entry.getKey();
                    final Executor executor = entry.getValue();
                    executor.execute(new Runnable() {
                        @Override
                        public void run() {
                            callback.onVolumeChanged(RoutePlayer2.this, volume);
                        }
                    });
                }
            }
