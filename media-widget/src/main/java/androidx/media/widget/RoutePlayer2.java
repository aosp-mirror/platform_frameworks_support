...
public class RoutePlayer2 extends RemoteSessionPlayer2 {
    ...
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    final List<SettableFuture<PlayerResult2>> mPendingVolumeResult = new ArrayList<>();
        ...
        public void onRouteVolumeChanged(MediaRouter router, MediaRouter.RouteInfo route) {
            if (TextUtils.equals(route.getId(), mSelectedRoute.getId())) {
                ...
                for (int i = 0; i < mPendingVolumeResult.size(); i++) {
                    mPendingVolumeResult.get(i).set(new PlayerResult2(
    ...
    public ListenableFuture<PlayerResult2> play() {
        ...
        if (mClient.isSessionManagementSupported()) {
            final SettableFuture<PlayerResult2> result = SettableFuture.create();
            mClient.resume(null, new SessionActionCallback() {
                ...
                public void onResult(Bundle data,
                    ...
                    // play state.
                    result.set(new PlayerResult2(RESULT_CODE_SUCCESS, getCurrentMediaItem()));
                }
            });
        }
            ...
            mClient.seek(mItemId, pos, null, new ItemActionCallback() {
                ...
                public void onResult(Bundle data,
                    ...
                    if (itemStatus != null) {
                        ...
                        result.set(new PlayerResult2(RESULT_CODE_UNKNOWN_ERROR,
                                getCurrentMediaItem()));
                    }
                }
            });
    ...
    @Override
    public Future<PlayerResult2> adjustVolume(int direction) {
        mSelectedRoute.requestUpdateVolume(direction);

        SettableFuture<PlayerResult2> result = SettableFuture.create();
        mPendingVolumeResult.add(result);
        return result;
    }

    @Override
    public Future<PlayerResult2> setVolume(int volume) {
    ...
    private ListenableFuture<PlayerResult2> playInternal() {
    ...
    private ListenableFuture<PlayerResult2> createResult() {
        return createResult(RESULT_CODE_SUCCESS);
    }

    private ListenableFuture<PlayerResult2> createResult(int code) {
        SettableFuture<PlayerResult2> result = SettableFuture.create();
        result.set(new PlayerResult2(code, getCurrentMediaItem()));
        return result;
    }
}
