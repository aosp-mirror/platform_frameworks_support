...
public final class AnimatorSet extends Animator implements AnimationHandler.AnimationFrameCallback {
    ...
    public boolean doAnimationFrame(long frameTime) {
        ...
        if (mSeekState.isActive()) {
            ...
            if (mReversing) {
                mFirstFrame = frameTime - (long) (mSeekState.getPlayTime() * durationScale);
            } else {
        ...
        if (!mReversing && frameTime < mFirstFrame + (long) (mStartDelay * durationScale)) {
    ...
    // masked a real bug in play movies.
     ...
     * a child animation with repeatCount = {@link ValueAnimator#INFINITE}
     *
     * @return true if the animator set is reversible, false otherwise.
     */
    @Override
    ...
    private static final Comparator<AnimationEvent> EVENT_COMPARATOR =
            new Comparator<AnimationEvent>() {
            @Override
    ...
    private void sortAnimationEvents() {
        ...
        Collections.sort(mEvents, EVENT_COMPARATOR);
