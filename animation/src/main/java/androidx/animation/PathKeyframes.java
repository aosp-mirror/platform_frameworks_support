...
class PathKeyframes implements Keyframes<PointF> {
    ...
    private float[] createKeyFrameData(Path path, float precision) {
        ...
        for (int i = 0; i < numPoints; ++i) {
            ...
            if (i > 0 && twoPointsOnTheSameLinePath(tangent[0], tangent[1], lastTangent[0],
                    lastTangent[1], position[0], position[1], results.get(lastIndex - 1),
    ...
    private boolean twoPointsOnTheSameLinePath(float tanX1, float tanY1, float tanX2, float tanY2,
            float x1, float y1, float x2, float y2) {
        if (Math.abs(tanX1 - tanX2) > EPSILON || Math.abs(tanY1 - tanY2) > EPSILON) {
            return false;
        }
        float deltaX = x1 - x2;
        float deltaY = y1 - y2;
        // If deltaY / deltaX = tanY1 / tanX1, that means the two points are on the same line as
        // the path.
        return Math.abs(deltaX * tanY1 - deltaY * tanX1) < EPSILON;
    }
