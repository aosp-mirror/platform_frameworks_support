...
public abstract class BroadcastReceiverWithCallbacks<T extends CallbackReceiver> extends
    ...
    public T createRemoteCallback() {
        CallbackHandlerRegistry.sInstance.ensureInitialized(getClass());
        return CallbackHandlerRegistry.sInstance.getAndResetStub(getClass());
    }
    ...
    public RemoteCallback toRemoteCallback(Context context) {
        ...
        intent.setComponent(new ComponentName(context.getPackageName(), clsName));
        Bundle args = CallbackHandlerRegistry.sInstance.getArgs(
                (CallbackHandlerRegistry.RemoteCallStub) this);
        intent.putExtras(args);
        return new RemoteCallback(context, TYPE_RECEIVER, intent, clsName, args);
    }
}
