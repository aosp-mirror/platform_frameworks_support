...
public class CallbackHandlerRegistry {
    ...
    <T extends CallbackReceiver> Bundle getArgs(RemoteCallStub stub) {
        Bundle b = stub.getArguments();
        b.putString(EXTRA_METHOD, stub.getMethod());
        return b;
    }
    ...
    <T extends CallbackReceiver> T getAndResetStub(Class<? extends CallbackReceiver> cls,
        ...
        try {
            stub.mLock.tryLock(2, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            throw new RuntimeException("Deadlock in callback generation,"
                    + " incomplete RemoteCallback generation?");
        }
        stub.mCallStub.reset();
        initStub(stub.mCallStub, context, authority);
        ...
        if (stub instanceof AppWidgetProviderWithCallbacks) {
            ...
            if (contentProvider.mAuthority == null) {
                contentProvider.mAuthority =
                        determineAuthority(context, authority, stub.getClass());
            }
        }
    }
    ...
    private String determineAuthority(Context context, String authority, Class<?> aClass) {
        ...
        try {
            ProviderInfo info = context.getPackageManager().getProviderInfo(
                    new ComponentName(context.getPackageName(), aClass.getSuperclass().getName()),
                    0);
    ...
    private <T extends CallbackReceiver> void runInit(Class<T> cls) {
        try {
            ...
            clsHandler.mCallStub =
                    (RemoteCallStub) findInitClass(cls).getDeclaredConstructor().newInstance();
     ...
     * otherwise.
     */
    public static RemoteCallback toRemoteCallback(CallbackReceiver receiver) {
        sInstance.findMap(receiver.getClass().getSuperclass()).mLock.unlock();
        return receiver.toRemoteCallback();
    }

    private static class ClsHandler {
        final ReentrantLock mLock = new ReentrantLock();
        final ArrayMap<String, CallbackHandler<? extends CallbackReceiver>> mHandlers =
                new ArrayMap<>();
        RemoteCallStub mCallStub;
    }
    ...
    /**
     * Interface implemented by generated RemoteCallable stubs.
     * Note: This should only be referenced by generated code, there is no reason to reference
     * this otherwise.
     */
    public interface RemoteCallStub {
        /**
         * Reset the state for a new remote call.
         * Note: This should only be referenced by generated code, there is no reason to reference
         * this otherwise.
         */
        void reset();
        ...
        /**
         * Get the arguments that were called with the method.
         * Note: This should only be referenced by generated code, there is no reason to reference
         * this otherwise.
         */
        Bundle getArguments();
