...
public class CallableMethod {
    ...
    public void addMethods(TypeSpec.Builder genClass, MethodSpec.Builder runBuilder,
        ...
        if (!"androidx.remotecallback.RemoteCallback".equals(mReturnType.toString())) {
            messager.printMessage(Diagnostic.Kind.ERROR,
                    "RemoteCallable methods must return RemoteCallback.NONE.");
            return;
        }
        ...
        code.addStatement("throw new IllegalStateException($S)",
                "Only one remotable method can be called per callback");
        code.endControlFlow();
        ...
        code.addStatement(
                "return androidx.remotecallback.CallbackHandlerRegistry.toRemoteCallback(this)");
