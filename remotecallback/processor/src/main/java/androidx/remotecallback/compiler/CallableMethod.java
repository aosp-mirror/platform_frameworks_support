...
public class CallableMethod {
    ...
    public void addMethods(TypeSpec.Builder genClass, MethodSpec.Builder runBuilder,
        ...
        code.beginControlFlow("if (args.length != $L)", countArgs(context));
        code.addStatement("throw new IllegalArgumentException($S)",
                mElement.getSimpleName() + " takes " + mNames.size() + " arguments");
        code.endControlFlow();
        ...
        for (int i = 0; i < mNames.size(); i++) {
            ...
            code.beginControlFlow("if (args[$L] != null)", argIndex);
            code.beginControlFlow("if (args[$L] instanceof $L)", argIndex, remoteInputHolder);

            // RemoteInputHolder case.
            code.addStatement("$L input = ($L) args[$L]", remoteInputHolder, remoteInputHolder,
                    argIndex);
            code.beginControlFlow("if (input.getType() != $L.class)", mTypes.get(i));
            code.addStatement("throw new ClassCastException($S)", mNames.get(i) + " expects a "
                    + mTypes.get(i));
            code.endControlFlow();
            ...
            code.addStatement("b.put$L($L, ($L) args[$L])", getTypeMethod(mTypes.get(i)),
                    getBundleKey(i), mTypes.get(i), argIndex);
            code.endControlFlow();
