...
class GhostViewHolder extends FrameLayout {
    ...
    private static boolean isOnTop(View view, View comparedWith) {
        ...
        final List<View> preorderedList = buildOrderedChildList(parent, new ArrayList<View>());
    ...
    @Nullable
    private static List<View> buildOrderedChildList(@NonNull ViewGroup viewGroup,
            @NonNull List<View> children) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            // there is no Z order before lollipop
            return null;
        }
        final int childrenCount = viewGroup.getChildCount();

        boolean hasChildWithZ = false;
        for (int i = 0; i < childrenCount; i++) {
            if (viewGroup.getChildAt(i).getZ() != 0) {
                hasChildWithZ = true;
                break;
            }
        }
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            final int childIndex = ViewGroupUtils.getChildDrawingOrder(viewGroup, i);
            final View nextChild = viewGroup.getChildAt(childIndex);
            final float currentZ = nextChild.getZ();

            // insert ahead of any Views with greater Z
            int insertIndex = i;
            while (insertIndex > 0 && children.get(insertIndex - 1).getZ() > currentZ) {
                insertIndex--;
            }
            children.add(insertIndex, nextChild);
        }
        return children;
    }
