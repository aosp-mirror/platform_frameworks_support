...
class RecyclerViewSmoothScrollToPositionTest {
    ...
    @Test
    @Throws(Throwable::class)
    fun smoothScrollToPosition_calledAfterPreviousJustBeforeStop_scrollStateCallbacksCorrect() {
        val called2ndTime = -1
        ...
        recyclerView.onScrolled {
            recyclerView.getChildWithTag(target)?.let {
        ...
        recyclerView.onScrollStateChanged { newState ->
            log.add(newState)
...
private fun RecyclerView.onScrollStateChanged(callback: (newState: Int) -> Unit) {
...
private fun RecyclerView.onScrolled(callback: () -> Unit) {
    this.addOnScrollListener(object : RecyclerView.OnScrollListener() {
...
private fun ViewGroup.getChildWithTag(tag: Int): View? {
