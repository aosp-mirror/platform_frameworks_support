...
public class RecyclerViewAccessibilityTest extends BaseRecyclerViewInstrumentationTest {
            ...
            public void onBindViewHolder(@NonNull TestViewHolder holder, int position) {
                ...
                ViewCompat.addAccessibilityAction(holder.itemView, customAction1Title,
                        new AccessibilityViewCommand() {
                            @Override
                            public boolean perform(CommandArguments arguments) {
    ...
    @Test
    public void actionsAreClearedOnReuse() throws Throwable {
        final String actionLabel = "Test swipe action";
        final TestAdapter adapter = new TestAdapter(10) {
            public void onBindViewHolder(@NonNull TestViewHolder holder, int position) {
                super.onBindViewHolder(holder, position);
                ViewCompat.addAccessibilityAction(holder.itemView, actionLabel,
                        new AccessibilityViewCommand() {
                            int mCallCount = 0;
                            @Override
                            public boolean perform(CommandArguments arguments) {
                                assertEquals("Action not removed on recycle", 1,
                                        ++mCallCount);
                                return true;
                            }
                        });
        ...
        final RecyclerView recyclerView = new RecyclerView(getActivity());
        recyclerView.setAdapter(adapter);
        //DumblayoutManager scraps all children on layout.
        final DumbLayoutManager layoutManager = new DumbLayoutManager();
        recyclerView.setLayoutManager(layoutManager);
        layoutManager.expectLayouts(1);
        setRecyclerView(recyclerView);
        ...
        callCustomSwipeActionOnEveryChild(recyclerView, actionLabel);
    ...
    private void callCustomSwipeActionOnEveryChild(RecyclerView recyclerView, String actionLabel) {
        for (int i = 0; i < recyclerView.getChildCount(); i++) {
            View view = recyclerView.getChildAt(i);
            for (AccessibilityActionCompat action : getActionsOnView(view)) {
                if (actionLabel.equals(action.getLabel())) {
                    ViewCompat.performAccessibilityAction(view, action.getId(), null);
                }
            }
        }
    }
    ...
    @Test
    public void removeStandardActionFromItem() throws Throwable {

        final int actionId = AccessibilityNodeInfo.AccessibilityAction.ACTION_SELECT.getId();

        //How to remove a standard action from a view
        // They can also use lambdas.
        final TestAdapter adapter = new TestAdapter(1) {
            public void onBindViewHolder(@NonNull TestViewHolder holder, int position) {
                super.onBindViewHolder(holder, position);
                ViewCompat.removeAccessibilityAction(holder.itemView, actionId);
            }
        };
        ...
        View view = recyclerView.getChildAt(0);
        for (AccessibilityActionCompat action : getActionsOnView(view)) {
            assertNotEquals("Select action should have been removed", actionId,
                    action.getId());
        }
    }

    @Test
    public void removeStandardActionFromItem_getsClearedUpOnReuse() throws Throwable {
        ...
        // They can also use lambdas.
        final TestAdapter adapter = new TestAdapter(1) {
            int mTimesToBeRemoved = 1;
            public void onBindViewHolder(@NonNull TestViewHolder holder, int position) {
                super.onBindViewHolder(holder, position);
                if (mTimesToBeRemoved > 0) {
                    ViewCompat.removeAccessibilityAction(holder.itemView, actionId);
                    mTimesToBeRemoved--;
                }
            }
        ...
        boolean selectActionFound = false;
        for (AccessibilityActionCompat action : getActionsOnView(view)) {
            selectActionFound |= actionId == action.getId();
        }
        assertTrue("Select action should not have been removed after reuse", selectActionFound);
    }
}
