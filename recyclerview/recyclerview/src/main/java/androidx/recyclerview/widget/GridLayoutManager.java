...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    private boolean mUseSpansToEstimateScrollBarDimensions;
    ...
    public abstract static class SpanSizeLookup {
         ...
         * {@link #getSpanGroupIndex(int, int)}, and you are using spans to calculate scrollbar
         * offset and range, you should set this to true for better performance.
         *
        ...
        public int getSpanIndex(int position, int spanCount) {
            ...
            if (mCacheSpanIndices && mSpanIndexCache.size() > 0) {
                int prevKey = findFirstKeyLessThan(position, mSpanIndexCache);
                if (prevKey >= 0) {
        ...
        static int findFirstKeyLessThan(int position, SparseIntArray cache) {
            int lo = 0;
            int hi = cache.size() - 1;
        ...
        public int getSpanGroupIndex(int adapterPosition, int spanCount) {
            ...
            int group = 0;
            int start = 0;
            if (mCacheSpanGroupIndices && mSpanGroupIndexCache.size() > 0) {
                // This finds the first non empty cached group cache key.
                int prevKey = findFirstKeyLessThan(adapterPosition, mSpanGroupIndexCache);
                group = mSpanGroupIndexCache.get(prevKey);
                start = prevKey + 1;
                span = getCachedSpanIndex(adapterPosition, spanCount) + getSpanSize(prevKey);
                if (span == spanCount) {
                    span = 0;
                    group++;
                }
    ...
    public int computeHorizontalScrollRange(RecyclerView.State state) {
        if (mUseSpansToEstimateScrollBarDimensions) {
     ...
     * {@link SpanSizeLookup#getSpanGroupIndex(int, int)} are fast and that set span group index
     * caching to true via a call to {@link SpanSizeLookup#setSpanGroupIndexCacheEnabled(boolean),
     * and span index caching via a call to
     * {@link SpanSizeLookup#setSpanIndexCacheEnabled(boolean)}}.
     */
    public void setUseSpansToEstimateScrollbarDimensions(
            boolean useSpansToEstimateScrollBarDimentsions) {
        mUseSpansToEstimateScrollBarDimensions = useSpansToEstimateScrollBarDimentsions;
    }

    /**
     * Returns true if the scroll offset and scroll range calculations take account of span
     * information. See {@link #setUseSpansToEstimateScrollbarDimensions(boolean)} for more
     * information on this topic.
     *
     * @return true if the scroll offset and scroll range calculations take account of span
     * information.
     */
    public boolean getUseSpansToEstimateScrollbarDimeinsions() {
        return mUseSpansToEstimateScrollBarDimensions;
    }
    ...
    private int computeScrollRangeWithSpanInfo(RecyclerView.State state) {
        ...
        final int firstVisibleRow = mSpanSizeLookup.getCachedSpanGroupIndex(getPosition(startChild),
                mSpanCount);
        final int lastVisibleRow = mSpanSizeLookup.getCachedSpanGroupIndex(getPosition(endChild),
                mSpanCount);
        final int totalRows = mSpanSizeLookup.getCachedSpanGroupIndex(state.getItemCount() - 1,
                mSpanCount) + 1;
    ...
    private int computeScrollOffsetWithSpanInfo(RecyclerView.State state) {
        ...
        int startChildRow = mSpanSizeLookup.getCachedSpanGroupIndex(getPosition(startChild),
                mSpanCount);
        int endChildRow = mSpanSizeLookup.getCachedSpanGroupIndex(getPosition(endChild),
                mSpanCount);
