...
public final class CallbackToFutureAdapter {
    ...
    public static final class Completer<T> {
        ...
        // synthetic access
        SafeFuture<T> future;
        private List<RunnableWithExecutor> cancellationListeners = new ArrayList<>();
        ...
        private volatile boolean cancellationListenersExecuted;
        ...
        public void addCancellationListener(@NonNull Runnable runnable,
            ...
            if (cancellationListenersExecuted) {
                executor.execute(runnable);
                return;
            }

            List<RunnableWithExecutor> list = cancellationListeners;
            // Null means the future already completed normally; the listener would never be
            // invoked.
            if (list != null) {
                addCancellationListener(list, new RunnableWithExecutor(runnable, executor));
            }
        }
        ...
        void fireCancellationListeners() {
            tag = null;
            future = null;
            List<RunnableWithExecutor> localListeners = cancellationListeners;
            cancellationListeners = null;
            executeCancellationListeners(localListeners);
        }
        ...
        private void addCancellationListener(List<RunnableWithExecutor> listeners,
                RunnableWithExecutor listener) {
            synchronized (listeners) {
                // while we're blocked execution happened
                if (cancellationListenersExecuted) {
        ...
        @SuppressWarnings("SynchronizationOnLocalVariableOrMethodParameter")
        private void executeCancellationListeners(List<RunnableWithExecutor> listeners) {
            cancellationListenersExecuted = true;
            synchronized (listeners) {
                for (RunnableWithExecutor entry : listeners) {
                    entry.executor.execute(entry.runnable);
                }
            }
        }
        ...
        protected void finalize() {
            ...
            if (!attemptedSetting) {
                List<RunnableWithExecutor> list = cancellationListeners;
                if (list != null) {
                    // Execute is idempotent, so even if this was already invoked it won't run
                    // listeners twice
                    executeCancellationListeners(list);
                }
            }
        }
    }

    static final class RunnableWithExecutor {
        final Runnable runnable;
        final Executor executor;

        RunnableWithExecutor(Runnable runnable, Executor executor) {
