...
class JavaNavWriter(
...
) : NavWriter<JavaCodeFile> {

    override fun generateDirectionsCodeFile(
        destination: Destination,
        parentDirectionFile: JavaCodeFile?
    ): JavaCodeFile {
        ...
        val typeSpec = generateDestinationDirectionsTypeSpec(
            className, parentDirectionFile?.toClassName(), destination
        )
    ...
    internal fun generateDirectionsTypeSpec(action: Action): TypeSpec {
        val annotations =
            Annotations.getInstance(useAndroidX)
        val specs = ClassWithArgsSpecs(
            action.args,
            annotations
        )
