...
class JavaNavWriter(private val useAndroidX: Boolean = false) : NavWriter {
    ...
    override fun generateDirectionsCodeFile(
        ...
        val superClassName = parentDestination?.let {
            val name = it.name
                ?: throw IllegalStateException("Parent destination with actions must have name")
            ClassName.get(name.packageName(), "${name.simpleName()}Directions")
        }
