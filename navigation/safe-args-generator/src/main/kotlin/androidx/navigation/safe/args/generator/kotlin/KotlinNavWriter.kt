...
class KotlinNavWriter(
    ...
    override fun generateDirectionsCodeFile(
        ...
        parentDirectionFile: KotlinCodeFile?
    ): KotlinCodeFile {
        ...
        val fromBundleFunSpec = FunSpec.constructorBuilder().apply {
            ...
            val bundleGetCodeBlocks = destination.args.map { arg ->
                CodeBlock.builder().apply {
                    beginControlFlow("%L.run {", bundleParamName)
                    addStatement("setClassLoader(%T::class.java.classLoader)", className)
                    beginControlFlow("if (containsKey(%S))", arg.name)
                    arg.type.addBundleGetStatement(this, arg)
                    ...
                    if (defaultValue != null) {
                        addStatement("%L", arg.defaultValue.write())
                    } else {
                        addStatement(
                            "throw·%T(%S)",
                            IllegalArgumentException::class.asTypeName(),
