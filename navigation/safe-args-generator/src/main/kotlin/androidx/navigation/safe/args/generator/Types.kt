...
sealed class NavType {
    ...
    companion object {
        fun from(name: String?) = when (name) {
            ...
            null -> throw IllegalStateException("Cannot infer type and type unspecified (null)")
            else -> {
                if (name.endsWith("[]")) {
                    name.substringBeforeLast("[]").let { clsName ->
                        ObjectArrayType(ClassName.get(
                                clsName.substringBeforeLast('.', ""),
...
data class ObjectType(private val typeName: TypeName) : NavType() {
    ...
    override fun addBundleGetStatement(
            ...
            arg: Argument,
            lValue: String,
            bundle: String): MethodSpec.Builder {
        return builder.apply {
            beginControlFlow("if ($T.class.isAssignableFrom($T.class) " +
                    ...
                    }/*.nextControlFlow("else if ($T.class.isEnum())", arg.type.typeName()).apply {
                        addStatement("String enumName = $N.getString($S)", bundle, arg.name)
                        beginControlFlow("for ($T enumConstant: $T.class.getEnumConstants())",
                                arg.type.typeName(), arg.type.typeName()).apply {
                            beginControlFlow(
                                    "if ($T.class.cast(enumConstant).name().equals(enumName))",
                                    enumType).apply {
                                addStatement("$N = enumConstant", lValue)
                                addStatement("break")
                            }.endControlFlow()
                        }.endControlFlow()
    ...
    override fun addBundlePutStatement(
            ...
            arg: Argument,
            bundle: String,
            argValue: String): MethodSpec.Builder {
        return builder.apply {
            beginControlFlow("if ($T.class.isAssignableFrom($T.class) || $N == null)",
                ...
                addStatement(
                        "$N.$N($S, $T.class.cast($N).name())",
                        bundle, "putString", arg.name, enumType, argValue
                )
            }*/.nextControlFlow("else").apply {
...
//        private val enumType = ClassName.get("java.lang", "Enum")
    }
}
