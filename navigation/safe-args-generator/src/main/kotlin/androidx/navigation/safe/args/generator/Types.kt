...
data class ObjectType(private val typeName: TypeName) : NavType() {
    ...
    override fun addBundlePutStatement(
        ...
        return builder.apply {
            beginControlFlow("if ($N instanceof $T || $N == null)",
                    argValue, parcelable, argValue).apply {
                addStatement(
                        "$N.$N($S, ($T) $N)",
                        bundle, "putParcelable", arg.name, parcelable, argValue
                )
            }.nextControlFlow("else").apply {
                addStatement(
                        "$N.$N($S, ($T) $N)",
                        bundle, "putSerializable", arg.name, serializable, argValue
                )
            }.endControlFlow()
        }
    }
    ...
    companion object {
        private val parcelable = ClassName.get("android.os", "Parcelable")
        private val serializable = ClassName.get("java.io", "Serializable")
    }
}
