 ...
 *     val viewmodel: NYViewModel by navGraphViewModels(R.navigation.main)
 * }
 * ```
 *
 ...
 * ```
 * class MyFragment : Fragment() {
 *     val viewmodel: MYViewModel by navGraphViewModels(R.navigation.main) { myFactory }
 * }
 * ```
 *
...
inline fun <reified VM : ViewModel> Fragment.navGraphViewModels(
    ...
    val viewModelStore: () -> ViewModelStore = { findNavController().getViewModelStore(navGraphId) }
    return createViewModelLazy(VM::class, viewModelStore, factoryProducer)
}