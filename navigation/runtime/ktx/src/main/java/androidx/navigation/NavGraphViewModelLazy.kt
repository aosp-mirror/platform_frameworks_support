...
import androidx.lifecycle.ViewModelLazyWithStore
 ...
 *     val viewmodel: NYViewModel by navGraphViewModels(Int)
 * }
 * ```
 *
 ...
 * ```
 * class MyFragment : Fragment() {
 *     val viewmodel: MYViewModel by navGraphViewModels(Int) { myFactory }
 * }
 * ```
 *
...
inline fun <reified VM : ViewModel> Fragment.navGraphViewModels(
    ...
    val viewModelStore: () -> ViewModelStore = {
        findNavController(requireView()).getViewModelStore(navGraphId)
    }
    ...
    return ViewModelLazyWithStore(VM::class, viewModelStore, factoryPromise)
}