    ...
    fun testGetViewModelStoreSameGraph() {
        ...
        val graph = provider.navigation(1, startDestination = 1) {
            navigation(1, startDestination = 2) {
                destination(NavDestinationBuilder(provider[TestNavigator::class], 2))
            }
        }
        navController.setGraph(graph, null)
        val viewStore1 = navController.getViewModelStore(graph.id)
        assertThat(viewStore1).isNotNull()

        assertThat(navController.getViewModelStore(graph.id)).isSameAs(viewStore1)
    }
