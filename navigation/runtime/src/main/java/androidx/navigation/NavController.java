    ...
    private void navigate(@NonNull NavDestination node, @Nullable Bundle args,
        ...
        if (newDest != null) {
            ...
            while (addNextParent) {
                ...
                // We can stop adding NavGraphs to the hierarchy once we find the one we originally
                // navigated to, as long as the back stack is not empty,
                if (parent.getId() == node.getId()) {
                    addNextParent = mBackStack.isEmpty();
                }
                parent = parent.getParent();
                addNextParent = addNextParent && parent != null;
            }
