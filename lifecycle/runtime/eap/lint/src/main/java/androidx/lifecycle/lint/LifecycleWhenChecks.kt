...
class LifecycleWhenChecks : Detector(), SourceCodeScanner {
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        valueArguments[0].accept(LifecycleWhenVisitor(context))
    }
...
class LifecycleWhenVisitor(private val context: JavaContext) : AbstractUastVisitor() {
    ...
    override fun visitLambdaExpression(node: ULambdaExpression): Boolean {
        // See HasSuspendingCallsVisitor.visitLambdaExpression for comment
...
class TouchViewVisitor(private val context: JavaContext) : AbstractUastVisitor() {
    override fun visitCallExpression(node: UCallExpression): Boolean {
        ...
        if (psiMethod != null && psiMethod.isLifecycleWhenExtension(context)) {
            // we don't want to go inside, inner whenLifecycleStarted will be checked anyway.
            return true
        }
