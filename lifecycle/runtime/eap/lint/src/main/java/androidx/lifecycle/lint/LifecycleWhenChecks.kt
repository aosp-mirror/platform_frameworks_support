...
internal class LifecycleWhenChecks : Detector(), SourceCodeScanner {
    ...
    override fun getApplicableMethodNames() = listOf("whenStarted")

    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        super.visitMethodCall(context, node, method)
        val valueArguments = node.valueArguments
        if (!method.isLifecycleWhenExtension(context) || valueArguments.size != 1) {
            return
        }
    ...
    companion object {
        val ISSUE = Issue.create(
            ...
            explanation = "If the Lifecycle is destroyed while the block of " +
                    ...
                    "you should check the Lifecycle.getCurrentState before accessing the UI." +
                    " See documentation of Lifecycle.whenStateAtLeast for more details",
...
internal class LifecycleWhenVisitor(private val context: JavaContext) : AbstractUastVisitor() {
    ...
    private var tryState = State.DEFAULT
    private var hasSuspendCall = false

    override fun visitTryExpression(node: UTryExpression): Boolean {
        val prevTryState = tryState
        val prevHasSuspendCall = hasSuspendCall
        hasSuspendCall = false
        tryState = State.IN_TRY
        node.tryClause.accept(this)
        // TODO: support catch
        tryState = if (hasSuspendCall) State.AFTER_SUSPEND_TRY else State.DEFAULT
        node.finallyClause?.accept(this)
        tryState = prevTryState
        hasSuspendCall = (tryState != State.DEFAULT) and (hasSuspendCall or prevHasSuspendCall)
        return true
    }
    ...
    override fun visitCallExpression(node: UCallExpression): Boolean {
        ...
        if (isSuspend) {
            ...
            psiMethod.toUElement()?.accept(this)
        }
        hasSuspendCall = hasSuspendCall or isSuspend

        if (tryState == State.AFTER_SUSPEND_TRY) {
