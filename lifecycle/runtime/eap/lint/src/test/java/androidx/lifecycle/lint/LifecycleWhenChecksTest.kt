...
class LifecycleWhenChecksTest {
    ...
    fun error(
        ...
        val errorMessage = errorMessage("whenStarted")
        return """
            src/foo/test.kt:$l: Error: $errorMessage$additionalMessage [${ISSUE.id}]
                $customExpression
                $highlight
            1 errors, 0 warnings
        """.trimIndent()
    ...
    fun viewAccessInFunction() {
        ...
        check(input).expect(
            error(
                4, "accessView(view)",
                ". \"accessView\" internally access view at test.kt:30:4"
            )
        )
    }
}