...
class PausingDispatcherTest {
    ...
    fun updateMainHandlerAndDispatcher() {
        Dispatchers.setMain(testHandler.asCoroutineDispatcher())
    }
    ...
    fun clenarHandlerAndDispatcher() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            assertThat(testHandler.looper.queue.isIdle).isTrue()
        }
        runBlocking {
    ...
    fun assertThread() {
        ...
        assertThat(Looper.myLooper()).isSameAs(testHandler.looper)
    }
    ...
    private fun Array<CountDownLatch>.assertFalse(seconds: Int = 1) {
        runBlocking {
            map {
                async {
