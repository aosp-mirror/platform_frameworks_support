...
public abstract class ViewModel {
    ...
    final void clear() {
        mCleared = true;
        // invariant mBagOfTags can't go back to null.
        ConcurrentHashMap<String, Object> localBag = mBagOfTags;
        if (localBag != null) {
            for (Object value: localBag.values()) {
        ...
        if (localBag == null) {
            synchronized (LOCK) {
                if (mBagOfTags == null) {
                    ...
                    localBag = new ConcurrentHashMap<>(2);
                    mBagOfTags = localBag;
                }
            }
        }
        @SuppressWarnings("unchecked") T previous = (T) localBag.putIfAbsent(key, newValue);
    ...
    <T> T getTag(String key) {
        ...
        ConcurrentHashMap<String, Object> localBag = mBagOfTags;
        return localBag != null ? (T) localBag.get(key) : null;
    }
