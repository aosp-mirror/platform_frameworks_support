...
suspend fun <T> withStateAtLeast(
    ...
    return withContext(Dispatchers.Main) {
        when {
            ...
            else -> runBlockViaLifecycleObserver(state, lifecycle, block)
        }
    }
}

/**
 * Helper method for [withStateAtLeast] that runs the block using a Lifecycle observer and maintains
 * the observer lifecycle.
 */
private suspend fun <T> runBlockViaLifecycleObserver(
    state: Lifecycle.State,
    lifecycle: Lifecycle,
    block: () -> T
): T {
