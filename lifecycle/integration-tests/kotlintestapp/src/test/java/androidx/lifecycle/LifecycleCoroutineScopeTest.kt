...
class LifecycleCoroutineScopeTest {
    ...
    fun initialization() {
        ...
        assertThat(owner.lifecycle.mInternalScope).isSameAs(scope)
        val scope2 = owner.scope
    ...
    fun destroyWhileRunning() {
        ...
        val owner = FakeLifecycleOwner(Lifecycle.State.STARTED)
        val actionWasActive = owner.scope.async(Dispatchers.IO) {
            startMutex.unlock()
