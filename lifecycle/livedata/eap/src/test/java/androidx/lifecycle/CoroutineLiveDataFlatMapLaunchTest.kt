...
class CoroutineLiveDataFlatMapLaunchTest : CoroutineTestBase() {
    ...
    fun simple() {
        runTest {
            val ld = src.flatMap(testScope.coroutineContext) {
                yield("$it")
                yield("$it-$it")
            }
    ...
    fun throwException_thenSucceed() {
        runTest {
            ...
            val ld =
                src.flatMap(testScope.coroutineContext + exceptionHandler) {
                    if (shouldThrow) {
                        shouldThrow = false
            ...
            src.value = 2
            assertThat(items.await()).containsExactly("hello 2")
        }
    }
    ...
    fun multiLevel() {
        runTest {
            val liveData = src.flatMap(testScope.coroutineContext) {
                yield(it * it)
            }.flatMap(Dispatchers.IO) {
                yield("$it-$it")
            }
