...
class BuildLiveDataTest : CoroutineTestBase() {
    ...
    fun ignoreCancelledYields() {
        runTest {
            ...
            val ld = liveData(timeoutInMs = 0, context = Dispatchers.IO) {
                yield(1)
    ...
    fun readInitialValue() {
        runTest {
            val initial = CompletableDeferred<Int?>()
            val ld = liveData<Int>(Dispatchers.IO) {
                initial.complete(initialValue)
            }
            ld.value = 3
            ld.observeForever { }
            assertThat(initial.await()).isEqualTo(3)
        }
    }
    ...
    fun readInitialValue_ignoreYielded() {
        runTest {
            ...
            ld.observeForever { }
            assertThat(initial.await()).isNull()
        }
    }
    ...
    fun readInitialValue_keepYieldedFromBefore() {
        runTest {
            val initial = CompletableDeferred<Int?>()
            val ld = liveData<Int>(Dispatchers.IO, 10) {
                if (initialValue == null) {
                    yield(5)
                    delay(500000) // wait for cancelation
                }

                initial.complete(initialValue)
            }
            assertThat(ld.collect(1)).containsExactly(5)
            delay(100) // wait for it to be cancelled
            assertThat(initial.isActive).isTrue()
            // now add an observer to re-run but wait for timeout first
            ld.observeForever {}
            assertThat(initial.await()).isEqualTo(5)
        }
    }
    ...
    fun blockThrows() {
        ...
        val ld = liveData(Dispatchers.IO + exceptionHandler, 10) {
