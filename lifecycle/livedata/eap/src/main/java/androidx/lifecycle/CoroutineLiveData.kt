...
interface LiveDataScope<T> {
    ...
    /**
     * Returns the current value of the [LiveData]. This is either the last value passed into
     * [yield] or the last value of the [LiveData] that was plugged via [yieldSource]; whichever
     * `yield` was called the last.
     *
     * Checking this value might be useful if you want to handle block cancellation gracefully and
     * continue where you've left.
     *
     * @return The current [LiveData.getValue] or `null` if a value has never been yielded before.
     */
...
internal class LiveDataScopeImpl<T>(
    ...
    // use `buildLiveData` provided context + main dispatcher to communicate with the target
    ...
    override suspend fun currentValue() = withContext(coroutineContext) {
        target.value
    }
