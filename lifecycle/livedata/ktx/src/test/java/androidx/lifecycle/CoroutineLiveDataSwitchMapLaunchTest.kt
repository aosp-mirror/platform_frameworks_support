...
class CoroutineLiveDataSwitchMapLaunchTest {
    ...
    fun simple() {
        runTest {
            val ld = src.switchMapLaunch(testScope.coroutineContext) {
                yield("$it")
                yield("$it-$it")
            }
    ...
    fun throwException_thenSucceed() {
        runTest {
            ...
            val ld =
                src.switchMapLaunch(testScope.coroutineContext + exceptionHandler) {
                    if (shouldThrow) {
                        shouldThrow = false
    ...
    fun multiLevel() {
        runTest {
            val liveData = src.switchMapLaunch(testScope.coroutineContext) {
                yield(it * it)
            }.switchMapLaunch(Dispatchers.IO) {
                yield("$it-$it")
            }
