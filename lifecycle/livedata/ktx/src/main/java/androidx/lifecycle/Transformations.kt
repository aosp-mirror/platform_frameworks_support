 ...
 * ```
 */
@MainThread inline fun <X, Y> LiveData<X>.map(crossinline transform: (X) -> Y): LiveData<Y> =
 ...
 * ```
 */
@MainThread inline fun <X, Y> LiveData<X>.switchMap(
...
@MainThread inline fun <X> LiveData<X>.distinctUntilChanged(): LiveData<X> =
