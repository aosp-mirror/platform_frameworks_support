 ...
 * <p>
 * Implementations simply need to call {@link #performRestore(Map)} to initialize restored state and
 * call {@link #performSave()} once system requests saved state.
 *
...
public abstract class AbstractSavedStateRegistry<S> implements SavedStateRegistry<S> {
    ...
    @MainThread
    protected final void performRestore(@Nullable Map<String, S> initialState) {
    ...
    @MainThread
    @NonNull
    protected final Map<String, S> performSave() {
