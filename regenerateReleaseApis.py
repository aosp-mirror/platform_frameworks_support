...
def findMissingApiFilePaths():
  ...
  missingApiFilePaths = []
  output = runShellAndGetOutput("./gradlew checkApi --continue")
  lines = output.split("\n")
  for line in lines:
...
def regenerateOldApiFilePathsInParallel(apiFilePaths):
  ...
  for desiredApiFilePath in apiFilePaths:
    ...
    if os.path.isfile(createdApiFilePath):
...
def updateOldApiFilesRepeatedly():
  ...
  while True:
    ...
    if len(missingApiFilePaths) < 1:
      print("No api file paths missing")
      return
    if even:
      # in most cases we want to regenerate the api files in parallel because it's faster
      regenerateOldApiFilePathsInParallel(missingApiFilePaths)
    else:
      # in some cases we want to regenerate the api files in series because there might be a build failure when building one of them
