...
public final class WorkManagerSync {
    ...
    private static WorkManagerSync sDefaultInstance = null;
    private static final Object sLock = new Object();

    private WorkManagerSync(@NonNull WorkManagerImpl workManager) {
     ...
     * @throws IllegalStateException when WorkManager is not initialized properly.  This is most
        ...
        if (workManager == null) {
            ...
            synchronized (sLock) {
                if (sDefaultInstance == null) {
                    sDefaultInstance = new WorkManagerSync(workManager);
                }
     ...
     * @throws InterruptedException when the thread enqueueing the {@link WorkRequest}s is
     *                              interrupted.
     * @throws ExecutionException   when there is an error executing the request to enqueue
     *                              {@link WorkRequest}s.
     */
    @WorkerThread
    public void enqueue(@NonNull WorkRequest... workRequests)
            throws InterruptedException, ExecutionException {
        mWorkManager.enqueue(workRequests).get();
    }
    ...
    public void enqueue(@NonNull List<? extends WorkRequest> requests)
            throws InterruptedException, ExecutionException {
        mWorkManager.enqueue(requests).get();
    }
     ...
     *                           {@code uniqueWorkName}.
     * @throws InterruptedException when the thread enqueueing the {@link OneTimeWorkRequest}s is
     *                              interrupted.
     * @throws ExecutionException   when there is an error executing the request to enqueue
    ...
    public void enqueueUniqueWork(
            ...
            throws InterruptedException, ExecutionException {
        mWorkManager.enqueueUniqueWork(uniqueWorkName, existingWorkPolicy, Arrays.asList(work))
                .get();
    }
    ...
    public void enqueueUniqueWork(
            ...
            throws InterruptedException, ExecutionException {
        mWorkManager.enqueueUniqueWork(uniqueWorkName, existingWorkPolicy, work).get();
    }
     ...
     * @throws InterruptedException when the thread enqueueing the {@link PeriodicWorkRequest} is
     *                              interrupted.
     * @throws ExecutionException   when there is an error executing the request to enqueue
     ...
     * @throws InterruptedException when the thread cancelling work by id is interrupted.
     * @throws ExecutionException   when there is an error executing the request to cancel work
     *                              by id.
     */
    @WorkerThread
    public void cancelWorkById(@NonNull UUID id)
            throws InterruptedException, ExecutionException {
        mWorkManager.cancelWorkById(id).get();
    }
     ...
     * @throws InterruptedException when the thread cancelling work by tag is interrupted.
     * @throws ExecutionException   when there is an error executing the request to cancel work
     *                              by tag.
     */
    @WorkerThread
    public void cancelAllWorkByTag(@NonNull String tag)
            throws InterruptedException, ExecutionException {
        mWorkManager.cancelAllWorkByTag(tag).get();
    }
     ...
     * @throws InterruptedException when the thread cancelling unique work is interrupted.
     * @throws ExecutionException   when there is an error executing the request to cancel unique
     *                              work.
     */
    @WorkerThread
    public void cancelUniqueWork(@NonNull String uniqueWorkName)
            throws InterruptedException, ExecutionException {
        mWorkManager.cancelUniqueWork(uniqueWorkName).get();
    }
     ...
     * @throws InterruptedException when the thread cancelling all work is interrupted.
     * @throws ExecutionException   when there is an error executing the request to cancel all
     *                              work.
     */
    @WorkerThread
    public void cancelAllWork() throws InterruptedException, ExecutionException {
        mWorkManager.cancelAllWork().get();
    }
     ...
     *
     * @throws InterruptedException when the thread pruning all work is interrupted.
     * @throws ExecutionException   when there is an error executing the request to prune all
     *                              work.
     */
    @WorkerThread
    public void pruneWork() throws InterruptedException, ExecutionException {
        mWorkManager.pruneWork().get();
    }
}
