 ...
 * {@link WorkManager} and the {@link #onStartWork()} method is called on the main thread. In case
 * the work is preempted for any reason, the same instance of Worker is not reused.  This means
 * that {@link #onStartWork()} is called exactly once per Worker instance.
 *
 * The {@link NonBlockingWorker} signals work completion by calling {@code
 * {@link #onWorkFinished(Result)}}.
 */
public abstract class NonBlockingWorker {
     ...
     * completion via the completion callback {@link #onWorkFinished(Result)}.
     */
    @WorkerThread
     ...
     * <p>
     * This method is invoked after {@link #onStartWork()} returns {@link Result#SUCCESS} or
     * {@link Result#FAILURE}.
     * <p>
     ...
     */
    @WorkerThread
    protected void onWorkFinished(@NonNull Result result) {
        setResult(result);
        Extras.RuntimeExtras runtimeExtras = mExtras.getRuntimeExtras();
