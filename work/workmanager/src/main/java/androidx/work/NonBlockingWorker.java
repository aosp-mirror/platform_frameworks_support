 ...
 * the background thread. In case the work is preempted for any reason, the same instance of Worker
 * is not reused.  This means that {@link #onStartWork(WorkFinishedCallback)}
 * is called exactly once per Worker instance.
 *
 * The {@link NonBlockingWorker} signals work completion by using {@link WorkFinishedCallback}.
 *
 * @hide
 */
...
public abstract class NonBlockingWorker implements WorkFinishedCallback {
    ...
    private @NonNull volatile Result mResult = Result.FAILURE;

    /**
     * The result of the Worker's computation that is returned in the
     * {@link #onWorkFinished(Result)} method.
     */
    public enum Result {
        /**
     ...
     * completion via the completion callback {@link WorkFinishedCallback}.
     *
     * @param callback The {@link WorkFinishedCallback} that helps signal work completion.
     */
    @WorkerThread
     ...
     * returns {@link Result#SUCCESS} or {@link Result#FAILURE}.
     * <p>
    ...
    /**
     * @return the {@link Result} of executing the {@link Worker}.
     * @hide
     */
    @NonNull
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    ...
    /**
     * Sets the {@link Result} of the {@link Worker}s execution.
     * @hide
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
     ...
     *
     * @param result the {@link Result} of the {@link NonBlockingWorker}'s execution.
     * @hide
     */
    @WorkerThread
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    public void onWorkFinished(@NonNull Result result) {
        ...
        if (runtimeExtras.mExecutionListener != null) {
            ...
            boolean needsReschedule = result == Result.RETRY;
