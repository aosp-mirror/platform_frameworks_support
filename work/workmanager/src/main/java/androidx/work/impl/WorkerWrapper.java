...
public class WorkerWrapper implements Runnable {
    ...
    private void runWorker() {
        ...
        // in test mode.
        if (mWorker == null) {
            Class<? extends NonBlockingWorker> className;
            try {
                className = Class.forName(mWorkSpec.workerClassName)
                        .asSubclass(NonBlockingWorker.class);
            } catch (ClassNotFoundException e) {
                Logger.error(TAG, "Class not found: " + mWorkSpec.workerClassName);
                setFailedAndNotify();
                return;
            }
