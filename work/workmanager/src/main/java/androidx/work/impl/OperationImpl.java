...
public class OperationImpl implements Operation {
    ...
    private final SettableFuture<State> mOperationFuture;

    public OperationImpl() {
        mOperationState = new MutableLiveData<>();
        mOperationFuture = SettableFuture.create();
        // Mark the operation as in progress.
        setState(State.InProgress.INSTANCE);
    }

    @Override
    public ListenableFuture<State> getResult() {
    ...
    public void setState(@NonNull State state) {
        ...
        if (state instanceof State.Successful) {
            mOperationFuture.set(state);
