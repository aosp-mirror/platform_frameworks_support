    ...
    public void run() {
        if (shouldRescheduleWorkers()) {
            ...
            try {
                ...
                if (workSpecIds != null && !workSpecIds.isEmpty()) {
                    Logger.get().debug(TAG, "Found unfinished work, rescheduling.");
                    ...
                    for (String id : workSpecIds) {
                        workSpecDao.markWorkSpecScheduled(id, SCHEDULE_NOT_REQUESTED_YET);
                    }
                    Schedulers.schedule(
