...
public class Schedulers {
    ...
    private static final String GCM_TASK_SERVICE =
    ...
    @Nullable
    private static Scheduler tryGcmBasedScheduler(@NonNull Context context) {
        try {
            Class<?> klass = Class.forName(GCM_SCHEDULER);
            Logger.get().debug(TAG, String.format("Created %s", GCM_SCHEDULER));
            return (Scheduler) klass.getConstructor(Context.class).newInstance(context);
