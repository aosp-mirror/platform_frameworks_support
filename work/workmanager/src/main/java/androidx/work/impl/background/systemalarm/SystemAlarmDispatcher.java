...
public class SystemAlarmDispatcher implements ExecutionListener {
    ...
    public boolean add(@NonNull final Intent intent, final int startId) {
        ...
        synchronized (mIntents) {
            ...
            if (mCurrentIntent == null) {
                // Only call processCommand if we are not already processing commands.
                // The processor drives itself to completion.
                processCommand();
            }
        }
        return true;
    ...
    void dequeueAndCheckForCompletion() {
        ...
        synchronized (mIntents) {
            ...
            if (mCurrentIntent != null) {
                Logger.debug(TAG, String.format("Dequeue-ing command %s", mCurrentIntent));
        ...
        synchronized (mIntents) {
            for (Intent intent : mIntents) {
                if (action.equals(intent.getAction())) {
                    Logger.debug(TAG,
                            String.format("%s : has intent with action => %s", action, true));
                    return true;
                }
            }
            Logger.debug(TAG,
                    String.format("%s : has intent with action => %s", action, false));
            return false;
        }
    }
