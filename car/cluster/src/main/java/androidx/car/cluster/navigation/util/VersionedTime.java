 ...
 */
@VersionedParcelize
public class VersionedTime implements VersionedParcelable {
    @ParcelField(1)
    ...
    /**
     * Creates a {@link VersionedTime} that wraps the given {@link ZonedDateTime}
     */
    public VersionedTime(@NonNull ZonedDateTime time) {
        mSecondsSinceEpoch = time.toEpochSecond();
        mZoneId = time.getZone().getId();
    }

    /**
     * Creates a {@link VersionedTime} based on the provided {@link Instant} and a timezone
     * identified by the provided {@link ZoneId}.
     */
    public VersionedTime(@NonNull Instant time, @Nullable ZoneId zoneId) {
        mSecondsSinceEpoch = time.getEpochSecond();
        mZoneId = zoneId != null ? zoneId.getId() : null;
    }

    /**
     * Creates a {@link VersionedTime} based on the provided {@link Instant} but without timezone
     * information.
     */
    public VersionedTime(@NonNull Instant time) {
        this(time, null);
    }
