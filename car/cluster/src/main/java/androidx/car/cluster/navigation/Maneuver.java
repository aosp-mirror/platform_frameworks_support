...
public final class Maneuver implements VersionedParcelable {
    /**
     * Value used in case a maneuver was not set.
     */
    public static Maneuver NOT_SET = new Maneuver();
    ...
    public enum Type {
        /**
         * Maneuver was not set, or its type is unknown to the consumer, in which case the consumer
     ...
     */
    public Maneuver(@NonNull Type type, @NonNull Type ... fallbackTypes) {
        mType = EnumWrapper.of(type, fallbackTypes);
    }

    /**
     * Sets the roundabout exit number, starting from 1 to designate the first exit after joining
    ...
    /**
     * Returns the maneuver type, or {@link Type#NO_MANEUVER} if the maneuver type received is
     * unknown to this consumer.
     */
    @NonNull
    public Type getType() {
        return EnumWrapper.getValue(mType, Type.NO_MANEUVER);
    }
