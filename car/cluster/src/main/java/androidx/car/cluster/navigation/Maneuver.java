...
public final class Maneuver implements VersionedParcelable {
    ...
    public enum Type {
        /**
         * Maneuver type is unknown to the consumer.
         */
        UNKNOWN,
        /**
        ...
        STRAIGHT,
        /**
         * Road finishes in a boat ferry for vehicles (e.g. "Take the ferry").
         */
        FERRY_BOAT,
        /**
         * Road finishes in a train ferry for vehicles (e.g. "Take the train").
         */
        FERRY_TRAIN,
        /**
    ...
    @ParcelField(1)
    EnumWrapper<Type> mType = EnumWrapper.of(Type.UNKNOWN);
    @ParcelField(2)
    ...
    /**
     * Creates a {@link Maneuver} based on the provided data.
     *
     * @param type Main maneuver type
     * @param roundaboutExitNumber Roundabout exit number, starting from 1 to designate the first
     *                             exit after joining the roundabout, and increasing in circulation
     *                             order. Only relevant if {@code type} is
     *                             {@link Type#ROUNDABOUT_EXIT} or any variation of
     *                             ROUNDABOUT_ENTER_AND_EXIT. See {@link #getRoundaboutExitNumber()}
     * @param fallbackTypes Variations of {@code type}, in case the consumer of this API doesn't
     *                      know the main one (used for backward compatibility). For example, if the
     *                      main type is {@link Type#OFF_RAMP_NORMAL_LEFT}, a fallback type could be
     *                      {@link Type#TURN_NORMAL_LEFT}.
     */
    public Maneuver(@NonNull Type type, int roundaboutExitNumber,
            @NonNull Type ... fallbackTypes) {
        mType = EnumWrapper.of(type, fallbackTypes);
        mRoundaboutExitNumber = roundaboutExitNumber;
    }

    /**
     * Type of maneuver, or null if the type of maneuver is unknown to the consumer.
     */
    @NonNull
    public Type getType() {
        return mType.getValue(Type.UNKNOWN);
    }

    /**
     * Roundabout exit number, starting from 1 to designate the first exit after joining the
     * roundabout, and increasing in circulation order. Only relevant if {@link #getType()} is
    ...
    public boolean equals(Object o) {
        ...
        return mRoundaboutExitNumber == maneuver.mRoundaboutExitNumber
                && Objects.equals(mType, maneuver.mType);
    }
