...
public class CarToolbar extends ViewGroup {
     ...
     */
    private void layoutViewFromLeftVerticallyCentered(View view, int left, int height) {
        int vh = view.getMeasuredHeight();
        int top = (height - vh) / 2;

        view.layout(left, top, left + view.getMeasuredWidth(), top + vh);
    }
     ...
     * @param height Height of the parent view.
     */
    private void layoutViewFromRightVerticallyCentered(View view, int right, int height) {
        int vh = view.getMeasuredHeight();
        int top = (height - vh) / 2;

        view.layout(right - view.getMeasuredWidth(), top, right, top + vh);
    }
