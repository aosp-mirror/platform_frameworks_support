...
public class CarToolbar extends ViewGroup {
    ...
    public void showOverflowMenu() {
        ...
        if (mOverflowDialog == null) {
            throw new IllegalStateException("showOverflowMenu was called with no overflow items.");
        }
        mOverflowDialog.show();
    }
    ...
    private void populateOverflowMenu() {
        if (mOverflowMenuItems == null) {
            return;
        }
        mOverflowDialog = new CarListDialog.Builder(getContext())
            .setItems(mOverflowMenuItems.stream()
                        .map(CarMenuItem::getTitle)
                        .toArray(CharSequence[]::new), mOverflowDialogClickListener)
            .create();
    }
