...
public class CarUxRestrictions implements Parcelable {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(mIsDistractionOptimizationRequired ? 1 : 0);
    ...
    CarUxRestrictions(Parcel in) {
        mActiveRestrictions = in.readInt();
        mTimeStamp = in.readLong();
        mIsDistractionOptimizationRequired = in.readInt() != 0;
     ...
     */
    public boolean isSame(CarUxRestrictions other) {
        if (other == null) {
            return false;
        }
        if (other == this) {
