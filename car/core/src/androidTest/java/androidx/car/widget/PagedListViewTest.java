...
public final class PagedListViewTest {
    ...
    /** Sets up {@link #mPagedListView} with the given number of items with alpha jump button. */
    private void setUpPagedListViewWithAlphaJump() {
        try {
            mActivityRule.runOnUiThread(() -> {
                mPagedListView.setMaxPages(PagedListView.ItemCap.UNLIMITED);
                mPagedListView.setAdapter(new CheeseAdapter());
            });
        } catch (Throwable throwable) {
            throwable.printStackTrace();
            throw new RuntimeException(throwable);
    ...
    public void testScrollBarViewNoShowButtonWhenLimitedSpace() {
        ...
        View mScrollBarView = mPagedListView.findViewById(R.id.paged_scroll_view);
        ImageView upButton = mPagedListView.findViewById(R.id.page_up);
        ViewGroup.LayoutParams params = mScrollBarView.getLayoutParams();
        params.height = 2 * upButton.getHeight() + mPagedListView.getPaddingTop()
                + res.getDimensionPixelSize(
                androidx.car.R.dimen.car_padding_4) - 1;
        try {
            mActivityRule.runOnUiThread(() -> {
    ...
    public void testScrollBarViewNoShowThumbWhenLimitedSpace() {
        ...
        ViewGroup.LayoutParams params = mScrollBarView.getLayoutParams();
        params.height = upButton.getHeight() * 2 + res.getDimensionPixelSize(
                androidx.car.R.dimen.car_padding_4) + scrollThumb.getMeasuredHeight() - 1;
        try {
            mActivityRule.runOnUiThread(() -> {
                mScrollBarView.setLayoutParams(params);
            });
    ...
    public void testScrollBarViewNoShowAlphaWhenLimitedSpace() {
        ...
        Resources res = mPagedListView.getContext().getResources();
        View mScrollBarView = mPagedListView.findViewById(R.id.paged_scroll_view);
        ImageView upButton = mScrollBarView.findViewById(R.id.page_up);
        Button alphaJump = mScrollBarView.findViewById(R.id.alpha_jump);
        View scrollThumb = mScrollBarView.findViewById(R.id.scrollbar_thumb);
        ViewGroup.LayoutParams params = mScrollBarView.getLayoutParams();
        params.height = upButton.getHeight() * 2 + 3 * res.getDimensionPixelSize(
                androidx.car.R.dimen.car_padding_4) + scrollThumb.getMeasuredHeight()
                + alphaJump.getMeasuredHeight() - 1;
        try {
            mActivityRule.runOnUiThread(() -> {
                mScrollBarView.setLayoutParams(params);
            });
    ...
    public static class CheeseAdapter extends RecyclerView.Adapter<CheeseAdapter.ViewHolder>
        ...
        public final String[] CHEESES = {
                ...
                "Saint-Marcellin", "Pave d'Affinois", "Quatre-Vents", "Galette du Paludier",
                "Pyramide",
                ...
                "Briquette de Brebis", "Niolo", "Selva", "Dunbarra",
                ...
                "Mahoe Aged Gouda", "Cornish Pepper", "Greuilh", "Ricotta (Australian)",
                "Grand Vatel",
                "Prince-Jean", "Coulommiers", "Scamorza", "Romans Part Dieu", "Quark", "Frinault",
                "Chabichou du Poitou", "Le Lacandou", "Maredsous", "Fin-de-Siecle",
                "Button (Innes)",
                "Washed Rind Cheese (Australian)", "Daralagjazsky", "Margotin",
                "Pithtviers au Foin",
