...
public class CarTabBarViewTest {
    ...
    public void setUp() {
        ...
        mTabBarView = mActivity.findViewById(R.id.car_tab_bar);
    ...
    public void testMultipleTabs_addsTabsAsChildren() throws Throwable {
        ...
        for (int i = 0; i < numOfTabs; i++) {
            Icon icon = Icon.createWithResource(mActivity, android.R.drawable.sym_def_app_icon);
    ...
    public void testTabSelection_manuallySetSelectedTab_outOfBounds() throws Throwable {
        ...
        mActivityRule.runOnUiThread(() -> {
            mTabBarView.setSelectedTab(numOfTabs + 1);
            mTabBarView.setTabs(tabs);
        });
    ...
    private void assertTabIsDisplayedWithAlpha(int position, float alpha) {
        onView(allOf(withId(R.id.car_tab), withTagKey(R.id.car_tab_position, is(position)),
                withAlpha(alpha))).check(matches(isDisplayed()));
    }
