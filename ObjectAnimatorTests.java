...
public class ObjectAnimatorTests {
    ...
    public void testOfFloat_Path() throws Throwable {
        ...
        for (int i = 0; i < 200; i++) {
            ...
            if (fraction <= firstSegEndFraction) {
                float x = (float) (LINE1_START + fraction / firstSegEndFraction * (LINE1_END - LINE1_START));
                assertEquals(x, target.positionX, EPSILON);
                assertEquals(LINE1_Y, target.positionY, EPSILON);
            } else {
                float x = (float) (LINE2_START + (fraction - firstSegEndFraction) / (1 - firstSegEndFraction) * (LINE2_END - LINE2_START));
                assertEquals(x, target.positionX, EPSILON);
