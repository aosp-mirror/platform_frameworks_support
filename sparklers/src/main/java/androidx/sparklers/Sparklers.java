...
public class Sparklers {
    ...
    public static Spark loadSpark(Context context, LoadPolicy policy) throws SparkException {
        ...
        try {
            ...
            byte[] sha256 = getSha256(sig.toByteArray());
            if (sha256 == null) {
                throw new Exception("No valid signature");
            }
            if (!Arrays.equals(policy.getReleaseSha256(), sha256)) {
                if (Build.TYPE.equals("user") || !Arrays.equals(policy.getDebugSha256(), sha256)) {
                    Log.e(TAG, "No signature match\nAPK: " + bytesToHex(sha256)
                            + "\nRelease: " + bytesToHex(policy.getDebugSha256())
                            + "\nDebug: " + bytesToHex(policy.getReleaseSha256()));
                    throw new Exception("Incorrect signature " + bytesToHex(sha256));
                }
            }
    ...
    private static int compareVersions(String local, String loadable) throws SparkException {
        if (local == null) {
            Log.d(TAG, "No local implementation available");
            return 1;
        }
        int[] f = parseVersion(local);
        int[] s = parseVersion(loadable);
        if (f.length == 0) {
            throw new SparkException("Invalid local version " + local);
        }
        if (s.length == 0) {
            Log.w(TAG, "Invalid loadable version " + loadable);
            return -1;
        }
        if (f[0] != s[0]) {
