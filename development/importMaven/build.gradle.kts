...
import Build_gradle.MavenListener
...
fun transformInternalPomFile(file: File): File {
    ...
    val contents = String(output.toByteArray())
...
fun copyArtifact(artifact: ResolvedArtifact, internal: Boolean = false) {
    ...
    for (supportingArtifact in supportingArtifacts) {
        ...
        if (file.name.endsWith(".pom")) {
            val license = if (!internal) licenseFor(file) else null
            if (license != null) {
                println("Copying License files for ${file.name} to $location")
                copy {
                    from(license)
                    into(location)
            ...
            val transformed = if (internal) transformInternalPomFile(file) else file
            println("Copying $supportingArtifact to $location (Transformed)")
