...
while read -r line; do
	...
	tagName="androidx_${groupIdWithoutAndroidx}_${artifactId}_${version}"
	releaseNotesURL="$(getReleaseNotesURL "$groupIdWithoutAndroidx" "$version")"
	echo "Adding tag $tagName to SHA: $sha..."
	addGitTag "$tagName" "$sha" "Release androidx.$groupIdWithoutAndroidx:$artifactId:$version, released on $date. Release notes for this artifact can be found here: $releaseNotesURL"
done < "$commit_list"
