...
class Job(object):
  ...
  def run(self):
    ...
    self.full_resetTo_state.withConflictsFrom(testState, True).apply(self.workPath)
      ...
      if len(activeJobs) > 0:
        ...
        if didAcceptState:
          ...
          if acceptedState.size() < maxRelevantSize:
            print("Queuing a retest of response from job " + str(identifier) + " because a much larger job of size " + str(maxRunningSize) + " is still running")
