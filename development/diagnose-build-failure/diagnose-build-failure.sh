...
tempDir="$checkoutRoot/build-failure-diagnosis-temp"
...
echo
echoAndDo "$supportRoot/development/file-utils/diff-filterer.py --assume-no-side-effects $tempDir/clean $tempDir/prev \"$scriptPath/restore-state.sh . && cd $supportRoot && ./gradlew --no-daemon $gradleArgs\""
