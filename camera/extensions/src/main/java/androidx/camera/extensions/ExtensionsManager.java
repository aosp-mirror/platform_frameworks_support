...
public final class ExtensionsManager {
    ...
    private static final EffectEnabler EFFECT_ENABLER = new AdaptingEffectEnabler();
    ...
    public static void enableExtension(@NonNull EffectMode effectMode,
        ...
        synchronized (EFFECT_LOCK) {
            EffectMode originalEffectMode = LIFECYCLE_OWNER_EFFECT_MODE_MAP.get(lifecycleOwner);

            if (originalEffectMode == null) {
                originalEffectMode = EffectMode.NORMAL;
            }

            CameraX.setEffectEnabler(EFFECT_ENABLER);
            LIFECYCLE_OWNER_EFFECT_MODE_MAP.put(lifecycleOwner, effectMode);

            if (effectMode != originalEffectMode) {
                CameraX.refreshUseCases(lifecycleOwner);
            }
        }
    }
