...
package androidx.camera.antelope.cameracontrollers
...
import androidx.camera.antelope.CameraParams
...
import androidx.camera.antelope.CameraXImageAvailableListener
import androidx.camera.antelope.CustomLifecycle
import androidx.camera.antelope.FocusMode
import androidx.camera.antelope.MainActivity
import androidx.camera.antelope.MainActivity.Companion.logd
import androidx.camera.antelope.PrefHelper
import androidx.camera.antelope.TestConfig
import androidx.camera.antelope.TestType
...
internal fun cameraXOpenCamera(
    ...
    try {
        ...
        val previewUseCase = ViewFinderUseCase(params.cameraXPreviewConfig)

        // Set preview to observe the surface texture
        activity.runOnUiThread {
            previewUseCase.setOnViewFinderOutputUpdateListener {
                viewFinderOutput: ViewFinderUseCase.ViewFinderOutput? ->
        ...
        when (testConfig.currentRunningTest) {
            ...
            TestType.MULTI_SWITCH -> {
                ...
                params.cameraXImageCaptureUseCase = ImageCaptureUseCase(params.cameraXCaptureConfig)
...
private fun cameraXPreviewUseCaseBuilder(
...
): ViewFinderUseCaseConfiguration {
    ...
    val configBuilder = ViewFinderUseCaseConfiguration.Builder()
        .setLensFacing(cameraXcameraID)
    Camera2Configuration.Extender(configBuilder)
...
): ImageCaptureUseCaseConfiguration {

    // TODO: As of 0.3.0 CameraX can only use front and back cameras. Update in future versions
    val cameraXcameraID = if (id.equals("0")) CameraX.LensFacing.BACK else CameraX.LensFacing.FRONT

    val configBuilder = ImageCaptureUseCaseConfiguration.Builder()
        .setLensFacing(cameraXcameraID)
