...
package androidx.camera.antelope
...
import androidx.camera.antelope.MainActivity.Companion.FIXED_FOCUS_DISTANCE
import androidx.camera.antelope.MainActivity.Companion.INVALID_FOCAL_LENGTH
import androidx.camera.antelope.MainActivity.Companion.NO_APERTURE
...
import androidx.camera.antelope.cameracontrollers.Camera2CaptureSessionCallback
import androidx.camera.antelope.cameracontrollers.Camera2DeviceStateCallback
import androidx.camera.antelope.cameracontrollers.CameraState
import androidx.camera.antelope.cameracontrollers.CameraXDeviceStateCallback
import androidx.camera.antelope.cameracontrollers.CameraXPreviewSessionStateCallback
import androidx.camera.antelope.cameracontrollers.CameraXCaptureSessionCallback
...
class CameraParams {
    ...
    internal var cameraXPreviewConfig: ViewFinderUseCaseConfiguration =
        ViewFinderUseCaseConfiguration.Builder().build()
    internal var cameraXCaptureConfig: ImageCaptureUseCaseConfiguration =
        ImageCaptureUseCaseConfiguration.Builder().build()

    // Custom lifecycle for CameraX API
    internal var cameraXLifecycle: CustomLifecycle = CustomLifecycle()
    internal var cameraXImageCaptureUseCase: ImageCaptureUseCase =
        ImageCaptureUseCase(ImageCaptureUseCaseConfiguration.Builder().build())
