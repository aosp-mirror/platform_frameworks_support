...
public final class BasicUITest {
    ...
    private void checkPreviewReady() {
        waitFor(new ElapsedTimeIdlingResource(IDLE_TIMEOUT_MS));
        if (!mActivityRule.getActivity().mViewIdlingResource.isIdleNow()) {
            CoreAppTestUtil.assumeCompatibleDevice("There is no any Preview update.");
        }
    ...
    private void checkAnalysisReady() {
        waitFor(new ElapsedTimeIdlingResource(IDLE_TIMEOUT_MS));
        if (!mActivityRule.getActivity().mAnalysisIdlingResource.isIdleNow()) {
            CoreAppTestUtil.assumeCompatibleDevice("There is no any Analysis update");
        }
    }
