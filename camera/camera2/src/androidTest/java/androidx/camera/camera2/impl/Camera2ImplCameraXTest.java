    ...
    public void lifecycleResume_opensCameraAndStreamsFrames() {
        ...
        mMainThreadHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                // Verify result in the same thread.
            }
        });
...
//        Collection<Invocation>  invokeCollestions =
//        Mockito.mockingDetails(mockObserver).getInvocations();
//
//        // Stay in idle state for a while.
//        Thread.sleep(5000);
//        verify(mockObserver, timeout(1000).times(1)).onChanged(any());
//        Collection<Invocation>  invokeCollestions2 =
//        assertThat(invokeCollestions.size()).isEqualTo(invokeCollestions2.size());
//        // Additional frames should not be observed.
////        final Long secondObservedCount = observedCount.get();
////        assertThat(secondObservedCount).isEqualTo(firstObservedCount);
...
//        verify(mMockStateCallback, never()).onClosed(any(CameraDevice.class));
        verify(mMockStateCallback, timeout(3000).times(0)).onClosed(any(CameraDevice.class));
    }
