...
public final class Camera2CameraControl implements CameraControl {
    ...
    void notifyCaptureRequest(final List<CaptureConfig> captureConfigs) {
        if (Looper.myLooper() != mHandler.getLooper()) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Camera2CameraControl.this.notifyCaptureRequest(captureConfigs);
                }
            });
            return;
        }
        mControlUpdateListener.onCameraControlCaptureRequest(captureConfigs);
    }
    ...
    /** {@inheritDoc} */
    @Override
    public void submitCaptureRequest(final List<CaptureConfig> captureConfigs) {
        if (Looper.myLooper() != mHandler.getLooper()) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Camera2CameraControl.this.submitCaptureRequest(captureConfigs);
                }
            });
            return;
        }
        ...
        notifyCaptureRequest(captureConfigsWithImpl);
    }
