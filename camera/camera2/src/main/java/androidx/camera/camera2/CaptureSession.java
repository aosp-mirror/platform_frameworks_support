...
import androidx.camera.core.SessionEventCallback;
...
final class CaptureSession {
        ...
        synchronized (mStateLock) {
            switch (mState) {
                ...
                case INITIALIZED:
                    ...
                    SessionEventCallback comboSessionEventCallback =
                            SessionEventCallbacks.createComboCallback(
                                    sessionConfig.getSessionEventCallback());
                    List<CaptureStage> presetList = comboSessionEventCallback.onPresetSession();
                    CaptureRequest.Builder builder =
                            sessionConfig.getCaptureConfig().buildCaptureRequest(cameraDevice);
            ...
            switch (mState) {
                ...
                case OPENED:
                    ...
                    if (mSessionConfig != null) {
                        SessionEventCallback comboSessionEventCallback =
                                SessionEventCallbacks.createComboCallback(
                                        mSessionConfig.getSessionEventCallback());
                        List<CaptureStage> list = comboSessionEventCallback.onDisableSession();
                        if (!list.isEmpty()) {
    ...
    void issueRepeatingCaptureRequests() {
        ...
        try {
            ...
            SessionEventCallback comboSessionEventCallback =
                    SessionEventCallbacks.createComboCallback(
                            mSessionConfig.getSessionEventCallback());
            List<CaptureStage> repeatingRequestList = comboSessionEventCallback.onRepeating();
                    ...
                    case OPENING:
                        ...
                        if (mSessionConfig != null) {
                            SessionEventCallback comboSessionEventCallback =
                                    SessionEventCallbacks.createComboCallback(
                                            mSessionConfig.getSessionEventCallback());
                            List<CaptureStage> list = comboSessionEventCallback.onEnableSession();
                            if (!list.isEmpty()) {
                                List<CaptureConfig> configList = new LinkedList<>();
                                for (CaptureStage s : list) {
                                    configList.add(s.getCaptureConfig());
    ...
    private static class ExecutorHandlerAdapter implements Executor {
        ...
        public void execute(Runnable command) {
            if (mHandler == null) {
                // Do nothing if the handler is null.
                return;
            }
