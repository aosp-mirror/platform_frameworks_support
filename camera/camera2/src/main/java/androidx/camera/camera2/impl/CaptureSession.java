    ...
    void issueRepeatingCaptureRequests() {
        ...
        try {
            ...
            CameraEventCallbacks eventCallbacks = new Camera2Config(
                    mSessionConfig.getImplementationOptions()).getCameraEventCallback(
                    CameraEventCallbacks.createEmptyCallback());
            List<CaptureConfig> onRepeatingCaptureConfigs =
            ...
            for (CaptureConfig config : onRepeatingCaptureConfigs) {
                captureConfigBuilder.addImplementationOptions(config.getImplementationOptions());
            }

            // mSessionConfig.getRepeatingCaptureConfig()'s ImplementationOptions have higher
            // priority than the options returned in CameraEventCallback's onRepeating(). So here we
            // add the options again to override the same options. CameraEventCallback's onRepeating
            // is a hook to allow other modules to insert some custom options to the repeating
            // request. There are risks that the modules could modify some shared options like the
            // 3A parameters and make our 3A operation failed.
    ...
    void issueBurstCaptureRequest() {
        ...
        try {
            ...
            for (CaptureConfig captureConfig : mCaptureConfigs) {
                ...
                // Burst requests' own captureConfig's ImplementationOptions have higher priority
                // than the repeating requests's options, so here we add the options again to
                // override the same options.
                captureConfigBuilder.addImplementationOptions(
