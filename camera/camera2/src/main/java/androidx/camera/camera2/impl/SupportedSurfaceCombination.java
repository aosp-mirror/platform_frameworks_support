...
final class SupportedSurfaceCombination {
    ...
    private List<Size> getSupportedOutputSizes(UseCase useCase) {
        ...
        for (Size outputSize : outputSizeCandidates) {
            ...
            if (isAspectRatioMatch(aspectRatio, outputSize)) {
    ...
    private boolean isAspectRatioMatch(Rational targetAspectRatio, Size size) {
        boolean isMatch = false;
        if (targetAspectRatio == null) {
            isMatch = false;
        } else if (targetAspectRatio.equals(new Rational(size.getWidth(), size.getHeight()))) {
            isMatch = true;
        } else if (size.getHeight() % ALIGN16 == 0 && size.getWidth() % ALIGN16 == 0) {
            // Check whether the resolution is a mod16 version of target aspect ratio with
            ...
            isMatch =
                    size.getHeight() == ceilAlignedHeight || size.getHeight() == floorAlignedHeight;
        }
        return isMatch;
    }
