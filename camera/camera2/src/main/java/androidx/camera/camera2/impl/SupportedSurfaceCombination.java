...
final class SupportedSurfaceCombination {
    ...
    boolean requiresCorrectedAspectRatio() {
        boolean requiresCorrectedAspectRatio = false;
        ...
        if (mHardwareLevel == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY
            ...
            requiresCorrectedAspectRatio = true;
        }
        return requiresCorrectedAspectRatio;
    }
    ...
    Rational getCorrectedAspectRatio(UseCaseConfig<?> useCaseConfig) {
         ...
         * resolution to preview. The Camera2 LEGACY mode API always sends the HAL a configure call
         * for a preview resolution with the same aspect ratio as the maximum JPEG resolution, and
         * do the cropping/scaling before returning the output. There is a bug because of a flipped
         * scaling factor in the intermediate texture transform matrix, and it was fixed in L MR1.
         */
        if (mHardwareLevel == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY
                && Build.VERSION.SDK_INT == 21) {
