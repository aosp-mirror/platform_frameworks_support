...
final class DeferredImageReaderSurface extends DeferrableSurface {
        ...
        runOnMainThread(new Runnable() {
            ...
            public void run() {
                ...
                if (mImageReader != null) {
                    ...
                    if (DEBUG) {
                        Log.d(TAG, "ImageReader release list size: "
                                + mImageReaderToReleaseList.size());
                    }
                }
    ...
    public void freeUnusedSurfaces() {
        // Free surfaces in ImageReader release list
        freeImageReaderList();
    }
        ...
        setOnSurfaceDetachedListener(
                ...
                new DeferrableSurface.OnSurfaceDetachedListener() {
                    ...
                    public void onSurfaceDetached() {
                        ...
                        if (mImageReader != null) {
                            if (DEBUG) {
                                Log.d(TAG, "onSurfaceDetached to free ImageReader: "
                                        + mImageReader);
                            }
                            // There will be the strict mode error under API Level 23. Add surface
    ...
    private void freeImageReaderList() {
        // Release inactive image readers
        if (!mImageReaderToReleaseList.isEmpty()) {
            if (DEBUG) {
                Log.d(TAG, "freeUnusedSurfaces list size: " + mImageReaderToReleaseList.size());
            }
