...
public final class CameraX {
    ...
    /**
     * Returns all the camera ids for the cameras with the specified lens facing.
     *
     * @param lensFacing the lens facing of the camera
     * @return the cameraId set if camera exists or {@code null} if no camera with specified facing
     * exists
     * @throws CameraInfoUnavailableException if unable to access cameras, perhaps due to
     *                                        insufficient permissions.
     * @hide
    ...
    @RestrictTo(Scope.LIBRARY_GROUP)
    @Nullable
    public static Set<String> getCameraSetWithLensFacing(LensFacing lensFacing)
            throws CameraInfoUnavailableException {
        return INSTANCE.getCameraFactory().cameraIdSetForLensFacing(lensFacing);
    }
    ...
    public static String getCameraWithCameraDeviceConfig(CameraDeviceConfig config)
        ...
        availableCameraIds = config.getCameraIdFilterSet().filter(availableCameraIds);
