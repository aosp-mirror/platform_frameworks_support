 ...
 * let the lifecycle handle the rest, so application code generally does not need to call unbind
 * nor call {@link #bindToLifecycle} more than once.
 *
 ...
 * longer receive camera data.  Use cases can remain bound and will become active again on the
 * next {@link Lifecycle.Event#ON_START} transition.
 *
 ...
 * bound use cases are unbound, the camera session is destroyed and the camera is closed
 * asynchronously on a CameraX managed thread.  Calls to {@link #bindToLifecycle}
 ...
 *   // disassociate the use case from the  lifecycle to move use case to a different lifecycle.
...
public final class CameraX {
     ...
     * listed in {@link android.hardware.camera2.CameraDevice#createCaptureSession}.
     *
     ...
     * {@link #unbind(UseCase...)} or {@link #unbindAll()}.  A use case will not be unbound in
     * the middle of a method call as long as calls on the UseCases are made on the main thread.
     * This is because a lifecycle events only execute on the main thread.
     */
