...
public final class MetadataImageReaderTest {
    ...
    private final FakeCameraCaptureResult mCameraCaptureResult2 = new FakeCameraCaptureResult();
    ...
    public void setUp() {
        ...
        mCameraCaptureResult2.setTimestamp(TIMESTAMP_2);
    }
    ...
    public void maxImageHasBeenAcquired() throws InterruptedException {
        ...
        createMetadataImageReaderWithCapacity(2);

        // Feeds two CaptureResult into it.
        mMetadataImageReader.getCameraCaptureCallback().onCaptureCompleted(mCameraCaptureResult0);
        mMetadataImageReader.getCameraCaptureCallback().onCaptureCompleted(mCameraCaptureResult1);
        mMetadataImageReader.getCameraCaptureCallback().onCaptureCompleted(mCameraCaptureResult2);

        final List<ImageProxy> receivedImage = new ArrayList<>();
        ...
        ImageReaderProxy.OnImageAvailableListener outputListener =
                new ImageReaderProxy.OnImageAvailableListener() {
                    ...
                    public void onImageAvailable(ImageReaderProxy imageReader) {
                        ...
                        ImageProxy resultImage = imageReader.acquireNextImage();
                        receivedImage.add(resultImage);
                        if (receivedImage.size() == 2) {
                            mSemaphore.release();
                        }
                    }
                };
        ...
        assertThat(receivedImage.get(0).getTimestamp()).isEqualTo(TIMESTAMP_0);
        assertThat(receivedImage.get(0).getImageInfo().getTimestamp()).isEqualTo(
                TIMESTAMP_0);

        assertThat(receivedImage.get(1).getTimestamp()).isEqualTo(TIMESTAMP_1);
        assertThat(receivedImage.get(1).getImageInfo().getTimestamp()).isEqualTo(
                TIMESTAMP_1);

        final AtomicBoolean receivedSecondImage = new AtomicBoolean(false);
        outputListener =
                new ImageReaderProxy.OnImageAvailableListener() {
                    @Override
                    public void onImageAvailable(ImageReaderProxy imageReader) {
                        // The second ImageProxy should be dropped, otherwise fail the test case.
                        receivedSecondImage.set(true);
                        mSemaphore.release();
                    }
                };
        mMetadataImageReader.setOnImageAvailableListener(outputListener, mBackgroundHandler);
        // Feeds the second Image.
        assertThat(mImageReader.triggerImageAvailable(null, TIMESTAMP_2, 50,
                TimeUnit.MILLISECONDS)).isFalse();

        // Waits for a time period.
        mSemaphore.tryAcquire(300, TimeUnit.MILLISECONDS);

        assertThat(receivedSecondImage.get()).isFalse();
    }
