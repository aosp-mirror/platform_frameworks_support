...
class CommonPlayer {
    ...
    CommonPlayer(@NonNull MediaController controller, @NonNull Executor executor,
            @NonNull CommonCallback callback) {
        mController = controller;
        mControllerCallback = new MediaControllerCallback(callback);
        mController.registerExtraCallback(executor, mControllerCallback);
    ...
    private class MediaControllerCallback extends MediaController.ControllerCallback {
        private final CommonCallback mCommonCallback;

        MediaControllerCallback(@NonNull CommonCallback callback) {
        ...
        public void onAllowedCommandsChanged(@NonNull MediaController controller,
            ...
            mCommonCallback.onAllowedCommandsChanged(CommonPlayer.this, commands);
        }
        ...
        public SessionResult onCustomCommand(@NonNull MediaController controller,
            ...
            return mCommonCallback.onCustomCommand(CommonPlayer.this, command, args);
        }

        @Override
        public void onPlayerStateChanged(@NonNull MediaController controller, int state) {
            mCommonCallback.onPlayerStateChanged(CommonPlayer.this, state);
        }

        @Override
        public void onPlaybackSpeedChanged(@NonNull MediaController controller, float speed) {
            mCommonCallback.onPlaybackSpeedChanged(CommonPlayer.this, speed);
        }

        @Override
        public void onSeekCompleted(@NonNull MediaController controller, long position) {
            mCommonCallback.onSeekCompleted(CommonPlayer.this, position);
        }
        ...
        public void onCurrentMediaItemChanged(@NonNull MediaController controller,
            ...
            mMediaMetadata = item == null ? null : item.getMetadata();
            mCommonCallback.onCurrentMediaItemChanged(CommonPlayer.this, item);
        }

        @Override
        public void onPlaybackCompleted(@NonNull MediaController controller) {
            mCommonCallback.onPlaybackCompleted(CommonPlayer.this);
        }

        @Override
        public void onVideoSizeChanged(@NonNull MediaController controller, @NonNull MediaItem item,
                @NonNull VideoSize videoSize) {
            mCommonCallback.onVideoSizeChanged(CommonPlayer.this, item, videoSize);
        }

        @Override
        public void onTrackInfoChanged(@NonNull MediaController controller,
                @NonNull List<TrackInfo> trackInfos) {
            mCommonCallback.onTrackInfoChanged(CommonPlayer.this, trackInfos);
        }

        @Override
        public void onTrackSelected(@NonNull MediaController controller,
                @NonNull TrackInfo trackInfo) {
            mCommonCallback.onTrackSelected(CommonPlayer.this, trackInfo);
        }

        @Override
        public void onTrackDeselected(@NonNull MediaController controller,
                @NonNull TrackInfo trackInfo) {
            mCommonCallback.onTrackDeselected(CommonPlayer.this, trackInfo);
        }

        @Override
        public void onSubtitleData(@NonNull MediaController controller, @NonNull MediaItem item,
                @NonNull TrackInfo track, @NonNull SubtitleData data) {
            mCommonCallback.onSubtitleData(CommonPlayer.this, item, track, data);
        }
    }

    abstract static class CommonCallback {
        void onAllowedCommandsChanged(@NonNull CommonPlayer player,
                @NonNull SessionCommandGroup commands) {
        }
        void onCurrentMediaItemChanged(@NonNull CommonPlayer player, @Nullable MediaItem item) {
        }
        // TODO: Remove onCustomCommand (b/133124026)
        SessionResult onCustomCommand(@NonNull CommonPlayer player,
                @NonNull SessionCommand command, @Nullable Bundle args) {
            return new SessionResult(SessionResult.RESULT_ERROR_NOT_SUPPORTED, null);
        }
        void onPlayerStateChanged(@NonNull CommonPlayer player, int state) {
        }
        void onPlaybackSpeedChanged(@NonNull CommonPlayer player, float speed) {
        }
        void onSeekCompleted(@NonNull CommonPlayer player, long position) {
        }
        void onPlaybackCompleted(@NonNull CommonPlayer player) {
        }
        void onVideoSizeChanged(@NonNull CommonPlayer player, @NonNull MediaItem item,
                @NonNull VideoSize videoSize) {
        }
        void onTrackInfoChanged(@NonNull CommonPlayer player,
                @NonNull List<TrackInfo> trackInfos) {
        }
        void onTrackSelected(@NonNull CommonPlayer player, @NonNull TrackInfo trackInfo) {
        }
        void onTrackDeselected(@NonNull CommonPlayer player, @NonNull TrackInfo trackInfo) {
        }
        void onSubtitleData(@NonNull CommonPlayer player, @NonNull MediaItem item,
                @NonNull TrackInfo track, @NonNull SubtitleData data) {
        }
    }
}
