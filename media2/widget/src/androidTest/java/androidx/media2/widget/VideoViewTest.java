...
public class VideoViewTest {
    ...
    public void testSubtitleSelection() throws Throwable {
        ...
        verify(mPlayerCallback, timeout(TIME_OUT).atLeastOnce()).onTrackInfoChanged(
                any(SessionPlayer.class),
                argThat(new SubtitleTrackListMatcher(2)));

        List<TrackInfo> tracks = mPlayer.getTrackInfoInternal();
        ...
        mPlayer.selectTrackInternal(subtitleTracks.get(0));
        verify(mPlayerCallback, timeout(TIME_OUT).atLeastOnce()).onTrackSelected(
                any(SessionPlayer.class),
                argThat(new SubtitleTrackMatcher(subtitleTracks.get(0))));

        // Select the second subtitle track
        mPlayer.selectTrackInternal(subtitleTracks.get(1));
        verify(mPlayerCallback, timeout(TIME_OUT).atLeastOnce()).onTrackSelected(
                any(SessionPlayer.class),
                argThat(new SubtitleTrackMatcher(subtitleTracks.get(1))));

        // Deselect subtitle track
        mPlayer.deselectTrackInternal(subtitleTracks.get(1));
        verify(mPlayerCallback, timeout(TIME_OUT).atLeastOnce()).onTrackDeselected(
                any(SessionPlayer.class),
                argThat(new SubtitleTrackMatcher(subtitleTracks.get(1))));
    }
