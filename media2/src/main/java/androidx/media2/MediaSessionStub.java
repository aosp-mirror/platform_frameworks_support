...
class MediaSessionStub extends IMediaSession.Stub {
    ...
    @Override
    public void subscribeRoutesInfo(IMediaController caller, int seq) {
        if (caller == null) {
            return;
        }
        dispatchSessionTask(caller, seq, SessionCommand.COMMAND_CODE_SESSION_SUBSCRIBE_ROUTES_INFO,
                new SessionCallbackTask<Integer>() {
                    @Override
                    public Integer run(ControllerInfo controller) {
    ...
    @Override
    public void unsubscribeRoutesInfo(IMediaController caller, int seq) {
        if (caller == null) {
            return;
        }
        dispatchSessionTask(caller, seq,
                SessionCommand.COMMAND_CODE_SESSION_UNSUBSCRIBE_ROUTES_INFO,
                new SessionCallbackTask<Integer>() {
                    @Override
                    public Integer run(ControllerInfo controller) {
    ...
    @Override
    public void selectRoute(IMediaController caller, int seq, final Bundle route) {
        if (caller == null) {
            return;
        }
        if (MediaUtils.isUnparcelableBundle(route)) {
