...
class MediaSessionService2ImplBase implements MediaSessionService2Impl {
    ...
    public IBinder onBind(Intent intent) {
        final MediaSessionService2 service = mInstance;
        if (service == null) {
                        ...
                        public void onPlayerStateChanged(int state) {
                            ...
                            if (isPlaybackStopped(state)) {
                                ...
                                for (int i = 0; i < sessions.size(); i++) {
                                    MediaSession2 session = sessions.get(i);
                                    if (!isPlaybackStopped(session.getPlayerState())) {
                                        areAllStopped = false;
                                        break;
                                    }
                                }
    ...
    static boolean isPlaybackStopped(int state) {
        return state == MediaPlayerConnector.PLAYER_STATE_IDLE
            ...
            executor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        final MediaSessionService2 service = serviceImpl.mInstance;
                        if (service == null) {
                            if (DEBUG) {
