...
public class MediaUtils2 {
    ...
    private static class MediaItemParcelImpl extends ParcelImpl {
        ...
        MediaItemParcelImpl(MediaItem2 item) {
            // Sets the sanitized copy for the writeToParcel().
            super(new MediaItem2(item));

            // Keeps the original copy, so getVesionedParcel() can return the original one for the
            // local binder.
            mItem = item;
        }

        MediaItemParcelImpl(Parcel parcel) {
            super(parcel);
            mItem = super.getVersionedParcel();
        }
