...
public class MediaNotificationHandler {
    ...
    /**
     * Gets a default media style notification for {@link MediaSessionService2}.
     */
    ...
    private PendingIntent createPendingIntent(@PlaybackStateCompat.Actions long action) {
        ...
        intent.setComponent(new ComponentName(
                mServiceInstance.getPackageName(), mServiceInstance.getClass().getCanonicalName()));
        intent.putExtra(Intent.EXTRA_KEY_EVENT, new KeyEvent(KeyEvent.ACTION_DOWN, keyCode));

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && keyCode == KEYCODE_MEDIA_PLAY) {
