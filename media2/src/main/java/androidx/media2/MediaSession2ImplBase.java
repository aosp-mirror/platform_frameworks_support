...
class MediaSession2ImplBase implements MediaSession2Impl {
    ...
    private static class SessionPlayerCallback extends SessionPlayer2.PlayerCallback {
        ...
        private final CurrentMediaItemListener mListenerForCurrentItemChanged;
        private final PlaylistItemListener mListenerForPlaylistChanged;

        SessionPlayerCallback(MediaSession2ImplBase session) {
            mSession = new WeakReference<>(session);
            mListenerForCurrentItemChanged = new CurrentMediaItemListener(session);
            mListenerForPlaylistChanged = new PlaylistItemListener(session);
        }
