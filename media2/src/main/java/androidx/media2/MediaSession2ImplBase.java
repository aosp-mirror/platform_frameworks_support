...
class MediaSession2ImplBase implements MediaSession2Impl {
    ...
    private <T> T executeCommand(@NonNull PlayerCommand<T> command, T fallbackResult) {
        final SessionPlayer2 player;
        synchronized (mLock) {
            player = mPlayer;
        }
    ...
    ListenableFuture<SessionResult> sendCommand(@NonNull ControllerInfo controller,
            @NonNull ControllerCommand command) {
        if (!isConnected(controller)) {
            return SessionResult.createResult(RESULT_CODE_DISCONNECTED);
        }
        try {
            final ListenableFuture<SessionResult> result;
            final int seq;
            final CallSequenceManager manager = mSession2Stub.getConnectedControllersManager()
                    .getCallSequenceManager(controller);
            if (manager != null) {
                result = manager.createRemoteProcessFuture();
                seq = ((CallSequenceManager.RemoteProcessFuture<SessionResult>) result)
                        .getSequenceNumber();
            } else {
                ...
                seq = 0;
                result = SessionResult.createResult(RESULT_CODE_SUCCESS);
            }
            ...
            //   - DeadSystemException means that errors around it can be ignored.
            Log.w(TAG, "Exception in " + controller.toString(), e);
            return SessionResult.createResult(RESULT_CODE_UNKNOWN_ERROR);
        }
    }

    void broadcastCommand(@NonNull ControllerCommand command) {
        List<ControllerInfo> controllers =
                mSession2Stub.getConnectedControllersManager().getConnectedControllers();
        ...
        for (int i = 0; i < controllers.size(); i++) {
            ...
            try {
                ...
                final int seq;
                if (manager != null) {
                    seq = manager.obtainSequenceNumber();
                    // Can be null in two cases. Use the 0 as sequence number in both cases because
                    //     Case 1) Controller is from the legacy stub
                    //             -> Sequence number isn't needed, so 0 is OK
                    //     Case 2) Controller is removed after the connection check above
