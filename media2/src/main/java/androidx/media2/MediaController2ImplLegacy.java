...
class MediaController2ImplLegacy implements MediaController2Impl {
    ...
    void onConnectedNotLocked() {
        ...
        synchronized (mLock) {
            ...
            // If metadata is null (i.e. current media item is null),
            // then try to set the current media item with active queue item id.
            if (mCurrentMediaItem == null && mQueue != null && mPlaybackStateCompat != null) {
                long activeQueueItemId = mPlaybackStateCompat.getActiveQueueItemId();
                for (QueueItem item : mQueue) {
                    if (item.getQueueId() == activeQueueItemId) {
                        mCurrentMediaItem = MediaUtils2.convertToMediaItem2(item);
                        break;
                    }
                }
            }
        ...
        public void onPlaybackStateChanged(final PlaybackStateCompat state) {
            ...
            synchronized (mLock) {
                ...
                if (mQueue != null && state != null) {
                    for (int i = 0; i < mQueue.size(); ++i) {
                        ...
                        // MediaMetadataCompat can contain more information than QueueItem.
            ...
            if (bufferingState != prevBufferingState) {
                mCallbackExecutor.execute(new Runnable() {
                    @Override
                    public void run() {
                        mCallback.onBufferingStateChanged(mInstance, newCurrentItem,
                                bufferingState);
                    }
                });
            }
