...
public class XMediaPlayer extends SessionPlayer2 {
    ...
    private static final int CALL_TYPE_NONE_PLAYER2_COMMAND = -1000;
    ...
    /* A list for tracking the futures that will be submitted to MediaPlayer2.*/
    ...
    /**
     * PendingPlayer2Future is a PendingFuture for MediaPlayer2.
     * onExecute() should return the token from MediaPlayer2 returned from its asynchronous call.
     */
    abstract static class PendingPlayer2Future<V> extends PendingFuture<V> {
        Object mToken;

        PendingPlayer2Future(int callType) {
            super(callType);
        }
    ...
    abstract class PendingXPlayerFuture<V> extends PendingFuture<V> {
        PendingXPlayerFuture() {
            super(CALL_TYPE_NONE_PLAYER2_COMMAND);
        }
    ...
    private Runnable getPendingFutureListenerForCancel(final PendingFuture future) {
        if (future instanceof PendingPlayer2Future) {
            final PendingPlayer2Future player2Future = (PendingPlayer2Future) future;
            return new Runnable() {
                @Override
                public void run() {
    ...
    void addPendingFutureFront(PendingFuture<PlayerResult> future) {
        ArrayList<PendingFuture<PlayerResult>> futures = new ArrayList<>();
        futures.add(future);
    ...
    void addPendingFuturesFront(final List<PendingFuture<PlayerResult>> futures) {
        synchronized (mPendingFutures) {
            for (int i = futures.size() - 1; i >= 0; --i) {
                final PendingFuture future = futures.get(i);
                future.addListener(getPendingFutureListenerForCancel(future), mExecutor);
    ...
    public ListenableFuture<PlayerResult> seekTo(final long position) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_SEEK_TO) {
                    @Override
                    Object onExecute() {
    ...
    public ListenableFuture<PlayerResult> setPlaybackSpeed(final float playbackSpeed) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(
    ...
    public ListenableFuture<PlayerResult> setSurface(final Surface surface) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_SET_SURFACE) {
                    @Override
                    Object onExecute() {
    ...
    public ListenableFuture<PlayerResult> attachAuxEffect(final int effectId) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(
                        MediaPlayer2.CALL_COMPLETED_ATTACH_AUX_EFFECT) {
                    @Override
                    Object onExecute() {
    ...
    public ListenableFuture<PlayerResult> selectTrack(final int index) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_SELECT_TRACK) {
                    @Override
                    Object onExecute() {
    ...
    public ListenableFuture<PlayerResult> deselectTrack(final int index) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_DESELECT_TRACK) {
                    @Override
                    Object onExecute() {
    ...
    public ListenableFuture<DrmResult> prepareDrm(@NonNull final UUID uuid) {
        PendingFuture<DrmResult> pendingFuture =
                new PendingPlayer2Future<DrmResult>(MediaPlayer2.CALL_COMPLETED_PREPARE_DRM) {
                    @Override
                    Object onExecute() {
    ...
    PendingFuture<PlayerResult> setPlayerVolumeInternal(final float volume) {
        return new PendingPlayer2Future<PlayerResult>(
    ...
    PendingFuture<PlayerResult> setMediaItemInternal(final MediaItem2 item) {
        PendingFuture<PlayerResult> pendingFuture =
                new PendingPlayer2Future<PlayerResult>(
                        MediaPlayer2.CALL_COMPLETED_SET_DATA_SOURCE) {
                    @Override
                    Object onExecute() {
                        synchronized (mPlaylistLock) {
    ...
    PendingFuture<PlayerResult> playInternal() {
        return new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_PLAY) {
            @Override
            Object onExecute() {
                return mPlayer.play();
            }
    ...
    PendingFuture<PlayerResult> pauseInternal() {
        return new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_PAUSE) {
            @Override
            Object onExecute() {
    ...
    PendingFuture<PlayerResult> prepareInternal() {
        return new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_PREPARE) {
            @Override
            Object onExecute() {
    ...
    PendingFuture<PlayerResult> skipToNextInternal() {
        return new PendingPlayer2Future<PlayerResult>(MediaPlayer2.CALL_COMPLETED_SKIP_TO_NEXT) {
            @Override
            Object onExecute() {
    ...
    void handleCallComplete(MediaPlayer2 mp, final MediaItem2 item, int what, int status) {
        PendingFuture expected;
        synchronized (mPendingFutures) {
            expected = mPendingFutures.pollFirst();
        }
        if (expected == null) {
    ...
    void executePendingFuturesIfNeeded() {
        synchronized (mPendingFutures) {
            ...
            while ((future = mPendingFutures.peekFirst()) != null) {
                if (future.mCallType == CALL_TYPE_NONE_PLAYER2_COMMAND) {
                    // For CALL_TYPE_NONE_PLAYER2_COMMAND, mPendingFutures.removeFirst() should
                    // be called before future.run() since some pending futures call
                    // addPendingFuturesFront() in onExecute(). ex) setPlayerMediaItemsInternal()
            ...
            boolean stopOnNonePlayer2Command = false;
            while (it.hasNext()) {
                future = it.next();
                if (future.mCallType != CALL_TYPE_NONE_PLAYER2_COMMAND) {
                    stopOnNonePlayer2Command = true;
                } else if (stopOnNonePlayer2Command) {
                    break;
                }
                if (!future.mExecuted) {
                    future.run();
