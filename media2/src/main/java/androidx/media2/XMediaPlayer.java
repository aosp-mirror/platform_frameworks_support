...
public class XMediaPlayer extends SessionPlayer2 {
    ...
    @Override
    // TODO: consider to change to replacePlaylistItem(MI2, MI2)
    ...
    private ListenableFuture<CommandResult2> createFutureForResultCodeInternal(int resultCode) {
        SettableFuture<CommandResult2> future = SettableFuture.create();
        synchronized (mCallTypeAndFutures) {
            if (mCallTypeAndFutures.size() > 0) {
                mCallTypeAndFutures.add(
                        new Pair<>(CALL_COMPLETE_PLAYLIST_BASE - resultCode, future));
            } else {
    ...
    private Pair<MediaItem2, MediaItem2> updateAndGetCurrentNextItemIfNeededLocked() {
        ...
        if (mCurPlaylistItem != mShuffledList.get(mCurrentShuffleIdx)) {
        ...
        if (nextShuffleIdx == END_OF_PLAYLIST) {
            mNextPlaylistItem = null;
        } else if (mNextPlaylistItem != mShuffledList.get(nextShuffleIdx)) {
    ...
    class Mp2Callback extends MediaPlayer2.EventCallback {
        ...
        public void onCallCompleted(
            ...
            while (true) {
                ...
                if (pair.first <= CALL_COMPLETE_PLAYLIST_BASE) {
                    pair.second.set(new CommandResult2(
                            CALL_COMPLETE_PLAYLIST_BASE - pair.first, item));
                } else {
                    synchronized (mCallTypeAndFutures) {
                        mCallTypeAndFutures.addFirst(pair);
                    }
                    break;
                }
            }
