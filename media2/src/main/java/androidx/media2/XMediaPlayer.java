...
public class XMediaPlayer extends SessionPlayer2 {
     ...
     *
     * @param loop {@code true} if the current media item is meant to loop.
     * @return a {@link ListenableFuture} which represents the pending completion of the command.
     * {@link CommandResult2} will be delivered when the command completes.
     */
    public ListenableFuture<CommandResult2> loopCurrent(boolean loop) {
        SettableFuture<CommandResult2> future = SettableFuture.create();
        synchronized (mCallTypeAndFutures) {
            mCallTypeAndFutures.add(new Pair<>(MediaPlayer2.CALL_COMPLETED_LOOP_CURRENT, future));
    ...
    public ListenableFuture<CommandResult2> attachAuxEffect(int effectId) {
        ...
        synchronized (mCallTypeAndFutures) {
            ...
            mPlayer.setAudioSessionId(effectId);
        }
        return future;
    }
    ...
    private void notifySessionPlayerCallback(final SessionPlayerCallbackNotifier notifier) {
    ...
    private void notifyXMediaPlayerCallback(final XMediaPlayerCallbackNotifier notifier) {
        Map<SessionPlayer2.PlayerCallback, Executor> map = getCallbacks();
        for (final SessionPlayer2.PlayerCallback callback : map.keySet()) {
