...
public class XMediaPlayer extends SessionPlayer2 {
    ...
    public ListenableFuture<CommandResult2> setMediaItem(MediaItem2 item) {
        ...
        return getFutureForSetMediaItem(item);
    }
        ...
        synchronized (mPlaylistLock) {
            ...
            if (prevShuffleIdx < 0) {
                if (mRepeatMode == REPEAT_MODE_ALL || mRepeatMode == REPEAT_MODE_GROUP) {
                    ...
                    SettableFuture<CommandResult2> future = SettableFuture.create();
                    future.set(new CommandResult2(RESULT_CODE_NO_ERROR, mCurrentItem));
                    return future;
                }
            }
    ...
    private ListenableFuture<CommandResult2> setPlayerMediaItems(
        ...
        synchronized (mStateLock) {
            isPlayerIdle = mState == PLAYER_STATE_IDLE;
        }

        ListenableFuture<CommandResult2> future = isPlayerIdle ? getFutureForSetMediaItem(curItem)
