...
public class XMediaPlayer extends SessionPlayer2 {
    ...
    public ListenableFuture<CommandResult2> setPlaylist(
            @NonNull List<MediaItem2> playlist, @Nullable MediaMetadata2 metadata) {
    ...
    class Mp2Callback extends MediaPlayer2.EventCallback {
        ...
        public void onCallCompleted(
            ...
            if (expected == null || what != expected.mCallType) {
            ...
            while (true) {
                synchronized (mPendingCommands) {
                    command = mPendingCommands.pollFirst();
                }
                if (command == null) {
                    break;
                }
                if (command.mCallType <= CALL_COMPLETE_PLAYLIST_BASE) {
                    command.mFuture.set(new CommandResult2(
                            CALL_COMPLETE_PLAYLIST_BASE - command.mCallType, item));
                } else {
                    synchronized (mPendingCommands) {
                        mPendingCommands.addFirst(command);
                    }
                    break;
                }
            }
