...
public final class SessionToken2 implements VersionedParcelable {
    ...
    public static void createSessionToken2(@NonNull final Context context,
        ...
        try {
            ...
            if (token2Parcelable != null) {
                notifySessionToken2Created(executor, listener, token,
                        ParcelUtils.<SessionToken2>fromParcelable(token2Parcelable));
                return;
            }
                ...
                public void handleMessage(Message msg) {
                    synchronized (listener) {
                        if (msg.what == MSG_SEND_TOKEN2_FOR_LEGACY_SESSION) {
                            ...
                            token.setSessionToken2Parcelable(
                                    ParcelUtils.toParcelable(token2ForLegacySession));
                            notifySessionToken2Created(executor, listener, token,
            ...
            MediaControllerCompat.Callback callback = new MediaControllerCompat.Callback() {
                ...
                public void onSessionReady() {
                    synchronized (listener) {
                        ...
                        if (token.getSessionToken2Parcelable() == null) {
                            token.setSessionToken2Parcelable(
                                    ParcelUtils.toParcelable(token2ForLegacySession));
                        }
                        notifySessionToken2Created(executor, listener, token,
                                ParcelUtils.<SessionToken2>fromParcelable(
                                        token.getSessionToken2Parcelable()));
                        if (Build.VERSION.SDK_INT >= 18) {
                            thread.quitSafely();
                        } else {
                            thread.quit();
