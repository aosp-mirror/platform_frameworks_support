...
import static androidx.media2.MediaLibraryService2.LibraryParams.KEY_OFFLINE;
import static androidx.media2.MediaLibraryService2.LibraryParams.KEY_RECENT;
import static androidx.media2.MediaLibraryService2.LibraryParams.KEY_SUGGESTED;
...
class MediaBrowser2ImplLegacy extends MediaController2ImplLegacy implements MediaBrowser2Impl {
    ...
    public ListenableFuture<BrowserResult> getLibraryRoot(@Nullable final LibraryParams params) {
        final ResolvableFuture<BrowserResult> result = ResolvableFuture.create().create();
        final MediaBrowserCompat browser = getBrowserCompat(params);
        if (browser != null) {
            // Already connected with the given extras.
            result.set(new BrowserResult(RESULT_CODE_SUCCESS, createMediaItem(browser)));
        } else {
            getCallbackExecutor().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (params != null) {
                        rootHints = params.getExtras() == null
                                ? new Bundle() : new Bundle(params.getExtras());
                        // Only add keys from the BrowserRoot
                        rootHints.putBoolean(KEY_OFFLINE, params.getBoolean(KEY_OFFLINE));
                        rootHints.putBoolean(KEY_RECENT, params.getBoolean(KEY_RECENT));
                        rootHints.putBoolean(KEY_SUGGESTED, params.getBoolean(KEY_SUGGESTED));
                    }
            ...
            public void onSearchResult(final String query, final Bundle extrasSent,
                ...
                getCallbackExecutor().execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        future.set(new BrowserResult(RESULT_CODE_SUCCESS, item2List));
                    }
                });
            }
    ...
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    MediaItem2 createMediaItem(@NonNull MediaBrowserCompat browser) {
    ...
    private class SubscribeCallback extends SubscriptionCallback {
        ...
        public void onChildrenLoaded(final String parentId, List<MediaItem> children,
            ...
            final LibraryParams params = MediaUtils2.convertToLibraryParams(
    ...
    private class GetChildrenCallback extends SubscriptionCallback {
        ...
        public void onChildrenLoaded(final String parentId, List<MediaItem> children,
            ...
            if (children == null) {
                ...
                // - New API: Extra from MediaLibraryService2 to MediaBrowser2
                mFuture.set(new BrowserResult(RESULT_CODE_SUCCESS, items));
            }
        }
    }
}
