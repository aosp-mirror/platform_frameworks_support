...
public class MediaItem2 implements VersionedParcelable {
    ...
    void registerListener(OnMetadataChangedListener listener, Executor executor) {
        synchronized (mLock) {
            for (Pair<OnMetadataChangedListener, Executor> pair : mListeners) {
                if (pair.first == listener && pair.second != null) {
                    return;
                }
            }
    ...
    void unregisterListener(OnMetadataChangedListener listener) {
        synchronized (mLock) {
