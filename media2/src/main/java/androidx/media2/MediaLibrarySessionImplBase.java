...
class MediaLibrarySessionImplBase extends MediaSession2ImplBase implements MediaLibrarySessionImpl {
    ...
    private LibraryResult ensureNonNullResult(LibraryResult returnedResult) {
        if (returnedResult == null) {
            if (THROW_EXCEPTION_FOR_NULL_RESULT) {
                throw new RuntimeException("Shouldn't return null");
            }
            return new LibraryResult(LibraryResult.RESULT_CODE_UNKNOWN_ERROR);
        }
        return returnedResult;
    }

    private void ensureValidList(LibraryResult result, int pageSize) {
        if (result != null && result.getMediaItems() != null
                && result.getMediaItems().size() > pageSize) {
            throw new RuntimeException("List shouldn't contain more than pageSize. size="
                    + result.getMediaItems().size() + ", pageSize" + pageSize);
        }
    }
    ...
    public LibraryResult onGetLibraryRootOnExecutor(ControllerInfo controller,
            final LibraryParams params) {
        LibraryResult result = getCallback().onGetLibraryRoot(
                getInstance(), controller, params);
        return ensureNonNullResult(result);
    }
    ...
    public LibraryResult onGetChildrenOnExecutor(ControllerInfo controller, final String parentId,
        ...
        ensureValidList(result, pageSize);
        return ensureNonNullResult(result);
    }
    ...
    public int onSubscribeOnExecutor(ControllerInfo controller, String parentId,
        ...
        if (resultCode != LibraryResult.RESULT_CODE_SUCCESS) {
            synchronized (mLock) {
