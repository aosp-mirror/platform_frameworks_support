...
public class MediaController2 implements AutoCloseable {
     ...
     * session instead of the standard result codes defined in the {@link ControllerResult}.
     *
    ...
    private static ListenableFuture<ControllerResult> createDisconnectedResult() {
        return ControllerResult.createResult(ControllerResult.RESULT_CODE_DISCONNECTED);
    }
     ...
     */
    @VersionedParcelize
    public static class ControllerResult extends Result2 implements VersionedParcelable {
        /**
        ...
        ControllerResult(SessionResult sessionResult) {
            this(sessionResult.getResultCode(), sessionResult.getMediaItem(),
                    sessionResult.getExtra(), sessionResult.getCompletionTime());
        }
        ...
        ControllerResult(int resultCode, MediaItem2 item) {
            this(resultCode, item, null, SystemClock.elapsedRealtime());
        }
         ...
         */
        // Note: resultCode is intentionally not annotated for subclass to return extra error codes.
        ControllerResult(int resultCode, MediaItem2 item, Bundle extra) {
            this(resultCode, item, extra, SystemClock.elapsedRealtime());
        }
         ...
         */
        ControllerResult(int resultCode, MediaItem2 item, Bundle extra, long completionTime) {
        ...
        static ListenableFuture<ControllerResult> createResult(int resultCode) {
            SettableFuture<ControllerResult> result = SettableFuture.create();
            result.set(new ControllerResult(resultCode, null));
            return result;
        }
    }
}
