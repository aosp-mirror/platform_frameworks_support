...
class MediaSession2Stub extends IMediaSession2.Stub {
    ...
    private MediaItem2 convertMediaItem2OnExecutor(ControllerInfo controller,
            ParcelImpl itemParcelImpl) {
        if (itemParcelImpl == null) {
            return null;
        }
        MediaItem2 item = ParcelUtils.fromParcelable(itemParcelImpl);
        return mSessionImpl.getCallback().onCreateMediaItem(mSessionImpl.getInstance(),
                controller, item);
    }
