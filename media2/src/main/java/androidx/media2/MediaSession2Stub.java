...
class MediaSession2Stub extends IMediaSession2.Stub {
        ...
        mSessionImpl.getCallbackExecutor().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (runnable instanceof PlayerCommandRunnable) {
                        final ListenableFuture<PlayerResult> result =
                                ((PlayerCommandRunnable) runnable).run(controller);
                        if (result == null) {
                        ...
                        ((CustomRunnable) runnable).run(controller);
                    }
                } catch (RemoteException e) {
    ...
    public void sendCustomCommand(final IMediaController2 caller, final int seq,
        ...
        onSessionCommand(caller, seq, sessionCommand, new CustomRunnable() {
            @Override
    ...
    private interface PlayerCommandRunnable extends SessionRunnable {
    ...
    private interface CustomRunnable extends SessionRunnable {
    ...
    final class Controller2Cb extends ControllerCb {
        ...
        void onSessionResult(int seq, SessionResult result) throws RemoteException {
            if (result == null) {
                result = new SessionResult(RESULT_CODE_NOT_SUPPORTED);
            }
