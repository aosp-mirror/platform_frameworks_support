...
public class RemoteMediaBrowser extends RemoteMediaController {
     ...
     *                          false otherwise.
     */
    public RemoteMediaBrowser(Context context, SessionToken token, Bundle connectionHints,
    ...
    void create(SessionToken token, Bundle connectionHints, boolean waitForConnection) {
        try {
            mBinder.create(true /* isBrowser */, mControllerId, MediaUtils.toParcelable(token),
                    connectionHints, waitForConnection);
        } catch (RemoteException ex) {
