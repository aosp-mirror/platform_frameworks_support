...
public class MediaItemTest {
        ...
        public MediaItem create(Context context) {
            ...
            final DataSourceCallback callback = new DataSourceCallback() {
                ...
                public int readAt(long position, @NonNull byte[] buffer, int offset, int size)
                        throws IOException {
                    if (buffer != null && buffer.length > 0 && size > 0 && position == 0
                            && offset == 0) {
                        buffer[0] = 0x1;
                        return 1;
                    }
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
                {"MediaItem", sMediaItemFactory, MediaItem.Builder.class},
                {"UriMediaItem", sUriMediaItemFactory, UriMediaItem.Builder.class},
                {"CallbackMediaItem", sCallbackMediaItemFactory, CallbackMediaItem.Builder.class},
                {"FileMediaItem", sFileMediaItemFactory, FileMediaItem.Builder.class}});
    }

    public MediaItemTest(String testClass, MediaItemFactory factory, Class builderClass) {
