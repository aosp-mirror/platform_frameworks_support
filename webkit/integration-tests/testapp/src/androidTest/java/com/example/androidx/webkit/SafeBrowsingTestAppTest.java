...
public class SafeBrowsingTestAppTest {
    ...
    public void setUp() {
        ...
        IdlingRegistry.getInstance().register(SafeBrowsingActivity.getIdlingResource());
        // Skip features after registering IdlingResources, so that resources are registered before
        // we hit tearDown().
        mRule.assumeFeature(WebViewFeature.START_SAFE_BROWSING);
    }

    @After
    public void tearDown() {
        IdlingRegistry.getInstance().unregister(SafeBrowsingActivity.getIdlingResource());
    }
