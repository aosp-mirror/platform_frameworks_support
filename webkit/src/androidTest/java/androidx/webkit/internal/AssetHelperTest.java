...
public class AssetHelperTest {
    ...
    @Test
    @SmallTest
    public void testOpenFileInDifferentSubdirectoryInInternalStorage() throws IOException {
        writeToInternalStorage(Uri.parse("/some/path/to/file.txt"), TEST_STRING);
        writeToInternalStorage(Uri.parse("/some/path/file.txt"), TEST_STRING);
        writeToInternalStorage(Uri.parse("/some/path_2/to/file.txt"), TEST_STRING);

        InputStream stream = mAssetHelper.openFile("/some/path/",
                Uri.parse("/some/path/to/./file.txt"));
        Assert.assertNotNull(
                  "failed to opened a file from the mounted subdirectory in internal storage",
                  stream);
        Assert.assertEquals(readAsString(stream), TEST_STRING);

        stream = mAssetHelper.openFile("/some/path/", Uri.parse("/some/path/to/../file.txt"));
        Assert.assertNotNull(
                  "failed to opened a file from the mounted subdirectory in internal storage",
                  stream);
        Assert.assertEquals(readAsString(stream), TEST_STRING);

        stream = mAssetHelper.openFile("/some/path/to",
                 Uri.parse("/some/path/to/../file.txt"));
        Assert.assertNull(
                  "opened a file from a different mounted subdirectory in internal storage using "
                  + "\"..\"", stream);

        stream = mAssetHelper.openFile("/some/path/", Uri.parse("/some/path_2/to/file.txt"));
        Assert.assertNull(
                  "opened a file from a different mounted subdirectory in internal storage",
                  stream);
