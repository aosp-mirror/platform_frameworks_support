...
public class AssetHelperTest {
    ...
    @Test
    @SmallTest
    public void testOpenNonExistFileFromInternalStorage() throws IOException {
        writeToInternalStorage(Uri.parse("another_file.txt"), TEST_STRING);

        InputStream stream = mAssetHelper.openFile(Uri.parse("some_file.txt"));
        Assert.assertNull("some_file.txt doesn't exist, should fail", stream);
    }
