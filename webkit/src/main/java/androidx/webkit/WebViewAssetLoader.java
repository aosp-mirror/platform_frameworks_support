 ...
 * for this purpose) or the default domain that has been reserved for this:
...
public class WebViewAssetLoader {
        ...
        public Builder register(@NonNull PathHandler handler) {
            for (PathMatcher matcher : mBuilderMatcherList) {
                if (handler.getRegisteredPath().startsWith(matcher.mPath)) {
                    throw new IllegalArgumentException("This path matches an already matched path: "
                            + matcher.mPath);
                }
            }
    ...
    public WebResourceResponse shouldInterceptRequest(@NonNull Uri url) {
        ...
        // no match found, a null reposne is returned to fallback to Network.
        if (handler == null) {
            return null;
        }
