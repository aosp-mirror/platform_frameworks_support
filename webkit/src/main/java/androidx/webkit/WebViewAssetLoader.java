...
public class WebViewAssetLoader {
    ...
    public static final class Builder {
         ...
         * {@code "/"}.
         * <p>
         * A custom prefix path can be used in conjunction with a custom domain, to avoid conflicts
         * with real paths which may be hosted at that domain.
         * <p>
         * If a URL matches the registered prefix path, an attempt to load the requested asset will
         * be made. If the the requested asset cannot be returned based upon the local content, no
         * attempt will be made to fall back to a network request. For this reason you should not
         * use a prefix path that is also a prefix of any valid URL hosted on the same domain used
         * for this asset loader. All such URLs will resolve only to local content, rendering the
         * network content inaccessible.
         *
        ...
        public Builder setAssetsHostingPath(@NonNull String path) {
            if (mResourcesUri.getPath() == path) {
                throw new
                    IllegalArgumentException("Assets and Resources couldn't have the same path");
            }
     ...
     * <p>
     * If it is not possible to resolve the URL to the content of a resource or asset, then no
     * attempt will be made to fall back to a network request, and a 404 error will be generated.
     * <p>
     * This method should be invoked from within
     * {@link android.webkit.WebViewClient#shouldInterceptRequest(android.webkit.WebView, WebResourceRequest)}.
     * The return value of this method should be returned directly:
     *
     * <pre class="prettyprint">
