 ...
 *     // https://{package_name_sha1}.androidplatform.net/assets/index.html
...
public class WebViewAssetLoader {
     ...
     * @param authority default domain to host assets/resources.
     */
    public WebViewAssetLoader(@NonNull Context context, @NonNull String authority) {
        ...
        this(new AssetHelper(context.getApplicationContext()), authority);
    }
    ...
    public WebViewAssetLoader(@NonNull Context context) {
        this(context, getPackageHashDomain(context));
    }

    /**
     * @return The default authority/domain to be used.
     */
    @NonNull
    ...
    /**
     * @param context application context to get package name.
     *
     * @return A subdomain of androidplatform.net where the subdomain is SHA-1 hash
     *         of the application package name.
     */
    @NonNull
    private static String getPackageHashDomain(@NonNull Context context) {
        try {
            String packageName = context.getApplicationContext().getPackageName();
            String packageNameHash = getHexSha1(packageName);
            return packageNameHash + "." + KNOWN_UNUSED_AUTHORITY;
        } catch (NoSuchAlgorithmException e) {
            Log.e(TAG, "Cannot get hash code of package name");
            return KNOWN_UNUSED_AUTHORITY;
        }
    }

    @NonNull
    private static String getHexSha1(@NonNull String text) throws NoSuchAlgorithmException {
        try {
            MessageDigest messageDigest = MessageDigest.getInstance("SHA-1");
            byte[] hash = messageDigest.digest(text.getBytes(StandardCharsets.UTF_8));
            StringBuilder builder = new StringBuilder();
            for (int i = 0; i < hash.length; i++) {
                // Convert Byte to it's hex string.
                builder.append(String.format("%02x", hash[i]));
            }
            return builder.toString();
        } catch (NoSuchAlgorithmException e) {
            Log.e(TAG, "MessageDigest doesn't support SHA-1 algorithm");
            throw e;
        }
    }
     ...
     *            (domain) will be matched exactly. The path will be matched if it's a prefix.
     ...
     * <code>http(s)://{package_name_sha1_hex}.androidplatform.net/assets/...</code>.
     *
     ...
     * Hosts the application's assets on an http(s):// URL. Assets from the local path
     * <code>assetPath/...</code> will be available under
     * <code>http(s)://{package_name_sha1_hex}.androidplatform.net/{virtualAssetPath}/...</code>.
     *
     * @param assetPath the local path in the application's asset folder which will be made
     *                  available by the server. Should start with a leading slash (for example
     *                  "/www").
     ...
     * <code>http(s)://{package_name_sha1_hex}.androidplatform.net/res/{resource_type}/
     * {resource_name}</code>.
     *
    ...
    /**
     * Hosts the application's resources on an http(s):// URL. Resources
     * <code>http(s)://{package_name_sha1_hex}.androidplatform.net/{virtualResourcesPath}/
