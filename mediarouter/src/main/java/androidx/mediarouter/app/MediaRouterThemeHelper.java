...
final class MediaRouterThemeHelper {
    static final boolean USE_SUPPORT_DYNAMIC_GROUP =
    ...
    private static final int COLOR_DARK_RES_ID = R.color.mr_dynamic_dialog_icon_dark;

    private MediaRouterThemeHelper() {
    }

    static StateListDrawable getMuteButtonDrawableIcon(Context context) {
        Resources resources = context.getResources();
        VectorDrawableCompat mutedIcon =
                VectorDrawableCompat.create(resources, R.drawable.ic_vol_mute, null);
        VectorDrawableCompat unmutedIcon =
                VectorDrawableCompat.create(resources, R.drawable.ic_vol_unmute, null);

        if (isLightTheme(context)) {
            int tintColor = resources.getColor(COLOR_DARK_RES_ID);
            mutedIcon.setTint(tintColor);
            unmutedIcon.setTint(tintColor);
        }

        StateListDrawable muteButtonIcon = new StateListDrawable();
        muteButtonIcon.addState(new int[]{ android.R.attr.state_activated }, mutedIcon);
        muteButtonIcon.addState(new int[]{ -android.R.attr.state_activated }, unmutedIcon);
    ...
    static Drawable getSpeakerGroupDrawableIcon(Context context) {
        return getDrawableIcon(context, R.attr.mediaRouteSpeakerGroupIconDrawable);
    }
    ...
    private static Drawable getDrawableIcon(Context context, int resId) {
        ...
        if (USE_SUPPORT_DYNAMIC_GROUP && isLightTheme(context)) {
            int tintColor = context.getResources().getColor(COLOR_DARK_RES_ID);
