...
public class MediaRouteCastDialog extends AppCompatDialog {
    ...
    private boolean isSelectedRouteActualGroup() {
        ...
        if (dynamicGroup != null) {
            ...
            return dynamicGroup.getMemberRoutes().size() > 1;
    ...
    private final class RecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        void setItems() {
            ...
            if (!mGroupableRoutes.isEmpty()) {
                ...
                if (exists) {
                    String title = dynamicGroup == null
                            ? mContext.getString(R.string.mr_dialog_groupable_header)
                            : dynamicGroup.getController().getGroupableSelectionTitle();
            ...
            if (!mTransferableRoutes.isEmpty()) {
                ...
                if (exists) {
                    String title = dynamicGroup == null
                            ...
                            : dynamicGroup.getController().getTransferableSectionTitle();
                    mItems.add(new Item(title, ITEM_TYPE_HEADER));
                    for (MediaRouter.RouteInfo transferableRoute : mTransferableRoutes) {
                        if (!mSelectedRoute.getId().equals(transferableRoute.getId())) {
        ...
        int getAdapterPositionByRoute(MediaRouter.RouteInfo route) {
            ...
            for (int i = 0; i < mItems.size(); i++) {
                ...
                if (routeId.equals(itemRoute.getId())) {
                    return i;
                }
            }
        ...
        private class RouteViewHolder extends MediaRouteVolumeSliderHolder {
            ...
            public void bindRouteViewHolder(Item item) {
                ...
                if (mSelectedRoute instanceof MediaRouter.DynamicGroupInfo) {
                    ...
                    // Set checked state of checkbox and replace progress bar with route type
                    // icon if selected.
                    if (selected) {
                    ...
                    Drawable checkBoxIcon =
                            MediaRouterThemeHelper.getCheckBoxDrawableIcon(mContext);
