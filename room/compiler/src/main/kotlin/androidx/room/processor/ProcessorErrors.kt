...
object ProcessorErrors {
    ...
    val TRANSACTION_METHOD_ASYNC = "Method annotated with @Transaction must not return a deferred" +
            "/async return type such as LiveData, Flowable, Completable, Single," +
            " ListenableFuture, etc. Since transactions are thread confined and Room cannot" +
            " guarantee that all queries in the method implementation are performed on the same" +
            " thread, then only synchronous @Transaction implemented methods are allowed. If a" +
            " transaction is started and a change of thread is done and waited upon then a" +
            " database deadlock can occur if the additional thread attempts to perform a query." +
            " This restrictions prevents such situation from occurring."
