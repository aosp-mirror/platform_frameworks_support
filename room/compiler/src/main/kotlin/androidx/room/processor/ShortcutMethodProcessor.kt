...
class ShortcutMethodProcessor(
    ...
    fun extractParams(
        ...
        val entities = params.filter { it.pojoType != null }.let {
            if (targetEntity != null) {
                mapOf(extractTargetEntity(targetEntity, params, onValidatePartialEntity))
            } else {
    ...
    private fun extractTargetEntity(
        targetEntity: Entity,
        params: List<ShortcutQueryParameter>,
        onValidatePartialEntity: (Entity, Pojo) -> Unit
    ): Pair<String, ShortcutEntity> {
        if (params.size > 1) {
            context.logger.e(executableElement, ProcessorErrors.TOO_MANY_PARAMETERS_IN_SHOTCUT)
        }

        val param = params.first()
        return if (context.processingEnv.typeUtils.isSameType(targetEntity.type, param.pojoType)) {
            param.name to ShortcutEntity(entity = targetEntity, partialEntity = null)
        } else {
            ...
            param.name to ShortcutEntity(entity = targetEntity, partialEntity = pojo)
        }
    }
