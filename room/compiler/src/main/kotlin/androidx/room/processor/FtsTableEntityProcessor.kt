...
class FtsTableEntityProcessor internal constructor(
    ...
    private fun doProcess(): FtsEntity {
        ...
        val (ftsVersion, ftsAnnotation) = if (element.hasAnnotation(Fts3::class)) {
        ...
        validateExternalContentEntity(ftsOptions.contentEntity, pojo.fields, primaryKey, languageId)
    ...
    private fun validateExternalContentEntity(
        ...
        fields.filterNot {
            // Filter pk and lid fields, these will never be in the external content table.
            primaryKey.fields.isNotEmpty() && primaryKey.fields.first() == it ||
                    languageId.field?.columnName == it.columnName
        }.filterNot {
            contentEntity.fields.any { contentField -> contentField.columnName == it.columnName }
        }.forEach {
            context.logger.e(ProcessorErrors.missingFtsContentField(it.columnName), it.element)
        }
    }
