...
class ShortcutParameterProcessor(
    ...
    fun process(): ShortcutQueryParameter {
        ...
        val entityType = extractEntityType(element.toAnnotationBox(AsEntity::class), pojoType)
        // TODO: Reword error message, hinting on @AsEntity
    ...
    private fun extractEntityType(
        annotationBox: AnnotationBox<AsEntity>?,
        pojoType: TypeMirror?
    ): TypeMirror? {
        val entityType = if (annotationBox != null) {
            annotationBox.getAsTypeMirror("value")
        } else {
            pojoType
        }
        return if (MoreTypes.asElement(entityType).isEntityElement()) {
            entityType
        } else {
            null
        }
