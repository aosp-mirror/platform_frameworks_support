...
abstract class MethodProcessorDelegate(
    ...
    companion object {
        fun createFor(
            ...
            val kotlinMetadata = KotlinMetadataElement(context, containing.asElement())
            return if (kotlinMetadata.isSuspendFunction(executableElement)) {
                val hasCoroutineArtifact = context.processingEnv.elementUtils
                    .getTypeElement(RoomCoroutineTypeNames.COROUTINE_ROOM.toString()) != null
...
class DefaultMethodProcessorDelegate(
    ...
    override fun extractParams(): List<VariableElement> {
...
class SuspendMethodProcessorDelegate(
    ...
    private val continuationType: TypeMirror by lazy {
        context.processingEnv.typeUtils.erasure(
            context.processingEnv.elementUtils
                .getTypeElement(KotlinTypeNames.CONTINUATION.toString())
                .asType()
        )
    }
    private val coroutineParamName: String by lazy {
        val typesUtil = context.processingEnv.typeUtils
        executableElement.parameters.first {
    ...
    override fun findResultBinder(returnType: TypeMirror, query: ParsedQuery) =
        CoroutineResultBinder(
            ...
            continuationParamName = coroutineParamName
        )
