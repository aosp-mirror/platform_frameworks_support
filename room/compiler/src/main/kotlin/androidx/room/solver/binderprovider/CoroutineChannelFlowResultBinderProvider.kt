...
sealed class CoroutineChannelFlowResultBinderProvider(
    val context: Context,
    val channelFlowType: CoroutineChannelFlowResultBinder.Type
    ...
    override fun matches(declared: DeclaredType): Boolean =
        declared.typeArguments.size == 1 && matchesType(declared)

    private fun matchesType(declared: DeclaredType): Boolean {
        val erasure = context.processingEnv.typeUtils.erasure(declared)
        val match = erasure.typeName() == channelFlowType.className
...
class CoroutineChannelResultBinderProvider(context: Context) :
    CoroutineChannelFlowResultBinderProvider(context, CoroutineChannelFlowResultBinder.Type.CHANNEL)
