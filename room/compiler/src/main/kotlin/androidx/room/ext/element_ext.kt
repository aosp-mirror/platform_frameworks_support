...
interface ClassGetter {
    fun callClassFun(methodName: String): TypeMirror?
    fun <T : Annotation> callNestedAnnotationsFun(methodName: String): Array<AnnotationBox<T>>
}
...
fun <T : Annotation> AnnotationMirror.asAnnotation(cl: Class<T>): AnnotationBox<T> {
    if (!cl.isAnnotation) {
    ...
    return AnnotationBox(Proxy.newProxyInstance(cl.classLoader,
        ...
        when (method.name) {
            ClassGetter::callClassFun.name -> map[args[0]]
            "callNestedAnnotationsFun" -> map[args[0]]
            else -> map[method.name]
        }
    })
}

fun <T : Annotation> TypeElement.toAnnotationBox(cl: Class<T>) =
        MoreElements.getAnnotationMirror(this, cl).orNull()?.asAnnotation(cl)
...
private class AnnotationClassVisitor<T : Annotation>(private val annotationClass: Class<T>)
    ...
    override fun visitAnnotation(a: AnnotationMirror?, v: Void?) = a?.asAnnotation(annotationClass)
}
