...
class QueryInterpreterTest {
    ...
    fun embedded() {
        queryWithPojo(
            ...
            assertThat(
                expanded,
                `is`(equalTo("SELECT `id`, `firstName`, `lastName`, `teamId` FROM User"))
            )
        }.compilesWithoutError()
    }
                ...
                expanded, `is`(
                    equalTo(
                        "SELECT `u`.`id`, `u`.`firstName`, `u`.`lastName`, `u`.`teamId`, " +
                                ...
                                "LEFT OUTER JOIN pet p ON u.id = p.ownerId"
                    )
                )
            )
        }.compilesWithoutError()
            ...
            assertThat(
                expanded, `is`(
                    equalTo(
                        "SELECT `User`.`id`, `User`.`firstName`, `User`.`lastName`, " +
                                "`User`.`teamId`, " +
                                "`Pet`.`petId`, `Pet`.`ownerId` FROM User " +
                                "LEFT OUTER JOIN Pet ON User.id = Pet.ownerId"
                    )
                )
            )
        }.compilesWithoutError()
