...
abstract class ShortcutMethodProcessorTest<out T : ShortcutMethod>(
    ...
    fun asEntity() {
        ...
        singleShortcutMethod(
            """
                @${annotation.java.canonicalName}
                abstract public int foo(@AsEntity(User.class) Username username);
                """,
            ...
            when (annotation) {
                ...
                Delete::class ->
                    ProcessorErrors.missingPrimaryKeysInPartialEntityForDelete(
                        partialEntityName = "foo.bar.Username",
                        primaryKeyNames = listOf("uid"))
                else -> throw IllegalStateException("Only @Insert and @Update in this test.")
            }
        )
    }
    ...
    fun asEntityWithRelation() {
        ...
        singleShortcutMethod(
            """
                @${annotation.java.canonicalName}
                abstract public int foo(@AsEntity(User.class) UserPets userPets);
                """,
