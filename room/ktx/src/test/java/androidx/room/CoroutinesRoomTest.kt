...
class CoroutinesRoomTest {
    ...
    @Test
    fun verifyChannel() = testRun {
        var callableExecuted = false
        val channel = CoroutinesRoom.createChannel(
            db = database,
            inTransaction = false,
            tableNames = arrayOf("Pet"),
            callable = Callable { callableExecuted = true }
        )
        yield() // yield for channel creation
