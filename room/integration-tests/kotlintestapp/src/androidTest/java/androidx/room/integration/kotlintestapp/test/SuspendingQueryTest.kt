...
class SuspendingQueryTest : TestDatabaseTest() {
    ...
    fun runSuspendingTransaction() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
                    TestUtil.PUBLISHER.name
                )
    ...
    @Test
    fun withTransactionContext_newThreadDispatcher() {
        runBlocking(NewThreadDispatcher()) {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_ioDispatcher() {
    ...
    @Test
    fun withTransactionContext_contextSwitch() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_exception() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_nested() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_nested_exception() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_nested_finally() {
        runBlocking {
    ...
    @Test
    fun withTransactionContext_nested_contextSwitch() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_childCoroutine_defaultDispatcher() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_childCoroutine_ioDispatcher() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_cancelCoroutine() {
        runBlocking {
            booksDao.insertPublisherSuspend(
                TestUtil.PUBLISHER.publisherId,
                TestUtil.PUBLISHER.name
    ...
    @Test
    fun withTransactionContext_cancelCoroutine_beforeThreadAcquire() {
        runBlocking {
            val job = launch {
                database.withTransactionContext {
    ...
    @Test
    fun withTransactionContext_blockingDaoMethods() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    fun withTransactionContext_blockingDaoMethods_contextSwitch() {
        runBlocking {
            database.withTransactionContext {
                // normal query
                try {
    ...
    fun withTransactionContext_async() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    @Suppress("DeferredResultUnused")
    fun withTransactionContext_async_ioDispatcher() {
        runBlocking {
            database.withTransactionContext {
                booksDao.insertPublisherSuspend(
                    TestUtil.PUBLISHER.publisherId,
    ...
    @Test
    @Suppress("DeferredResultUnused")
    fun withTransactionContext_multipleTransactions() {
        runBlocking {
            booksDao.insertPublisherSuspend(
                TestUtil.PUBLISHER.publisherId,
                TestUtil.PUBLISHER.name
    ...
    @Test
    @Suppress("DeferredResultUnused")
    fun withTransactionContext_multipleTransactions_multipleThreads() {
        runBlocking {
            booksDao.insertPublisherSuspend(
                TestUtil.PUBLISHER.publisherId,
                TestUtil.PUBLISHER.name
    ...
    @Test
    @Suppress("DeferredResultUnused")
    fun withTransactionContext_leakTransactionContext_async() {
        runBlocking {
            val leakedContext = database.withTransactionContext {
                coroutineContext
            }
    ...
    @Test
    fun withTransactionContext_leakTransactionContext_launch() {
        runBlocking {
            val leakedContext = database.withTransactionContext {
                coroutineContext
            }
    ...
    @Test
    fun withTransactionContext_leakTransactionContext_withContext() {
        runBlocking {
            val leakedContext = database.withTransactionContext {
                coroutineContext
            }
    ...
    @Test
    fun withTransactionContext_leakTransactionContext_runBlocking() {
        runBlocking {
            val leakedContext = database.withTransactionContext {
                coroutineContext
            }
    ...
    @Test
    fun withTransactionContext_busyExecutor() {
        runBlocking {
            ...
            val job = async(Dispatchers.IO) {
                asyncExecuted = true
                localDatabase.withTransactionContext {
    ...
    @Test
    fun withTransactionContext_shutdownExecutor() {
        runBlocking {
            ...
            try {
                ...
                fail("An exception should have been thrown by withTransactionContext")
            } catch (ex: IllegalStateException) {
                assertThat(ex).hasMessageThat()
    ...
    @Test
    fun withTransactionContext_databaseOpenError() {
        runBlocking {
            val localDatabase = Room.inMemoryDatabaseBuilder(
                ApplicationProvider.getApplicationContext(), TestDatabase::class.java)
    ...
    @Test
    fun withTransactionContext_beginTransaction_error() {
        runBlocking {
    ...
    @Test
    @Suppress("DEPRECATION")
    fun withTransactionContext_setTransactionSuccessful_error() {
        runBlocking {
            try {
                database.withTransactionContext {
                    // ending transaction prematurely so that setTransactionSuccessful() invoked by
                    // withTransactionContext throws.
                    database.endTransaction()
                }
            } catch (ex: IllegalStateException) {
                assertThat(ex).hasMessageThat()
    ...
    @Test
    @Suppress("DEPRECATION")
    fun withTransactionContext_endTransaction_error() {
        runBlocking {
            try {
                database.withTransactionContext {
                    // ending transaction prematurely and quickly throwing so that endTransaction()
                    // invoked by withTransactionContext throws.
                    database.endTransaction()
