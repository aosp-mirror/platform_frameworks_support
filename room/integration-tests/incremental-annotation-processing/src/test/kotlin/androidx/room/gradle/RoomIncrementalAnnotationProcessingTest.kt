...
class RoomIncrementalAnnotationProcessingTest(private val withIncrementalRoom: Boolean) {
    ...
    companion object {
        ...
        fun parameters() = listOf(
            // true, // Disabled due to bug 134470119
            false
        )
    ...
    // Timestamps of generated files and compiled classes
    private lateinit var fileToTimestampMap: Map<File, Long>

    // Set of generated files and compiled classes that have changed
    private lateinit var changedFiles: Set<File>

    // Set of generated files and compiled classes that have not changed
    private lateinit var unchangedFiles: Set<File>

    // Set of generated files and compiled classes that have been deleted
    ...
    private fun recordChangedAndDeletedFiles() {
    ...
    private fun assertUnchangedFiles(vararg files: File) {
        expect.that(unchangedFiles)
            .named("Unchanged files")
            .containsAtLeastElementsIn(files)
    }

    private fun assertDeletedFiles(vararg files: File) {
        expect.that(deletedFiles)
            .named("Deleted files")
            .containsAtLeastElementsIn(files)
    }
    ...
    fun `verify first full build`() {
        ...
        assertThat(result.task(COMPILE_TASK)!!.outcome).isEqualTo(TaskOutcome.SUCCESS)

        // Check annotation processing outputs
        assertThat(genDatabase1.exists()).isTrue()
        assertThat(genDao1.exists()).isTrue()
        assertThat(genDatabase2.exists()).isTrue()
        assertThat(genDao2.exists()).isTrue()
        assertThat(genSchema1.exists()).isTrue()
        assertThat(genSchema2.exists()).isTrue()

        // Check compilation outputs
        assertThat(classSrcDatabase1.exists()).isTrue()
        assertThat(classSrcDao1.exists()).isTrue()
        assertThat(classSrcEntity1.exists()).isTrue()
        assertThat(classSrcDatabase2.exists()).isTrue()
        assertThat(classSrcDao2.exists()).isTrue()
        assertThat(classSrcEntity2.exists()).isTrue()
        assertThat(classGenDatabase1.exists()).isTrue()
        assertThat(classGenDao1.exists()).isTrue()
        assertThat(classGenDatabase2.exists()).isTrue()
        assertThat(classGenDao2.exists()).isTrue()
    }
    ...
    fun `delete group of source files`() {
        ...
        if (withIncrementalRoom) {
            ...
            assertThat(result.task(COMPILE_TASK)!!.outcome).isEqualTo(TaskOutcome.UP_TO_DATE)
        } else {
            assertThat(result.task(COMPILE_TASK)!!.outcome).isEqualTo(TaskOutcome.SUCCESS)
        }
