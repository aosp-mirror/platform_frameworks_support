...
private suspend fun createTransactionContext(db: RoomDatabase): CoroutineContext {
    ...
    try {
        // Suspend until we acquire a thread from the query executor, preventing the queue from
        // being filled until we are ready to dispatch.
        ...
        // a thread, but we can mark it as released so once it gets acquired it is quickly released.
        isReleased.set(true)
        runnableQueue.put(TransactionElement.RELEASE)
        throw cancellation
    }
