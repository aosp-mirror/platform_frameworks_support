...
suspend fun <R> RoomDatabase.runSuspendingTransaction(block: suspend CoroutineScope.() -> R): R {
    ...
    return withContext(transactionContext) {
        try {
            ...
            val inTransaction = try { inTransaction() } catch (ex: RuntimeException) { false }
            if (!inTransaction) {
...
private suspend fun RoomDatabase.createTransactionContext(): CoroutineContext {
    val job = Job()
    val dispatcher = queryExecutor.acquireTransactionThread(job)
    val transactionElement = TransactionElement(job, dispatcher)
    val threadLocalElement = suspendingTransactionId.asContextElement(System.identityHashCode(job))
...
private suspend fun Executor.acquireTransactionThread(controlJob: Job): ContinuationInterceptor =
    try {
        ...
        // We got cancelled while waiting to acquire a thread, we can't stop our attempt to acquire
        // a thread, but we can cancel the controlling job so once it gets acquired it is quickly
        // released.
        controlJob.cancel()
        throw cancellation
    }
...
internal class TransactionElement(
    private val transactionJob: Job,
