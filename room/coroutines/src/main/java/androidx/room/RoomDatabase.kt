...
suspend fun <R> RoomDatabase.runSuspendingTransaction(block: suspend CoroutineScope.() -> R) {
...
private fun createTransactionContext(db: RoomDatabase): CoroutineContext {
    ...
    val dispatcher = Executor { command -> runnableQueue.put(command) }.asCoroutineDispatcher()
    val transactionElement = TransactionElement(dispatcher, runnableQueue)
    val threadLocalElement = db.suspendingTransactionId.asContextElement(dispatcher.hashCode())
