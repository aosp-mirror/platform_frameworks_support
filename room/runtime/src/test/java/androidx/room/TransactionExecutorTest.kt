...
class TransactionExecutorTest {
    ...
    fun testSerialExecution() {
        ...
        val runnableA = TimingRunnable()
        val runnableB = TimingRunnable()
        val runnableC = TimingRunnable()

        transactionExecutor.execute(runnableA)
        transactionExecutor.execute(runnableB)
        transactionExecutor.execute(runnableC)

        // Sleep for enough time for the runnables to finish.
        Thread.sleep(1000)
    ...
    private class TimingRunnable : Runnable {
        var start: Long = 0
        var finish: Long = 0
