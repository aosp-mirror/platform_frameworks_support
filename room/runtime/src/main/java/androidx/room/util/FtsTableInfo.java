...
public class FtsTableInfo {
    ...
    // A pattern for matching FTS Options including the tokenizer.
    private static final Pattern FTS_OPTIONS_PATTERN = Pattern.compile(
            "(tokenize|compress|content|languageid|matchinfo|notindexed|order|prefix|uncompress)=.*"
    );
    ...
    static Set<String> parseOptions(String createStatement) {
        ...
        for (String arg : args) {
            if (FTS_OPTIONS_PATTERN.matcher(arg).matches()) {
                options.add(arg);
            }
        }
