...
public class TableCreateInfo {
    ...
    private static boolean isCreateSqlEqual(String sql1, String sql2) {
        // Hacky-Hack because "IF NOT EXISTS" is dropped from create sql in sqlite_master when using
        // "CREATE VIRTUAL TABLE". See: <Insert reference archive mail here>
        if (sql1.contains("CREATE VIRTUAL TABLE") || sql2.contains("CREATE VIRTUAL TABLE")) {
            sql1 = sql1.replace("IF NOT EXISTS ", "");
            sql2 = sql2.replace("IF NOT EXISTS ", "");
        }
