...
public class InvalidationTracker {
    ...
    @NonNull
    @VisibleForTesting
    final boolean[] mTableInvalidStatus;
    ...
    public InvalidationTracker(RoomDatabase database, Map<String, String> shadowTablesMap,
        ...
        mTableInvalidStatus = new boolean[tableNames.length];
    }
    ...
    Runnable mRefreshRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (hasUpdatedTable) {
                ...
                Arrays.fill(mTableInvalidStatus, false);
            }
        }

        private boolean checkUpdatedTable() {
            boolean hasUpdatedTable = false;
            Cursor cursor = mDatabase.query(SELECT_UPDATED_TABLES_SQL);
            //noinspection TryFinallyCanBeTryWithResources
            try {
                while (cursor.moveToNext()) {
                    final int tableId = cursor.getInt(0);
                    mTableInvalidStatus[tableId] = true;
