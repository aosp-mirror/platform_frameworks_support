...
class InvalidationLiveDataContainer {
    ...
    InvalidationLiveDataContainer(InvalidationTracker invalidationTracker) {
        mInvalidationTracker = invalidationTracker;
    }

    <T> LiveData<T> create(String[] tableNames, Callable<T> computeFunction) {
        return new RoomTrackingLiveData<>(mInvalidationTracker.mDatabase,
                this, computeFunction, tableNames);
    }

    <T> void onActive(RoomTrackingLiveData liveData) {
        mLiveDataSet.add(liveData);
    }

    <T> void onInactive(RoomTrackingLiveData liveData) {
