...
class MultiInstanceInvalidationClient {
    ...
    // synthetic access
    @SuppressWarnings("WeakerAccess")
    InvalidationTracker.Observer mObserver;
    ...
    final Runnable mSetUpRunnable = new Runnable() {
        ...
        public void run() {
            try {
                mClientId = mService.registerCallback(mCallback, mName);
                mObserver = new InvalidationTracker.Observer(mInvalidationTracker.mTableNames) {
                    @Override
    ...
    private final Runnable mRemoveObserverRunnable = new Runnable() {
        @Override
        public void run() {
    ...
    private final Runnable mTearDownRunnable = new Runnable() {
        ...
        public void run() {
            mInvalidationTracker.removeObserver(mObserver);
            try {
                mService.unregisterCallback(mCallback, mClientId);
            } catch (RemoteException e) {
                Log.w(Room.LOG_TAG, "Can't unregister multi-instance invalidation callback", e);
            }
            if (mContext != null) {
    ...
    /**
     * @param context The Context to be used for binding {@link IMultiInstanceInvalidationService}.
