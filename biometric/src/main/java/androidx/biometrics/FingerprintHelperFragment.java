...
public class FingerprintHelperFragment extends Fragment {
    ...
    protected void cancel(int canceledFrom) {
        ...
        if (canceledFrom == USER_CANCELED_FROM_USER) {
            sendErrorToClient(BiometricConstants.ERROR_USER_CANCELED);
        }
    ...
    private boolean handlePreAuthenticationErrors(FingerprintManagerCompat fingerprintManager) {
        if (!mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_FINGERPRINT)) {
            sendErrorToClient(BiometricConstants.ERROR_HW_NOT_PRESENT);
            return true;
        } else if (!fingerprintManager.isHardwareDetected()) {
            sendErrorToClient(BiometricConstants.ERROR_HW_UNAVAILABLE);
            return true;
        } else if (!fingerprintManager.hasEnrolledFingerprints()) {
            sendErrorToClient(BiometricConstants.ERROR_NO_BIOMETRICS);
            return true;
        }
        return false;
    }
