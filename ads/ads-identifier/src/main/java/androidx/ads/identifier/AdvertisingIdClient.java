...
public class AdvertisingIdClient {
    ...
    @VisibleForTesting
    static final String ADVERTISING_ID_SERVICE = "androidx.ads.identifier.provider.START";
    ...
    BlockingServiceConnection getServiceConnection() throws IOException {
        Intent advertisingIdIntent = new Intent(ADVERTISING_ID_SERVICE);
        advertisingIdIntent.setComponent(mComponentName);

        final BlockingServiceConnection bsc = new BlockingServiceConnection();
        try {
            if (mContext.bindService(advertisingIdIntent, bsc, Service.BIND_AUTO_CREATE)) {
                return bsc;
            }
        } catch (Throwable e) {
