...
public class AdsIdentifierActivity extends Activity {
    ...
    public void listProvider(View view) {
        Intent advertisingIdIntent = new Intent("androidx.ads.identifier.provider.START");

        TextView textView = findViewById(R.id.text);
        textView.setText("Services:\n");
        List<ResolveInfo> resolveInfos = getPackageManager().queryIntentServices(
                advertisingIdIntent, 0);
    ...
    private static boolean isRequestHighPriority(String[] array) {
        ...
        for (String permission : array) {
            if ("androidx.ads.identifier.provider.HIGH_PRIORITY".equals(permission)) {
                return true;
            }
        }
        return false;
