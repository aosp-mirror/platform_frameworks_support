...
class AffectedModuleDetectorImplTest {
    ...
    fun init() {
       ...
       p4   p6
        ...
        Dependency list (arrow reads as "dependent on"):
            p3 -> p1
            p4 -> p3
            p5 -> p1, p2
            p6 -> p2
        ...
        p6config.dependencies.add(p6.dependencies.project(mutableMapOf("path" to ":p1:p3")))
    ...
    fun noChangeCLsOnlyDependent() {
        val detector = AffectedModuleDetectorImpl(
                ...
                logger = logger,
                ignoreUnknownProjects = false,
                projectSubset = ProjectSubset.ONLY_DEPENDENT,
                injectedGitClient = MockGitClient(
                        lastMergeSha = "foo",
                        changedFiles = emptyList())
                )
    ...
    fun noChangeCLsOnlyDirectlyAffected() {
        val detector = AffectedModuleDetectorImpl(
            ...
            logger = logger,
            ignoreUnknownProjects = false,
            projectSubset = ProjectSubset.ONLY_DIRECTLY_AFFECTED,
            injectedGitClient = MockGitClient(
                lastMergeSha = "foo",
                changedFiles = emptyList())
        )
