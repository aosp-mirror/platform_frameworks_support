 ...
 * If requireSameMajorRevision is true then the function returns the latest release within the
 * major release of maxVersionExclusive or null.
 */
private fun getLastReleasedApiFileFromDir(
    ...
    apiDir.listFiles().forEach { file ->
        if (file.name.endsWith(".txt")) {
            val fileVersion = file.name.removeSuffix(".txt")
            val parsed = Version.parseOrNull(fileVersion)
            parsed?.let { version ->
