...
open class VerifyDependencyVersionsTask : DefaultTask() {
    ...
    fun verifyDependencyVersions() {
        println("Ran for project ${project.name}")
    ...
    fun verifyDependencyVersion(dependency: Dependency) {
        // If the version is unspecified then assume an alpha version. If the depending project's
        ...
        if ((projectVersionExtra == "" && dependencyVersionExtra != "") ||
                (dependencyVersionExtra != "" && dependencyVersionExtra < projectVersionExtra)) {
