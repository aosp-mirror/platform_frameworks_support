...
object DokkaSourceDocs {
    ...
    private val ALTERNATE_ARCHIVE_TASK_NAME: String = "generateDocs" // TODO(b/72330103) make "generateDocs" be the only archive task once Doclava is fully removed
    ...
    @Synchronized fun Project.getOrCreateDocsTask(): DokkaTask {
        ...
        if (runnerProject.tasks.findByName(DokkaSourceDocs.RUNNER_TASK_NAME) == null) {
            ...
            if (runnerProject.tasks.findByName(DokkaSourceDocs.ALTERNATE_ARCHIVE_TASK_NAME) == null) {
                runnerProject.tasks.create(ALTERNATE_ARCHIVE_TASK_NAME)
            }
            runnerProject.tasks.getByName(ALTERNATE_ARCHIVE_TASK_NAME).dependsOn(runnerProject.tasks.getByName(ARCHIVE_TASK_NAME))
        }
