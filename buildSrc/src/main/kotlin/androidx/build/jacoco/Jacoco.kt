...
object Jacoco {
    ...
    fun createUberJarTask(project: Project): TaskProvider<Jar> {
        ...
        val task = project.tasks.register("jacocoAntUberJar", Jar::class.java) {
            ...
            it.from(config.resolvedConfiguration.resolvedArtifacts.map { project.zipTree(it.file) }) {
    ...
    fun registerClassFilesTask(project: Project, provider : TaskProvider<out Task>) {
        project.rootProject.tasks.named("packageAllClassFilesForCoverageReport", Jar::class.java).configure {
            it.from(provider.get())// TODO get is probably not necessary
        }
    }
    ...
    fun createCoverageJarTask(project: Project): TaskProvider<Jar> {
        ...
        return project.tasks.register("packageAllClassFilesForCoverageReport",
                Jar::class.java) {
            it.destinationDir = project.getDistributionDirectory()
