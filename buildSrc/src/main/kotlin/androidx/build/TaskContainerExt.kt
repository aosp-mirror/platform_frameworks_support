...
data class AutoRegisteredTasks(
    ...
    companion object {
        ...
        fun get(project: Project): AutoRegisteredTasks {
            val existing = project.extensions.findByName(KEY) as? AutoRegisteredTasks
            if (existing != null) {
                return existing
            }
            return lock.withLock {
                project.extensions.findByName(KEY) as? AutoRegisteredTasks
                    ?: AutoRegisteredTasks().also {
...
inline fun <reified T : Task> Project.maybeRegister(
    ...
    return AutoRegisteredTasks.get(project).once(name) {
