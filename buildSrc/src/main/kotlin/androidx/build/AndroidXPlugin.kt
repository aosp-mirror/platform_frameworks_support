...
class AndroidXPlugin : Plugin<Project> {
            ...
            when (plugin) {
                ...
                is LibraryPlugin -> {
                    ...
                    val reportLibraryMetrics = project.tasks.register(
                        REPORT_LIBRARY_METRICS, ReportLibraryMetrics::class.java
                    )

                    getDebugVariants(project).forEach {
                        reportLibraryMetrics.configure { task ->
                            task.dependsOn(it.packageLibraryProvider)
                        }
                    }
    ...
    companion object {
        ...
        fun getDebugVariants(project: Project): List<LibraryVariant> {
