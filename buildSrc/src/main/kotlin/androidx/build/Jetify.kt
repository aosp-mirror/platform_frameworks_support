...
open class PartiallyDejetifyArchive() : Exec() {

    @Input
    lateinit var jetifierBin: String

    @Input
    lateinit var migrationConfig: String

    @TaskAction
    @Suppress("unused")
    internal fun taskAction() {
        System.out.println("!!!! taskAction !!!!")
...
fun Project.partiallyDejetifyArchiveTask(archiveFile: Provider<RegularFile>):
        TaskProvider<PartiallyDejetifyArchive> {
    // archiveFile : ~/androidx-master-dev/out/dist

    val standaloneProject = project(":jetifier-standalone")
    val stripTask = stripArchiveForPartialDejetificationTask(archiveFile)
    val dejetifierTask = tasks.register("partiallyDejetifyArchive",
        PartiallyDejetifyArchive::class.java) {

        // this depends on might not be neccessary after linking in and out
        ...
        it.jetifierBin = "${standaloneProject.buildDir}/install/jetifier-standalone/bin/" +
                "jetifier-standalone"
        it.migrationConfig = "${standaloneProject.projectDir.getParentFile()}/migration.config"
    }

    return dejetifierTask
}

fun Project.stripArchiveForPartialDejetificationTask(archiveFile: Provider<RegularFile>):
        TaskProvider<Zip> {
    val stripTask = tasks.register("stripArchiveForPartialDejetification", Zip::class.java) {
        // this depends on might not be neccessary after linking in and out
