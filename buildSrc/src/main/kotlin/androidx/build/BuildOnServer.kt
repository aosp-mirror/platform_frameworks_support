...
open class BuildOnServer : DefaultTask() {
    ...
    fun checkAllBuildOutputs() {
        ...
        val perGroupsZipDir = File(distributionDirectory, "per-group-zips")
        if (!perGroupsZipDir.isDirectory || perGroupsZipDir.listFiles().count() != 60) {
            throw FileNotFoundException("buildOnServer required output missing: $perGroupsZipDir")
        }
    }
}

fun Project.createBuildOnServerTask(): BuildOnServer {
    val buildOnServer = tasks.create(AndroidXPlugin.BUILD_ON_SERVER_TASK, BuildOnServer::class.java)
    buildOnServer.distributionDirectory = getDistributionDirectory()
