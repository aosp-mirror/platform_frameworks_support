...
class DependencyTracker(
    ...
    fun debugForBuildServer(
        ...
        logger?.info("changes based on build props repo sha: ${gitClient.findChangedFilesSince(buildShas.repoSha)}")
        logger?.info("changes based on build props build sha: ${gitClient.findChangedFilesSince(buildShas.buildSha)}")
        logger?.info("changes based on build props build sha pre: ${gitClient.findChangedFilesSince("${buildShas.buildSha}^")}")
        logger?.info("changes based on build props repo sha pre: ${gitClient.findChangedFilesSince("${buildShas.repoSha}^")}")
        logger?.info("changes from repo to build shas:${gitClient.findChangedFilesSince(buildShas.repoSha, buildShas.buildSha)}")
        logger?.info("changes from build to repo shas:${gitClient.findChangedFilesSince(buildShas.buildSha, buildShas.repoSha)}")
    }
