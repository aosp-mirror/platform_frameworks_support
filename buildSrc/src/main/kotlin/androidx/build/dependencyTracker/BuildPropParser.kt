...
internal object BuildPropParser {
    ...
    fun getShaForThisBuild(
        ...
        val appliedProps = appliedPropsFile.readLines(Charsets.UTF_8).filterNot { it.isNullOrEmpty() }
        if (appliedProps.isEmpty() && appliedProps.size > 2) {
            logger?.info("We'll run everything because seems like too many things changed or nothing" +
                    " is changed. Changed projects: $appliedProps")
            return null
        }
        val changedProject = appliedProps[0]
        if (changedProject.indexOf("frameworks/support") == -1) {
            logger?.info("Changed project is not frameworks/support. I'll run everything." +
