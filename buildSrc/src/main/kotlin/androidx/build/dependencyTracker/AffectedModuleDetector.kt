...
internal class AffectedModuleDetectorImpl constructor(
    ...
    private fun findLocallyAffectedProjects(): Set<Project> {
        ...
        val alwaysBuild = rootProject.subprojects.filter { project ->
            ALWAYS_BUILD.any {
                project.projectDir.toString().contains(it)
            }
        }.toSet()
    ...
    private fun lookupProjectSetsFromPaths(projectNames: Set<Set<String>>): Set<Set<Project>> {
        val projectSets = mutableSetOf<Set<Project>>()
        projectNames.forEach { cobuiltTestPathSet ->
            projectSets.add(
                rootProject.subprojects.filter { project ->
                    cobuiltTestPathSet.any {
                        logger?.info(project.projectDir.toString())
                        project.projectDir.toString().contains(it)
                    }
                }.toSet()
            )
