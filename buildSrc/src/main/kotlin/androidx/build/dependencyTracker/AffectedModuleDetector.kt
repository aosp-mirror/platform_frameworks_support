 ...
 * CHANGED_PROJECTS, while maintaining our coverage on DEPENDENT_PROJECTS.
 *
 * The ProjectSubset specifies which projects we are interested in testing. With this subset as a
 * guide, the AffectedModuleDetector determines the minimum set of projects that must be built in
 * order to run all the tests successfully.
 *
 * The subsets are:
...
internal class AffectedModuleDetectorImpl constructor(
    ...
    private fun lookupProjectSetsFromNames(projectNames: Set<Set<String>>): Set<Set<Project>> {
        ...
        projectNames.forEach { cobuiltTestNameSet ->
            cobuiltTestNameSet.forEach {
                projectSets.add(
                    setOf(rootProject.project(it))
                )
                }
            }
        return projectSets
