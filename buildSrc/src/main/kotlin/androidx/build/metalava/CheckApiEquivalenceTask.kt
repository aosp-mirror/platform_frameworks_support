...
open class CheckApiEquivalenceTask : DefaultTask() {
    ...
    /**
     * Message to show on comparison failure of private API
     */
    var privateApiFailureMessage: String = "Private API definition has changed. Please run ./gradlew updateApi to confirm\n" +
                "these changes are intentional by updating the private API definition."
    @TaskAction
    fun exec() {
        val publicApi1 = checkNotNull(builtApi?.publicApiFile) { "publicApi1 not set" }
        val privateApi1 = checkNotNull(builtApi?.privateApiFile) { "privateApi1 not set" }
        for (checkedInApi in checkedInApis) {
            val publicApi2 = checkNotNull(checkedInApi?.publicApiFile) { "publicApi2 not set" }
            val privateApi2 = checkNotNull(checkedInApi?.privateApiFile) { "privateApi2 not set" }
            if (!FileUtils.contentEquals(publicApi1, publicApi2)) {
                throw GradleException(publicApiFailureMessage);
            }
            if (!FileUtils.contentEquals(privateApi1, privateApi2)) {
                throw GradleException(privateApiFailureMessage);
            }
        }
    }
}
