...
object Metalava {
    ...
    fun setupProject(
        ...
        val generateApi = project.tasks.register("generateApi", GenerateApiTask::class.java) { t ->
            t.group = "API"
            t.description = "Generates API files from source"
            System.out.println("Registering generateApi for ${project.name}. api:$builtApiLocation")
            t.apiLocation.set(ApiLocation.fromPublicApiFile(
                File(project.docsDir(), "release/${project.name}/current.txt")))
            t.configuration = metalavaConfiguration
            t.generateRestrictedAPIs = extension.trackRestrictedAPIs
            t.dependsOn(metalavaConfiguration)
            applyInputs(javaCompileInputs, t)
        }
