...
open class ListProjectDependencyVersionsTask : DefaultTask() {
    ...
    init {
        group = "Help"
        description = "Creates a json file of the dependency graph in " +
                getProjectDependencyGraphFileName()
    }

    // Label output file so gradle automatically creates it
    @OutputFile
    ...
    private fun getProjectDependencyGraphFileName(): String {
        return project.buildDir.toString() +
                "/dependencyGraph/${project.name}-dependency-graph.json"
    }
...
open class DependencyGraphFileTask : DefaultTask() {
    ...
    // Label output file so gradle automatically creates it
    @OutputFile
    var depGraphFile = File(getDependencyGraphFileName())

    private fun getDependencyGraphFileName(): String {
        return project.buildDir.parentFile.toString() +
                "/dependencyGraph/AndroidXDependencyGraph.json"
    }
