...
open class DependencyGraphFileTask : DefaultTask() {
    ...
    fun createDependencyGraphFile() {
        val depGraphFile = File("./development/dependencyGraph/AndroidXDependencyGraph.json")
        if (!depGraphFile.parentFile.exists()) { return }
        depGraphFile.createNewFile()
        val depGraphDir = depGraphFile.parentFile

        var fileStart: Boolean = true
        var output = StringBuilder()
        output.append("{ \"artifacts\": [\n")
        //depGraphFile.writeText("{ \"artifacts\": [\n", Charsets.UTF_8)
        ...
        depGraphDir.walk().filter { it.isFile and (it.name != depGraphFile.name) }.forEach { file ->
            ...
            //depGraphFile.appendText("$fileText,", Charsets.UTF_8)
            output.append("$fileText,")
//            if (fileStart) {
//                depGraphFile.appendText("{ \"artifacts\": [\n", Charsets.UTF_8)
//                fileStart = false
//            }
            //file.delete()
        }
        // depGraphFile.appendText("]}", Charsets.UTF_8)
        output.append("]}")
