 ...
 * AOT does give us much better performance stability.
 */
private fun Project.injectCompilationForBenchmarks(
    ...
    if (name.endsWith("-benchmark")) {
        ...
        val inject = "cmd package compile -m speed -f $group.benchmark.test"
        // NOTE: we assume here that all benchmarks have apk name $projectname-debug-androidTest.apk
        val options = "/data/local/tmp/$name-debug-androidTest.apk && $inject #".split(" ")
        extension.adbOptions.setInstallOptions(*options.toTypedArray())
    }
}
