...
class RequireSampledAnnotation : Detector(), SourceCodeScanner {
    ...
    private class Enforcer(private val context: JavaContext) : UElementHandler() {
        ...
        private fun handleSampleCode(function: KtDeclaration, node: UMethod) {
            ...
            val allKtFiles = getProjectDirectory().getAllKtFiles()
        ...
        private fun handleSampleLink(kdoc: KDoc, sourceNodeName: String) {
            ...
            tags.forEach tagCheck@{ pair ->
                ...
                val filesToSearch: List<PsiFile> = if (isFile) {
                    ...
                    // Look for PsiFile within this project
                    val file = projectDirectory.searchForFile(filename, packageName)
                        ?: throw IllegalStateException(
                            "Couldn't find $filename in $projectDirectory.name"
                        )
                    listOf(file)
                } else {
                    val packageNames = packageName.split(".")
                    val dir = projectDirectory.searchForDirectory(packageNames)
                        ?: throw IllegalStateException(
                            "Couldn't find a directory for $packageName in $projectDirectory.name"
                        )
                    dir.getAllKtFiles()
                }
        ...
        /**
         * Tries to find the closest module directory that isn't a samples or integration-tests
         * folder, to look for files in
         */
        private fun getProjectDirectory(): PsiDirectory {
            var currentDirectory = context.psiFile!!.parent!!
            // Climb to the src directory
            while (currentDirectory.name != SRC_DIRECTORY) {
                currentDirectory = currentDirectory.parent
                    ?: throw IllegalStateException("Couldn't find src directory")
            }
            // Climb to the module directory
            currentDirectory = currentDirectory.parent!!
            // Look for the nearest non samples/integration-tests directory to consider as the root
            // for the root project
            while (currentDirectory.name == SAMPLES_DIRECTORY ||
                currentDirectory.name == INTEGRATION_TESTS_DIRECTORY) {
                currentDirectory = currentDirectory.parent!!
            }
        ...
        private fun PsiElement.searchForSampleLink(sampleLink: String): Boolean {
            if (this is KDoc) {
                ...
                if (sections.any { section ->
                        ...
                        sampleLinks.any { sampleTag ->
                            sampleTag.getSubjectLink()?.getLinkText() == sampleLink
                        }
                    }) {
                    return true
                }
            }
            return children.any { it.searchForSampleLink(sampleLink) }
        }
    }
