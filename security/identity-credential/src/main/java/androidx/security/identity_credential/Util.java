...
class Util {
    ...
    static void cborPrettyPrintDataItem(StringBuilder sb, int indent, DataItem dataItem) {
        ...
        switch (dataItem.getMajorType()) {
            ...
            case TAG:
                // TODO: Major type 6: optional semantic tagging of other major types.
                throw new RuntimeException("Semantic tags not yet supported");
     ...
     *
     * Only types used for data items are supported.
     *
     * @param value the value to convert
     * @return A DataItem or null if the type is not supported.
     */
    static DataItem valueToCbor(Object value) {
        CborBuilder cborValueBuilder = new CborBuilder();
        if (value instanceof String) {
            cborValueBuilder.add((String) value);
        } else if (value instanceof Long) {
            cborValueBuilder.add(((Long) value).longValue());
        } else if (value instanceof Boolean) {
            cborValueBuilder.add(((Boolean) value).booleanValue());
        } else if (value instanceof byte[]) {
            cborValueBuilder.add((byte[]) value);
        } else {
            return null;
        }
        return cborValueBuilder.build().get(0);
    }

    static DataItem entryNamespaceToCbor(EntryNamespace entryNamespace) throws IdentityCredentialException {
        ...
        for (String entryName : entryNamespace.getEntryNames()) {
            Object entryValue = entryNamespace.getEntryValue(entryName);
            Collection<Integer> accessControlProfileIds = entryNamespace.getAccessControlProfileIds(entryName);

            CborBuilder accessControlProfileIdsBuilder = new CborBuilder();
            ArrayBuilder<CborBuilder> accessControlProfileIdsArrayBuilder = accessControlProfileIdsBuilder.addArray();
            for (int id : accessControlProfileIds) {
                accessControlProfileIdsArrayBuilder.add(id);
            }
            DataItem entryValueDataItem = Util.valueToCbor(entryValue);
            if (entryValueDataItem == null) {
            ...
            entryMapBuilder.put(new UnicodeString("accessControlProfiles"), accessControlProfileIdsBuilder.build().get(0));
            entryMapBuilder.put(new UnicodeString("value"), entryValueDataItem);
        }
        return entryBuilder.build().get(0);
    }

    public static EntryNamespace entryNamespaceFromCbor(String namespaceName, DataItem dataItem) throws IdentityCredentialException {
        ...
        for (DataItem item : array.getDataItems()) {
            ...
            co.nstant.in.cbor.model.Array accessControlProfileArray =  (co.nstant.in.cbor.model.Array) map.get(new UnicodeString("accessControlProfiles"));
            ...
            if (cborValue instanceof UnicodeString) {
                entryBuilder.addStringEntry(
                        name,
                        accessControlProfileIds,
                        ((UnicodeString) cborValue).getString());
            } else if (cborValue instanceof co.nstant.in.cbor.model.Number) {
                entryBuilder.addIntEntry(
                        name,
                        accessControlProfileIds,
                        ((co.nstant.in.cbor.model.Number) cborValue).getValue().intValue());
            } else if (cborValue instanceof SimpleValue) {
                entryBuilder.addBooleanEntry(
                        name,
                        accessControlProfileIds,
                        ((SimpleValue) cborValue) == SimpleValue.TRUE);
            } else if (cborValue instanceof ByteString) {
                entryBuilder.addBytestringEntry(
                        name,
                        accessControlProfileIds,
                        ((ByteString) cborValue).getBytes());
            } else {
                throw new IdentityCredentialException("Unexpected type for value");
            }
        }
    ...
    public static AccessControlProfile accessControlProfileFromCbor(DataItem item) throws IdentityCredentialException {
        if (!(item instanceof co.nstant.in.cbor.model.Map)) {
            throw new IdentityCredentialException("Item is not a map");
        }
        Map map = (Map) item;

        int accessControlProfileId = ((Number) map.get(new UnicodeString("id"))).getValue().intValue();
        AccessControlProfile.Builder builder = new AccessControlProfile.Builder(accessControlProfileId);
        ...
        if (item != null) {
            ...
            try {
                ...
                throw new IdentityCredentialException("Error decoding readerCertificate");
            }
        }
        ...
        if (item != null) {
            ...
            builder.setUserAuthenticationTimeout(item == null ? 0 : (((Number) item).getValue().intValue()));
        }
        return builder.build();
    }

    static DataItem accessControlProfileToCbor(AccessControlProfile accessControlProfile) throws IdentityCredentialException {
        ...
        if (readerCertificate != null) {
            try {
                ...
                e.printStackTrace();
                throw new IdentityCredentialException("Error encoding reader mCertificate");
            }
        }
...
Certificate:
    ...
    static X509Certificate signPublicKeyWithPrivateKey(String keyToSignAlias,
     ...
     * <em>specifically</em> don't check that each certificate is also a leaf certificate.
     *
    ...
    public static boolean validateCertificateChain(Collection<X509Certificate> certificateChain) {
        ...
        for (X509Certificate certificate : certificateChain) {
            if (prevCertificate != null) {
                ...
                // First check the previous certificate was signed.
                try {
                ...
                /*
                // ... and now check this is a CA certificate.
                ...
                int maxNumOfCAsToFollow = certificate.getBasicConstraints();
                if (maxNumOfCAsToFollow == -1) {
                    return false;
                }
                int numCertsToFollow = certChainLength - n - 1;

                // All certs following us must be CA certificates. We are only
                // a valid CA cert if our basic constraints are configured to
                // allow this.
                if (numCertsToFollow > maxNumOfCAsToFollow) {
                    return false;
                }
                */
            } else {
     ...
     * Computes an HKDF.
     *
     * This is based on https://github.com/google/tink/blob/master/java/src/main/java/com/google/crypto/tink/subtle/Hkdf.java
    ...
    static byte[] computeHkdf(
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("No such algorithm: " + macAlgorithm);
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Error MACing");
        }
    }

    // Helper to build proofOfProvisioning and proofOfDeletion CBOR data.
    static byte[] buildProofCbor(String proofString,
            String docType,
