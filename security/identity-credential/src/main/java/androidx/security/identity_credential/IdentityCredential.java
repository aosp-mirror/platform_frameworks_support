...
public abstract class IdentityCredential {
     ...
     */
    public abstract void setReaderEphemeralPublicKey(@NonNull PublicKey readerEphemeralPublicKey) throws IdentityCredentialException;
     ...
     */
    public @NonNull
    abstract byte[] encryptMessageToReader(@NonNull byte[] messagePlaintext) throws IdentityCredentialException;
     ...
     */
    public @NonNull
    abstract byte[] decryptMessageFromReader(@NonNull byte[] messageCiphertext) throws IdentityCredentialException;
     ...
     *
     * If not allowed, {@link #getEntries(Collection, byte[], byte[], Collection, byte[])} and/or
     ...
     * {@link #getEntries(Collection, byte[], byte[], Collection, byte[])} to have any meaningful
     * effect.
     *
    ...
    /**
     * Gets the {@link CryptoObject} that can be used with {@link androidx.biometric.BiometricPrompt}
     * for authenticating the user.
     *
     * @return A {@link CryptoObject}.
     *
     * @throws NoAuthenticationKeyAvailableException
     *           if authentication keys were never provisioned, {@link #setAvailableAuthenticationKeys(int, int)}
     *           was called with {@code keyCount} set to 0 or {@link #setAllowUsingExhaustedKeys(boolean)}
     ...
     * {@link #getEntries(Collection, byte[], byte[], Collection, byte[])}.</p>
     *
     * <p>If this method returns successfully (i.e. without throwing an exception), it must not be
     * called again on this instance.</p>
     *
     * <p>If the {@code requestMessage} parameter is not {@code null}, it must contain CBOR
     ...
     * </pre>
     *
     * <p>If the {@code readerSignature} is not {@code null} it must be an ECDSA signature over
     ...
     *
     * @param requestedEntryNamespaces
     *            A collection of {@link RequestNamespace} objects specifying all of the data
     *            elements to be retrieved, organized by namespace.
     *
     * @param requestMessage if not {@code null}, must contain CBOR data conforming to the
     *                       schema mentioned above. This argument may be {@code null} if
     *                       reader authentication is not used.
     *
     * @param sessionTranscript
     *            If not {@code null}, must contain CBOR data conforming to the schema
     *            mentioned above. This argument may be {@code null} if neither reader
     *            authentication nor dynamic data authentication are used.
     *
     * @param readerCertificateChain
     *            The certificate chain for the reader or {@code null} if reader authentication
     *            is not being used. If not {@code null}, the chain must include at least one
     *            certificate and may contain more.
     *
     * @param readerSignature
     *            An ECDSA signature over the content of the CBOR conforming to the schema
     *            mentioned above. The signature must be made with the public key in the
     *            top-most certificate in {@code readerCertificateChain}. This may be {@code null}
     *            if reader authentication is not being used.
     *
     * @return A {@link IdentityCredential.GetEntryResult} object containing entry data organized
     *         by namespace and a cryptographically authenticated representation of the same data.
     *
     * @throws AlreadyCalledException
     *           if already called on this instance and that call was successful.
     *
     * @throws NoAuthenticationKeyAvailableException
     *           if authentication keys were never provisioned, {@link #setAvailableAuthenticationKeys(int, int)}
     *           was called with {@code keyCount} set to 0 or {@link #setAllowUsingExhaustedKeys(boolean)}
     ...
     *           been exhausted.
     *
     * @throws InvalidReaderCertificateChainException
     *           if the reader presents an invalid certificate chain. Note that the <em>only</em>
     *           check performed is that each certificate is signed by the previous one. In
     *           particular it is not checked whether or not non-leaf certificates are CA
     *           certificates.
     */
    public abstract GetEntryResult getEntries(@NonNull Collection<RequestNamespace> requestedEntryNamespaces,
     ...
     *
     * <p>{@code IdentityCredential}s will select the least-used dynamic authentication key each time
     ...
     *
     * <p>When there aren't enough certified dynamic authentication keys, either because the key count
     ...
     * the {@code IdentityCredential} with {@link #storeStaticAuthenticationData}.</p>
     *
     ...
     */
    public @NonNull
    abstract Collection<X509Certificate> getAuthKeysNeedingCertification() throws IdentityCredentialException;
    ...
    public static class GetEntryResult {
         ...
         * credential.
         *
         * @return null if not implemented, otherwise a message authentication code for the data returned by
         ...
         */
        public @NonNull
        byte[] getStaticAuthenticationData() { return mStaticAuthenticationData;}
    }
}
