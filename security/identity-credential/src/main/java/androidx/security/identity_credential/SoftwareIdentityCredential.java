...
class SoftwareIdentityCredential extends IdentityCredential {
    ...
    private static Collection<RequestNamespace> parseRequestMessage(byte[] requestMessage)
            throws IdentityCredentialException {
        LinkedList<RequestNamespace> result = new LinkedList<>();
        try {
            ...
            e.printStackTrace();
            throw new IdentityCredentialException("Error decoding request message");
        }
        return result;
    }
    ...
    private void ensureAuthKey() throws IdentityCredentialException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new IdentityCredentialException(
                    "Error initializing key for DeviceAuthentication signature");
        }
    ...
    public GetEntryResult getEntries(
            @NonNull byte[] requestMessage,
            @Nullable Collection<RequestNamespace> entriesToNotRequest,
        ...
        Collection<RequestNamespace> requestedEntryNamespaces = parseRequestMessage(requestMessage);
        ...
        // If entriesToNotRequest is non-empty, looking up for every entry is going to
        // be really slow. Build hashtable for O(1) lookup.
        HashSet<Pair<String, String>> entriesToNotRequestHashset = new HashSet<>();
        if (entriesToNotRequest != null) {
            for (RequestNamespace requestNamespace : entriesToNotRequest) {
                String namespace = requestNamespace.getNamespaceName();
                for (String name : requestNamespace.getEntryNames()) {
                    entriesToNotRequestHashset.add(new Pair<>(namespace, name));
                }
            }
        }
        ...
        for (RequestNamespace requestedEntryNamespace : requestedEntryNamespaces) {
            for (EntryNamespace loadedNamespace : mData.getEntryNamespaces()) {
                ...
                if (requestedEntryNamespace.getNamespaceName().equals(
                        loadedNamespace.getNamespaceName())) {
                    ResultNamespace.Builder resultNamespaceBuilder = null;

                    for (String requestedEntryName : requestedEntryNamespace.getEntryNames()) {

                        if (entriesToNotRequestHashset.contains(
                                new Pair<>(requestedEntryNamespace.getNamespaceName(),
                                        requestedEntryName))) {
                            continue;
                        }
                        ...
                        if (value != null) {
                            ...
                            if (status != ResultNamespace.STATUS_OK) {
                                ...
                                deviceNamespaceBuilder.put(new UnicodeString(requestedEntryName),
                                        valueDataItem);
                            }
                        }
                    }
