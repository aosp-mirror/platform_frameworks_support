...
public class SecureCipher {
    ...
    public interface SecureAuthListener {
         ...
         */
        void authComplete(BiometricKeyAuthCallback.BiometricStatus status);
    }
    ...
    public static SecureCipher getDefault(
        ...
        return new SecureCipher(SecureConfig.getStrongConfig(biometricKeyAuthCallback));
    }
     ...
     */
    public void encryptSensitiveData(@NonNull String keyAlias,
            final byte[] clearData, @NonNull final SecureSymmetricEncryptionListener callback) {
        try {
            ...
            if (mSecureConfig.isSymmetricRequireUserAuthEnabled()) {
                mSecureConfig.getBiometricKeyAuthCallback().authenticateKey(cipher,
                        new SecureAuthListener() {
                    public void authComplete(BiometricKeyAuthCallback.BiometricStatus status) {
     ...
     * @param clearData The unencrypted data to encrypt
     * @return A Pair of byte[]'s, first is the encrypted data, second is the
     * IV (initialization vector)
     * used to encrypt which is required for decryption
     */
    public void encryptSensitiveDataAsymmetric(@NonNull String keyAlias,
            byte[] clearData, @NonNull SecureAsymmetricEncryptionListener callback) {
        try {
            KeyStore keyStore = KeyStore.getInstance(mSecureConfig.getAndroidKeyStore());
            keyStore.load(null);
    ...
    public Pair<byte[], byte[]> encryptEphemeralData(
            @NonNull EphemeralSecretKey ephemeralSecretKey, byte[] clearData) {
        try {
     ...
     * @return The byte[] of data that has been decrypted
     */
    public void decryptSensitiveData(@NonNull String keyAlias, final byte[] encryptedData,
                                     byte[] initializationVector,
            final SecureDecryptionListener callback) {
     ...
     * @return The byte[] of data that has been decrypted
     */
    public void decryptSensitiveDataAsymmetric(@NonNull String keyAlias,
            final byte[] encryptedData, @NonNull final SecureDecryptionListener callback) {
        byte[] decryptedData = new byte[0];
        try {
            KeyStore keyStore = KeyStore.getInstance(mSecureConfig.getAndroidKeyStore());
            keyStore.load(null);
            ...
            if (mSecureConfig.isAsymmetricRequireUserAuthEnabled()) {
                mSecureConfig.getBiometricKeyAuthCallback().authenticateKey(cipher, new SecureAuthListener() {
                    public void authComplete(BiometricKeyAuthCallback.BiometricStatus status) {
    ...
    public void decryptEncodedData(byte[] encodedCipherText,
        ...
        switch (encodingType) {
            case EPHEMERAL:
                ...
                decryptSensitiveDataAsymmetric(keyAlias, encodedEphKey,
