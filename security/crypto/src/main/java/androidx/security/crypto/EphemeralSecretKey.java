...
public class EphemeralSecretKey implements KeySpec, SecretKey {
    ...
    public EphemeralSecretKey(byte[] key, @NonNull String algorithm) {
        this(key, algorithm, SecureConfig.getStrongConfig());
    }

    public EphemeralSecretKey(byte[] key, @NonNull SecureConfig secureConfig) {
     ...
     *                                        <code>mKey</code>.
     */
    public EphemeralSecretKey(byte[] key, int offset, int len, @NonNull String algorithm) {
        if (key == null || algorithm == null) {
            throw new IllegalArgumentException("Missing argument");
        }
        if (key.length == 0) {
