...
public abstract class MediaBrowserServiceCompat extends Service {
    ...
    class MediaBrowserServiceImplApi21 implements MediaBrowserServiceImpl, ServiceCompatProxy {
        ...
        MediaBrowserServiceAdaptor mServiceObj;
        Messenger mMessenger;

        @Override
        public void onCreate() {
            mServiceObj = new MediaBrowserServiceAdaptor(
                    MediaBrowserServiceCompat.this, this);
            mServiceObj.onCreate();
        }

        @Override
        public IBinder onBind(Intent intent) {
            return mServiceObj.onBind(intent);
        }
        ...
        public void setSessionToken(final MediaSessionCompat.Token token) {
            mHandler.postOrRun(new Runnable() {
                ...
                public void run() {
                    ...
                    mServiceObj.setSessionToken((MediaSession.Token) token.getToken());
                }
            });
        }
        ...
        void notifyChildrenChangedForFramework(final String parentId, final Bundle options) {
            if (options != null && Build.VERSION.SDK_INT >= 26) {
                mServiceObj.notifyChildrenChanged(parentId, options);
            } else {
                mServiceObj.notifyChildrenChanged(parentId);
            }
        }
        ...
        public Bundle getBrowserRootHints() {
            if (Build.VERSION.SDK_INT >= 26) {
                ...
                return mServiceObj.getBrowserRootHints();
            }
        ...
        public RemoteUserInfo getCurrentBrowserInfo() {
            if (Build.VERSION.SDK_INT >= 28) {
                ...
                android.media.session.MediaSessionManager.RemoteUserInfo userInfoObj =
                        mServiceObj.getCurrentBrowserInfo();
