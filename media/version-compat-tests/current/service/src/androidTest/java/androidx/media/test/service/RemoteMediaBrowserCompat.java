...
import static androidx.media.test.lib.CommonConstants.REMOTE_MEDIA_BROWSER_COMPAT_SERVICE;
...
import android.support.mediacompat.testlib.IRemoteMediaBrowserCompat;
import android.support.v4.media.MediaBrowserCompat;
import android.util.Log;

import androidx.annotation.Nullable;
import androidx.media.test.lib.MediaBrowserCompatConstants;
...
/**
 * Represents remote {@link MediaBrowserCompat} the client app's MediaBrowserCompatService.
...
public class RemoteMediaBrowserCompat {
    ...
    public RemoteMediaBrowserCompat(Context context, ComponentName serviceComponent) {
        ...
        if (!connectToService()) {
            fail("Failed to connect to the RemoteMediaBrowserCompatService.");
        }
     ...
     *                One of the constants {@link MediaBrowserCompatConstants}.
    ...
    public void runCustomTestCommands(int command, @Nullable Bundle args) {
        try {
            mBinder.runCustomTestCommands(mBrowserId, command, args);
    ...
    /**
     * Connects to client app's RemoteMediaBrowserCompatService.
     * Should NOT be called main thread.
     *
    ...
    private boolean connectToService() {
        ...
        intent.setComponent(REMOTE_MEDIA_BROWSER_COMPAT_SERVICE);

        boolean bound = false;
        try {
            bound = mContext.bindService(intent, mServiceConnection, Context.BIND_AUTO_CREATE);
        } catch (Exception ex) {
            Log.e(TAG, "Failed to bind to the RemoteMediaBrowserCompatService.");
        }
    ...
    /**
     * Disconnects from client app's RemoteMediaBrowserCompatService.
     */
    ...
    class MyServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            Log.d(TAG, "Connected to client app's RemoteMediaBrowserCompatService.");
        ...
        public void onServiceDisconnected(ComponentName name) {
            Log.d(TAG, "Disconnected from client app's RemoteMediaBrowserCompatService.");
        }
    }
}
