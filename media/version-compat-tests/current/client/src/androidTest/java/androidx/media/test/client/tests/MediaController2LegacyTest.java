...
public class MediaController2LegacyTest extends MediaSession2TestBase {
    ...
    @Test
    @Ignore("This test doesn't work, since framework MediaSession does not notify controllers "
            + "when playback type is not changed (i.e. remote to remote, local to local).")
    // TODO: Add testControllerCallback_onPlaybackInfoChanged_byRemotePlaybackInfoChange
    public void testControllerCallback_onPlaybackInfoChanged_byLocalStreamChange()
            throws Exception {
        prepareLooper();
        final int testLocalStream = AudioManager.STREAM_ALARM;
        ...
        final ControllerCallback callback = new ControllerCallback() {
            ...
            public void onPlaybackInfoChanged(MediaController2 controller,
                    MediaController2.PlaybackInfo info) {
                assertEquals(testLocalStream, info.getAudioAttributes().getLegacyStreamType());
                latch.countDown();
            }
        };
        mController = createController(mSession.getSessionToken(), true, callback);
        mSession.setPlaybackToLocal(testLocalStream);
        assertTrue(latch.await(TIMEOUT_MS, TimeUnit.MILLISECONDS));
    }
