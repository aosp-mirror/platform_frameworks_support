...
function runTest() {
  ...
  for version in "${TEST_VERSIONS[@]}"; do
    ...
    # Search for the apks.
    # Need to search under out/host instead of out/dist, because apks aren't
    # created under the out/dist for MaxDepVersions
    local CLIENT_APK=$(find ../../out/host -iname "$CLIENT_MODULE_NAME-$version-debug-androidTest.apk")
    local SERVER_APK=$(find ../../out/host -iname "$CLIENT_MODULE_NAME-$version-debug-androidTest.apk")

    # Install the apks
    adb install -r $CLIENT_APK || { echo "Failed to install $CLIENT_APK. Aborting."; exit 1; }
    adb install -r $SERVER_APK || { echo "FAiled to install $SERVER_APK. Aborting."; exit 1; }
